...
package Editor.Ada_RM_Remediation.Diagnostics.Cancelled_Budget_Verdicts is

   --  Case 1386 remediates a concrete remaining gap extracted by the final
   --  inventory: cancelled/superseded/budget final verdict rows must keep
   ...
   type Remediated_Gap_Family is
     (Remaining_Cancelled_Budget_Final_Verdict_Edge,
      Remaining_Superseded_Request_Consumer_Edge,
      Remaining_Gap_Unknown);

   type Verdict_Context is
     (Context_Request_Token,
      Context_Budget_Exhaustion,
      Context_Superseded_Result,
      Context_Final_Verdict_Order,
      Context_Project_Index_Closure,
      Context_Recovered_Source,
      Context_Unknown);

   type Verdict_Form is
     (Verdict_Current_Clean,
      Verdict_Current_Hard_Illegal,
      Verdict_Runtime_Check_Preserved,
      Verdict_Warning_Only_Preserved,
      Verdict_Budget_Exceeded_Indeterminate,
      Verdict_Illegal_Cancelled_Consumed,
      Verdict_Illegal_Superseded_Consumed,
      Verdict_Illegal_Nondeterministic_Final_Order,
      Verdict_Illegal_Consumer_Final_State_Disagreement,
      Verdict_Indeterminate_Stale_Final_Gate,
      Verdict_Unknown);
   ...
   type Remediation_Row is record
      ...
      Context : Verdict_Context := Context_Request_Token;
      Form : Verdict_Form := Verdict_Current_Clean;
      ...
      Nondeterministic_Final_Order : Boolean := False;
      Runtime_Check_Preserved : Boolean := False;
      Warning_Only_Preserved : Boolean := False;
      Budget_Exceeded : Boolean := False;
      Complete_Final_Evidence : Boolean := True;
      Consumer_Final_State_Agrees : Boolean := True;
      Stale_Final_Gate : Boolean := False;
      ...
      Final_Verdict_Fingerprint : Natural := 0;
      Expected_Final_Verdict_Fingerprint : Natural := 0;
