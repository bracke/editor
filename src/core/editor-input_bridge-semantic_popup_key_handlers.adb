...
package body Editor.Input_Bridge.Semantic_Popup_Key_Handlers is
   ...
   begin
      ...
      if Chord.Key = Editor.Keybindings.Key_Escape then
         Execute (Editor.Command_Ids.Command_Semantic_Popup_Dismiss);
         Notify_Input;
         return True;
      end if;
      ...
      case Chord.Key is
         when Editor.Keybindings.Key_Enter =>
            Execute (Editor.Command_Ids.Command_Semantic_Completion_Accept);
         when Editor.Keybindings.Key_Down =>
            Execute (Editor.Command_Ids.Command_Semantic_Completion_Select_Next);
         when Editor.Keybindings.Key_Up =>
            Execute (Editor.Command_Ids.Command_Semantic_Completion_Select_Previous);
