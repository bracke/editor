...
package body Editor.Ada_RM_Remediation.Project_Index.Partial_Context_Index_Guard is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Partial_Context_Indexed
         ...
         when Status_Indeterminate_Recovered_Context
   ...
   begin
      return Row.Id
        ...
        + Natural (Context_Closure'Pos (Row.Closure)) * 5
        + Natural (Context_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Project_Index_Fingerprint
        + Row.Context_Closure_Fingerprint
   ...
   begin
      ...
      elsif Row.Context_Closure_Fingerprint /= Row.Expected_Context_Closure_Fingerprint then
         Set_Status (Item, Status_Context_Closure_Fingerprint_Mismatch);
   ...
   procedure Check_Context_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Project_Index then
         Set_Status (Item, Status_Indeterminate_Stale_Project_Index);
      elsif Row.Partial_Context_Indexed then
         Set_Status (Item, Status_Illegal_Partial_Context_Indexed);
      elsif Row.Duplicate_Unit_Accepted then
         Set_Status (Item, Status_Illegal_Duplicate_Unit_Accepted);
      elsif Row.Private_Child_Leak then
         Set_Status (Item, Status_Illegal_Private_Child_Leak);
      elsif Row.Recovered_Context
        or else Row.Missing_Context_Target
        or else not Row.Complete_Context_Evidence
        or else not Row.Consumer_Context_State_Agrees then
         Set_Status (Item, Status_Indeterminate_Recovered_Context);
      ...
      elsif Row.Form = Form_Current_Context_Resolved then
         Set_Status (Item, Status_Current_Context_Resolved);
      end if;
   end Check_Context_Rule;
