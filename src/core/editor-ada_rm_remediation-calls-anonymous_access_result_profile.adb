...
package body Editor.Ada_RM_Remediation.Calls.Anonymous_Access_Result_Profile is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Result_Profile_Mode_Mismatch
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         return Status_Accessibility_Fingerprint_Mismatch;
   ...
   begin
      ...
      then
        ...
        or else not Row.Consumer_Profile_State_Agrees
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Profile_Evidence then
         return Status_Indeterminate_Stale_Profile_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_View;
      elsif Row.Missing_Profile_Evidence
        or else not Row.Complete_Return_Profile_Evidence
      then
         return Status_Indeterminate_Missing_Profile_Evidence;
      elsif Row.Missing_Accessibility_Evidence
        or else not Row.Complete_Accessibility_Evidence
      then
         return Status_Indeterminate_Missing_Accessibility_Evidence;
      elsif Row.Result_Profile_Mode_Mismatch then
         return Status_Illegal_Result_Profile_Mode_Mismatch;
      elsif Row.Return_Object_Accessibility_Escape then
         return Status_Illegal_Return_Object_Accessibility_Escape;
      elsif Row.Null_Exclusion_Default_Null then
         return Status_Illegal_Null_Exclusion_Default_Null;
      elsif Row.Access_To_Subprogram_Convention_Mismatch then
         return Status_Illegal_Access_To_Subprogram_Convention_Mismatch;
      elsif Row.Anonymous_Access_Discriminant_Profile then
         return Status_Illegal_Anonymous_Access_Discriminant_Profile;
      ...
      elsif Row.Gap = Remaining_Anonymous_Access_Return_Profile_Edge then
         return Status_Anonymous_Access_Return_Profile_Resolved;
      else
   ...
   begin
      return Row.Id * 59
        + Remediation_Status'Pos (Status) * 29
        + Remediated_Gap_Family'Pos (Row.Gap) * 19
        + Anonymous_Access_Return_Form'Pos (Row.Form) * 13
