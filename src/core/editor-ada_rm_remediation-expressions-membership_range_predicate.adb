...
package body Editor.Ada_RM_Remediation.Expressions.Membership_Range_Predicate is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Membership_Result_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Runtime_Membership_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Subject_Type_Mismatch
            | Status_Illegal_Non_Static_Range_In_Static_Context
            | Status_Illegal_Range_Bounds_Reversed
            | Status_Illegal_Static_Predicate_Failure =>
            return Precision.Class_Illegal;
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Membership_Evidence
            ...
            | Status_Membership_Fingerprint_Mismatch
   ...
   begin
      ...
      elsif Row.Membership_Fingerprint /= Row.Expected_Membership_Fingerprint then
         return Status_Membership_Fingerprint_Mismatch;
   ...
   begin
      ...
      elsif not Row.Consumer_Reached or else not Row.Consumer_Membership_State_Agrees then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Membership_Evidence then
         return Status_Indeterminate_Stale_Membership_Evidence;
      elsif Row.Missing_Full_View
        or else Row.Missing_Subject_Type_Evidence
        or else not Row.Complete_Membership_Evidence
      then
         return Status_Indeterminate_Private_View;
      elsif Row.Subject_Type_Mismatch then
         return Status_Illegal_Subject_Type_Mismatch;
      elsif Row.Non_Static_Range_In_Static_Context then
         return Status_Illegal_Non_Static_Range_In_Static_Context;
      elsif Row.Range_Bounds_Reversed then
         return Status_Illegal_Range_Bounds_Reversed;
      elsif Row.Static_Predicate_Failure then
         return Status_Illegal_Static_Predicate_Failure;
      elsif Row.Runtime_Membership_Check_Preserved
        or else Row.Form = Form_Runtime_Membership_Check_Preserved
      then
      ...
      elsif Row.Gap = Remaining_Membership_Range_Predicate_Edge then
         return Status_Membership_Result_Resolved;
      else
   ...
   begin
      return Row.Id * 41
        ...
        + Membership_Form'Pos (Row.Form) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Membership_Fingerprint
...
end Editor.Ada_RM_Remediation.Expressions.Membership_Range_Predicate;
