...
package body Editor.Ada_RM_Remediation.Tagged_Types.Dispatching_Abstract_State_Effects is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Dispatching_Effect_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Abstract_State_Refinement_Mismatch
         ...
         when Status_Indeterminate_Private_State_View
   ...
   begin
      ...
      elsif Row.Abstract_State_Fingerprint /= Row.Expected_Abstract_State_Fingerprint then
         return Status_Abstract_State_Fingerprint_Mismatch;
      elsif Row.Dispatching_Effect_Fingerprint /= Row.Expected_Dispatching_Effect_Fingerprint then
         return Status_Dispatching_Effect_Fingerprint_Mismatch;
   ...
   begin
      if not Row.Inventory_Row_From_Final_Burn_Down then
         return Status_Missing_Final_Inventory_Row;
      ...
      then
        ...
        or else Length (Row.Candidate_Case) = 0
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Abstract_State_Evidence then
         return Status_Indeterminate_Stale_Abstract_State_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_State_View;
      elsif Row.Missing_Abstract_State_Evidence
        or else not Row.Complete_Abstract_State_Evidence
      then
         return Status_Indeterminate_Missing_Abstract_State_Evidence;
      elsif Row.Missing_Dispatching_Effect_Evidence
        or else not Row.Complete_Dispatching_Effect_Evidence
      then
         return Status_Indeterminate_Missing_Dispatching_Effect_Evidence;
      elsif Row.Abstract_State_Refinement_Mismatch then
         return Status_Illegal_Abstract_State_Refinement_Mismatch;
      elsif Row.Dispatching_Global_Effect_Mismatch then
         return Status_Illegal_Dispatching_Global_Effect_Mismatch;
      elsif Row.Depends_Coverage_Missing then
         return Status_Illegal_Depends_Coverage_Missing;
      elsif Row.Overriding_Effect_Weakening then
         return Status_Illegal_Overriding_Effect_Weakening;
      elsif Row.Class_Wide_Effect_Conflict then
         return Status_Illegal_Class_Wide_Effect_Conflict;
      elsif Row.Runtime_Dispatching_Effect_Check_Preserved
        or else Row.Form = Form_Runtime_Dispatching_Effect_Check_Preserved
      then
         return Status_Runtime_Dispatching_Effect_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Global_Depends_Dispatching_Resolved then
         return Status_Global_Depends_Dispatching_Resolved;
      elsif Row.Gap = Remaining_Dispatching_Abstract_State_Effect_Edge then
         return Status_Dispatching_Abstract_State_Effect_Resolved;
      else
   ...
   begin
      return Row.Id * 79
        ...
        + Dispatching_Abstract_State_Effect_Form'Pos (Row.Form) * 13
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Profile_Fingerprint
        + Row.Abstract_State_Fingerprint
        + Row.Dispatching_Effect_Fingerprint
   ...
   begin
      for Row of Input.Rows loop
         ...
         begin
            ...
            if Status in Status_Missing_Final_Inventory_Row .. Status_Multiple_Blockers then
