...
package body Editor.Ada_RM_Remediation.Representation.Low_Level_Access_Representation is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Size_Mismatch
         ...
         when Status_Indeterminate_Missing_Size_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Low_Level_Context'Pos (Row.Context)) * 5
        + Natural (Low_Level_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint + Row.AST_Fingerprint
        + Row.Type_Fingerprint + Row.Profile_Fingerprint
        + Row.Representation_Fingerprint + Row.Policy_Fingerprint
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Size_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Size_Evidence);
   ...
   procedure Check_Low_Level_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Size_Mismatch then
         Set_Status (Item, Status_Illegal_Size_Mismatch);
      elsif Row.Address_Alignment_Mismatch then
         Set_Status (Item, Status_Illegal_Address_Alignment);
      elsif Row.Storage_Pool_Conflict then
         Set_Status (Item, Status_Illegal_Storage_Pool_Conflict);
      elsif Row.Finalization_Hazard then
         Set_Status (Item, Status_Illegal_Finalization_Hazard);
      elsif Row.Restriction_Violation then
         Set_Status (Item, Status_Illegal_Restriction_Violation);
      ...
      elsif Row.Form = Low_Level_Unchecked_Conversion_Agreement then
         Set_Status (Item, Status_Legal_Unchecked_Low_Level_Agreement);
      end if;
   end Check_Low_Level_Rule;
