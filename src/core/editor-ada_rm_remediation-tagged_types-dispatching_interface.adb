...
package body Editor.Ada_RM_Remediation.Tagged_Types.Dispatching_Interface is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Abstract_Primitive_Unimplemented
         ...
         when Status_Indeterminate_Missing_Interface_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Dispatch_Context'Pos (Row.Context)) * 5
        + Natural (Dispatch_Form'Pos (Row.Form)) * 7
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Interface_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Interface_Evidence);
      elsif not Row.Private_Full_View_Available then
         Set_Status (Item, Status_Indeterminate_Private_View);
      elsif Row.Stale_Dispatch_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Dispatch_Evidence);
      end if;
   ...
   procedure Check_Dispatch_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Abstract_Primitive_Unimplemented then
         Set_Status (Item, Status_Illegal_Abstract_Primitive_Unimplemented);
      elsif Row.Null_Procedure_Profile_Mismatch then
         Set_Status (Item, Status_Illegal_Null_Procedure_Profile);
      elsif Row.Ambiguous_Dispatching_Candidate then
         Set_Status (Item, Status_Illegal_Ambiguous_Dispatching_Candidate);
      elsif Row.Class_Wide_Conversion_Mismatch then
         Set_Status (Item, Status_Illegal_Class_Wide_Conversion);
      elsif Row.Effect_Join_Mismatch then
         Set_Status (Item, Status_Illegal_Effect_Join_Mismatch);
      elsif not Row.Consumer_Surface_Agrees then
         Set_Status (Item, Status_Illegal_Consumer_Surface_Disagreement);
      elsif Row.Runtime_Tag_Check then
         Set_Status (Item, Status_Runtime_Tag_Check_Preserved);
      elsif Row.Form = Dispatch_Interface_Call_Agreement then
         Set_Status (Item, Status_Legal_Dispatching_Interface_Agreement);
      end if;
   end Check_Dispatch_Rule;
...
end Editor.Ada_RM_Remediation.Tagged_Types.Dispatching_Interface;
