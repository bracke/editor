...
package body Editor.Ada_RM_Remediation.Expressions.Decimal_Fixed_Point_Attributes is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Rounding_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Non_Decimal_Fixed_Attribute
         ...
         when Status_Indeterminate_Private_Fixed_View
   ...
   begin
      ...
      elsif Row.Attribute_Fingerprint /= Row.Expected_Attribute_Fingerprint then
         return Status_Attribute_Fingerprint_Mismatch;
      elsif Row.Scale_Fingerprint /= Row.Expected_Scale_Fingerprint then
         return Status_Scale_Fingerprint_Mismatch;
   ...
   begin
      ...
      then
        ...
        or else not Row.Consumer_Decimal_State_Agrees
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Scale_Evidence then
         return Status_Indeterminate_Stale_Scale_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_Fixed_View;
      elsif Row.Missing_Type_Evidence
        or else not Row.Complete_Type_Evidence
      then
         return Status_Indeterminate_Missing_Type_Evidence;
      elsif Row.Missing_Attribute_Evidence
        or else not Row.Complete_Attribute_Evidence
      then
         return Status_Indeterminate_Missing_Attribute_Evidence;
      elsif Row.Non_Decimal_Fixed_Attribute then
         return Status_Illegal_Non_Decimal_Fixed_Attribute;
      elsif Row.Delta_Digits_Mismatch then
         return Status_Illegal_Delta_Digits_Mismatch;
      elsif Row.Small_Not_Decimal_Scale then
         return Status_Illegal_Small_Not_Decimal_Scale;
      elsif Row.Attribute_Expected_Type_Mismatch then
         return Status_Illegal_Attribute_Expected_Type_Mismatch;
      elsif Row.Static_Range_Overflow then
         return Status_Illegal_Static_Range_Overflow;
      elsif Row.Runtime_Rounding_Check_Preserved
        or else Row.Form = Form_Runtime_Rounding_Check_Preserved
      then
         return Status_Runtime_Rounding_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Static_Delta_Small_Fore_Aft_Resolved then
         return Status_Static_Delta_Small_Fore_Aft_Resolved;
      elsif Row.Gap = Remaining_Decimal_Fixed_Point_Attribute_Edge then
         return Status_Decimal_Fixed_Point_Attribute_Resolved;
      else
   ...
   begin
      return Row.Id * 73
        ...
        + Decimal_Fixed_Attribute_Form'Pos (Row.Form) * 13
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Profile_Fingerprint
        + Row.Attribute_Fingerprint
        + Row.Scale_Fingerprint
...
end Editor.Ada_RM_Remediation.Expressions.Decimal_Fixed_Point_Attributes;
