...
package body Editor.Ada_RM_Remediation.Expressions.Array_Slice_Bounds is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Non_Array_Prefix
   ...
   begin
      ...
      elsif Row.Index_Fingerprint /= Row.Expected_Index_Fingerprint then
         return Status_Index_Fingerprint_Mismatch;
   ...
   begin
      ...
      elsif not Row.Consumer_Reached or else not Row.Consumer_Array_Index_State_Agrees then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Array_Index_Evidence then
         return Status_Indeterminate_Stale_Array_Index_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_View;
      elsif Row.Missing_Index_Subtype_Evidence or else not Row.Complete_Array_Index_Evidence then
         return Status_Indeterminate_Missing_Index_Subtype_Evidence;
      elsif Row.Non_Array_Prefix then
         return Status_Illegal_Non_Array_Prefix;
      elsif Row.Index_Count_Mismatch then
         return Status_Illegal_Index_Count_Mismatch;
      elsif Row.Index_Type_Mismatch then
         return Status_Illegal_Index_Type_Mismatch;
      elsif Row.Static_Index_Out_Of_Range then
         return Status_Illegal_Static_Index_Out_Of_Range;
      elsif Row.Slice_Bounds_Not_Discrete then
         return Status_Illegal_Slice_Bounds_Not_Discrete;
      elsif Row.Runtime_Index_Check_Preserved
        or else Row.Form = Form_Runtime_Index_Check_Preserved
      then
         return Status_Runtime_Index_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Gap = Remaining_Array_Slice_Index_Constraint_Edge then
         return Status_Array_Slice_Index_Resolved;
      else
   ...
   begin
      return Row.Id * 47
        ...
        + Array_Slice_Index_Form'Pos (Row.Form) * 7
...
end Editor.Ada_RM_Remediation.Expressions.Array_Slice_Bounds;
