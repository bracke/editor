...
package body Editor.Ada_RM_Remediation.Expressions.Case_Expressions is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Case_Result_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Missing_Case_Alternative
            | Status_Illegal_Alternative_Type_Mismatch
            | Status_Illegal_Choice_Overlap =>
         ...
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Case_Evidence
            ...
            | Status_Case_Fingerprint_Mismatch
            | Status_Alternative_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Case_Result_Closure'Pos (Row.Closure)) * 5
        + Natural (Case_Result_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Case_Fingerprint
        + Row.Alternative_Fingerprint
   ...
   begin
      ...
      elsif Row.Case_Fingerprint /= Row.Expected_Case_Fingerprint then
         Set_Status (Item, Status_Case_Fingerprint_Mismatch);
      elsif Row.Alternative_Fingerprint /= Row.Expected_Alternative_Fingerprint then
         Set_Status (Item, Status_Alternative_Fingerprint_Mismatch);
   ...
   procedure Check_Case_Result_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Case_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Case_Evidence);
      elsif Row.Missing_Case_Alternative then
         Set_Status (Item, Status_Illegal_Missing_Case_Alternative);
      elsif Row.Alternative_Type_Mismatch then
         Set_Status (Item, Status_Illegal_Alternative_Type_Mismatch);
      elsif Row.Choice_Overlap then
         Set_Status (Item, Status_Illegal_Choice_Overlap);
      elsif Row.Missing_Full_View
        or else Row.Missing_Subject_Type_Evidence
        or else not Row.Complete_Case_Evidence
        or else not Row.Consumer_Case_State_Agrees then
      ...
      elsif Row.Form = Form_Case_Result_Resolved then
         Set_Status (Item, Status_Case_Result_Resolved);
      end if;
   end Check_Case_Result_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Case_Result_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Expressions.Case_Expressions;
