...
package body Editor.Ada_RM_Remediation.Tagged_Types.Record_Extension_Interface_Aggregate is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Nonextension_Target
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         return Status_Aggregate_Fingerprint_Mismatch;
      elsif Row.Interface_Fingerprint /= Row.Expected_Interface_Fingerprint then
         return Status_Interface_Fingerprint_Mismatch;
   ...
   begin
      ...
      then
        ...
        or else not Row.Consumer_Interface_State_Agrees
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Interface_Evidence then
         return Status_Indeterminate_Stale_Interface_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_View;
      elsif Row.Missing_Aggregate_Evidence
        or else not Row.Complete_Aggregate_Evidence
      then
         return Status_Indeterminate_Missing_Aggregate_Evidence;
      elsif Row.Missing_Interface_Evidence
        or else not Row.Complete_Interface_Evidence
      then
         return Status_Indeterminate_Missing_Interface_Evidence;
      elsif Row.Nonextension_Target then
         return Status_Illegal_Nonextension_Target;
      elsif Row.Missing_Ancestor_Part then
         return Status_Illegal_Missing_Ancestor_Part;
      elsif Row.Ancestor_Type_Incompatible then
         return Status_Illegal_Ancestor_Type_Incompatible;
      elsif Row.Interface_Progenitor_Incomplete then
         return Status_Illegal_Interface_Progenitor_Incomplete;
      elsif Row.Abstract_Interface_Primitive_Unimplemented then
         return Status_Illegal_Abstract_Interface_Primitive_Unimplemented;
      elsif Row.Extension_Component_Association_Mismatch then
         return Status_Illegal_Extension_Component_Association_Mismatch;
      ...
      elsif Row.Gap = Remaining_Record_Extension_Aggregate_Interface_Edge then
         return Status_Record_Extension_Aggregate_Interface_Resolved;
      else
   ...
   begin
      return Row.Id * 67
        ...
        + Record_Extension_Interface_Form'Pos (Row.Form) * 13
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Profile_Fingerprint
        + Row.Aggregate_Fingerprint
        + Row.Interface_Fingerprint
...
end Editor.Ada_RM_Remediation.Tagged_Types.Record_Extension_Interface_Aggregate;
