...
package body Editor.Ada_RM_Remediation.Diagnostics.Cancelled_Budget_Verdicts is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Current_Clean
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Current_Hard_Illegal
            | Status_Illegal_Cancelled_Result_Consumed
            | Status_Illegal_Superseded_Result_Consumed
            | Status_Illegal_Nondeterministic_Final_Order
            | Status_Illegal_Consumer_Final_State_Disagreement =>
         ...
         when Status_Indeterminate_Budget_Exceeded
            | Status_Indeterminate_Stale_Final_Gate
            ...
            | Status_Final_Verdict_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Verdict_Context'Pos (Row.Context)) * 5
        + Natural (Verdict_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Request_Fingerprint
        + Row.Final_Verdict_Fingerprint
   ...
   begin
      ...
      elsif Row.Request_Fingerprint /= Row.Expected_Request_Fingerprint then
         Set_Status (Item, Status_Request_Fingerprint_Mismatch);
      elsif Row.Final_Verdict_Fingerprint /= Row.Expected_Final_Verdict_Fingerprint then
         Set_Status (Item, Status_Final_Verdict_Fingerprint_Mismatch);
   ...
   procedure Check_Verdict_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Final_Gate then
         Set_Status (Item, Status_Indeterminate_Stale_Final_Gate);
      elsif Row.Cancelled_Result_Consumed then
         Set_Status (Item, Status_Illegal_Cancelled_Result_Consumed);
      elsif Row.Superseded_Result_Consumed then
         Set_Status (Item, Status_Illegal_Superseded_Result_Consumed);
      elsif Row.Nondeterministic_Final_Order then
         Set_Status (Item, Status_Illegal_Nondeterministic_Final_Order);
      elsif not Row.Consumer_Final_State_Agrees then
         Set_Status (Item, Status_Illegal_Consumer_Final_State_Disagreement);
      elsif Row.Budget_Exceeded or else not Row.Complete_Final_Evidence then
         Set_Status (Item, Status_Indeterminate_Budget_Exceeded);
      elsif Row.Runtime_Check_Preserved
        or else Row.Form = Verdict_Runtime_Check_Preserved then
         Set_Status (Item, Status_Runtime_Check_Preserved);
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Verdict_Warning_Only_Preserved then
         Set_Status (Item, Status_Warning_Only_Preserved);
      elsif Row.Form = Verdict_Current_Hard_Illegal then
         Set_Status (Item, Status_Current_Hard_Illegal);
      elsif Row.Form = Verdict_Current_Clean then
         Set_Status (Item, Status_Current_Clean);
      end if;
   end Check_Verdict_Rule;
...
end Editor.Ada_RM_Remediation.Diagnostics.Cancelled_Budget_Verdicts;
