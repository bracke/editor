...
package body Editor.Ada_RM_Remediation.Elaboration.Imported_Subprogram_Preelaboration is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Imported_Subprogram_Body_Completion
   ...
   begin
      ...
      elsif Row.Import_Fingerprint /= Row.Expected_Import_Fingerprint then
         return Status_Import_Fingerprint_Mismatch;
      elsif Row.Elaboration_Fingerprint /= Row.Expected_Elaboration_Fingerprint then
         return Status_Elaboration_Fingerprint_Mismatch;
   ...
   begin
      ...
      then
        ...
        or else not Row.Consumer_Elaboration_State_Agrees
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Elaboration_Evidence then
         return Status_Indeterminate_Stale_Elaboration_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_View;
      elsif Row.Missing_Import_Evidence
        or else not Row.Complete_Import_Evidence
      then
         return Status_Indeterminate_Missing_Import_Evidence;
      elsif Row.Missing_Elaboration_Evidence
        or else not Row.Complete_Elaboration_Evidence
      then
         return Status_Indeterminate_Missing_Elaboration_Evidence;
      elsif Row.Imported_Subprogram_Body_Completion then
         return Status_Illegal_Imported_Subprogram_Body_Completion;
      elsif Row.Preelaborate_Elaboration_Call then
         return Status_Illegal_Preelaborate_Elaboration_Call;
      elsif Row.Import_Convention_Missing then
         return Status_Illegal_Import_Convention_Missing;
      elsif Row.External_Link_Name_Conflict then
         return Status_Illegal_External_Link_Name_Conflict;
      elsif Row.Preelaborable_Initialization_Violated then
         return Status_Illegal_Preelaborable_Initialization_Violated;
      elsif Row.Runtime_Elaboration_Check_Preserved
        or else Row.Form = Form_Runtime_Elaboration_Check_Preserved
      then
      ...
      elsif Row.Gap = Remaining_Imported_Subprogram_Preelaborate_Elaboration_Edge then
         return Status_Imported_Subprogram_Preelaborate_Resolved;
      else
   ...
   begin
      return Row.Id * 61
        ...
        + Imported_Preelaborate_Form'Pos (Row.Form) * 13
...
end Editor.Ada_RM_Remediation.Elaboration.Imported_Subprogram_Preelaboration;
