...
package body Editor.Ada_RM_Remediation.Finalization.Exception_Finalization is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Exception_Propagation_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Duplicate_Handler_Choice
         ...
         when Status_Indeterminate_Missing_Handler_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Exception_Context'Pos (Row.Context)) * 5
        + Natural (Exception_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint + Row.AST_Fingerprint
        + Row.Type_Fingerprint + Row.Exception_Fingerprint
        + Row.Finalization_Fingerprint + Row.Consumer_Fingerprint;
   end Row_Fingerprint;
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Handler_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Handler_Evidence);
         ...
         Set_Status (Item, Status_Indeterminate_Stale_Finalization_Evidence);
      end if;
   ...
   begin
      ...
      elsif Row.Exception_Fingerprint /= Row.Expected_Exception_Fingerprint then
         Set_Status (Item, Status_Exception_Fingerprint_Mismatch);
      elsif Row.Finalization_Fingerprint /= Row.Expected_Finalization_Fingerprint then
         Set_Status (Item, Status_Finalization_Fingerprint_Mismatch);
   ...
   procedure Check_Exception_Finalization_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Duplicate_Handler_Choice then
         Set_Status (Item, Status_Illegal_Duplicate_Handler_Choice);
      elsif Row.Unreachable_Handler_Choice then
         Set_Status (Item, Status_Illegal_Unreachable_Handler_Choice);
      elsif Row.Reraise_Outside_Handler then
         Set_Status (Item, Status_Illegal_Reraise_Outside_Handler);
      elsif Row.Handler_Kind_Mismatch then
         Set_Status (Item, Status_Illegal_Handler_Kind_Mismatch);
      elsif Row.Controlled_Finalize_Profile_Mismatch then
         Set_Status (Item, Status_Illegal_Controlled_Finalize_Profile_Mismatch);
      elsif Row.Finalization_Order_Hazard then
         Set_Status (Item, Status_Illegal_Finalization_Order_Hazard);
      elsif Row.Task_Abort_Finalization_Hazard then
         Set_Status (Item, Status_Illegal_Task_Abort_Finalization_Hazard);
      elsif not Row.Consumer_Surface_Agrees then
         Set_Status (Item, Status_Illegal_Consumer_Surface_Disagreement);
      elsif Row.Runtime_Exception_Propagation_Check then
         Set_Status (Item, Status_Runtime_Exception_Propagation_Check_Preserved);
      elsif Row.Form = Exception_Finalization_Compatible then
         Set_Status (Item, Status_Legal_Exception_Finalization_Agreement);
      end if;
   end Check_Exception_Finalization_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Exception_Finalization_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Finalization.Exception_Finalization;
