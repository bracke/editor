...
package body Editor.Ada_RM_Remediation.Expressions.String_Array_Constraints is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Length_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Component_Type_Mismatch
         ...
         when Status_Indeterminate_Private_String_View
   ...
   begin
      ...
      elsif Row.Array_Fingerprint /= Row.Expected_Array_Fingerprint then
         return Status_Array_Fingerprint_Mismatch;
      elsif Row.Encoding_Fingerprint /= Row.Expected_Encoding_Fingerprint then
         return Status_Encoding_Fingerprint_Mismatch;
   ...
   begin
      ...
      then
        ...
        or else not Row.Consumer_String_State_Agrees
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Encoding_Evidence then
         return Status_Indeterminate_Stale_Encoding_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_String_View;
      elsif Row.Missing_Literal_Evidence
        or else not Row.Complete_Literal_Evidence
      then
         return Status_Indeterminate_Missing_Literal_Evidence;
      elsif Row.Missing_Array_Evidence
        or else not Row.Complete_Array_Evidence
      then
         return Status_Indeterminate_Missing_Array_Evidence;
      elsif Row.Component_Type_Mismatch then
         return Status_Illegal_Component_Type_Mismatch;
      elsif Row.Wide_Character_Narrowing then
         return Status_Illegal_Wide_Character_Narrowing;
      elsif Row.Index_Length_Mismatch then
         return Status_Illegal_Index_Length_Mismatch;
      elsif Row.Nonstatic_Bounds_In_Static_Context then
         return Status_Illegal_Nonstatic_Bounds_In_Static_Context;
      elsif Row.Null_String_Non_Null_Range then
         return Status_Illegal_Null_String_Non_Null_Range;
      elsif Row.Runtime_Length_Check_Preserved
        or else Row.Form = Form_Runtime_Length_Check_Preserved
      then
         return Status_Runtime_Length_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Wide_Wide_String_Literal_Resolved then
         return Status_Wide_Wide_String_Literal_Resolved;
      elsif Row.Gap = Remaining_Array_String_Wide_Wide_Character_Edge then
         return Status_String_Literal_Array_Resolved;
      else
...
end Editor.Ada_RM_Remediation.Expressions.String_Array_Constraints;
