...
package body Editor.Ada_RM_Remediation.Tasking.Volatile_Atomic_Protected_Component is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Synchronization_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Atomic_Component_By_Copy
         ...
         when Status_Indeterminate_Private_Protected_View
   ...
   begin
      ...
      elsif Row.Component_Fingerprint /= Row.Expected_Component_Fingerprint then
         return Status_Component_Fingerprint_Mismatch;
      elsif Row.Aspect_Fingerprint /= Row.Expected_Aspect_Fingerprint then
         return Status_Aspect_Fingerprint_Mismatch;
   ...
   begin
      ...
      then
        ...
        or else not Row.Consumer_Concurrent_State_Agrees
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Synchronization_Evidence then
         return Status_Indeterminate_Stale_Synchronization_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_Protected_View;
      elsif Row.Missing_Component_Evidence
        or else not Row.Complete_Component_Evidence
      then
         return Status_Indeterminate_Missing_Component_Evidence;
      elsif Row.Missing_Aspect_Evidence
        or else not Row.Complete_Aspect_Evidence
      then
         return Status_Indeterminate_Missing_Aspect_Evidence;
      elsif Row.Atomic_Component_By_Copy then
         return Status_Illegal_Atomic_Component_By_Copy;
      elsif Row.Volatile_Atomic_Aspect_Conflict then
         return Status_Illegal_Volatile_Atomic_Aspect_Conflict;
      elsif Row.Protected_Component_Discriminant_Conflict then
         return Status_Illegal_Protected_Component_Discriminant_Conflict;
      elsif Row.Nonvolatile_Shared_Update then
         return Status_Illegal_Nonvolatile_Shared_Update;
      elsif Row.Component_Mode_Conflict then
         return Status_Illegal_Component_Mode_Conflict;
      elsif Row.Runtime_Synchronization_Check_Preserved
        or else Row.Form = Form_Runtime_Synchronization_Check_Preserved
      then
         return Status_Runtime_Synchronization_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Protected_Operation_Component_Access_Resolved then
         return Status_Protected_Operation_Component_Access_Resolved;
      elsif Row.Gap = Remaining_Volatile_Atomic_Protected_Component_Edge then
         return Status_Volatile_Atomic_Protected_Component_Resolved;
      else
