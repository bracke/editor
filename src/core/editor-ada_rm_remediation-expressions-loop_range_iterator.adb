...
package body Editor.Ada_RM_Remediation.Expressions.Loop_Range_Iterator is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Bounds_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Non_Discrete_Loop_Range
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         return Status_Static_Fingerprint_Mismatch;
      elsif Row.Iterator_Fingerprint /= Row.Expected_Iterator_Fingerprint then
         return Status_Iterator_Fingerprint_Mismatch;
   ...
   begin
      ...
      elsif not Row.Consumer_Reached or else not Row.Consumer_Iterator_State_Agrees then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Iterator_Evidence then
         return Status_Indeterminate_Stale_Iterator_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_View;
      elsif Row.Missing_Subtype_Evidence or else not Row.Complete_Iterator_Evidence then
         return Status_Indeterminate_Missing_Subtype_Evidence;
      elsif Row.Non_Discrete_Loop_Range then
         return Status_Illegal_Non_Discrete_Loop_Range;
      elsif Row.Reversed_Static_Range then
         return Status_Illegal_Reversed_Static_Range;
      elsif Row.Loop_Parameter_Assignment then
         return Status_Illegal_Loop_Parameter_Assignment;
      elsif Row.Iterator_Element_Type_Mismatch then
         return Status_Illegal_Iterator_Element_Type_Mismatch;
      elsif Row.Parallel_Shared_State_Write then
         return Status_Illegal_Parallel_Shared_State_Write;
      elsif Row.Runtime_Bounds_Check_Preserved
        or else Row.Form = Form_Runtime_Bounds_Check_Preserved
      then
      ...
      elsif Row.Gap = Remaining_Discrete_Subtype_Loop_Range_Iterator_Edge then
         return Status_Discrete_Loop_Iterator_Resolved;
      else
   ...
   begin
      return Row.Id * 47
        ...
        + Discrete_Loop_Iterator_Form'Pos (Row.Form) * 7
