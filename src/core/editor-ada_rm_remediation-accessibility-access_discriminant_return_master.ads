...
package Editor.Ada_RM_Remediation.Accessibility.Access_Discriminant_Return_Master is

   --  Case 1377 remediates a concrete remaining gap extracted by the final
   --  inventory: access discriminants, returned aggregate components,
   --  allocator-created objects, generic access actual substitution, and
   --  controlled/finalized return ownership must use one canonical
   --  master/lifetime/accessibility result.  The pass keeps static escapes
   --  separate from runtime accessibility checks and from indeterminate
   --  missing-master, private-view, and stale-lifetime evidence.
   ...
   type Remediated_Gap_Family is
     (Remaining_Access_Discriminant_Return_Object_Finalization_Edge,
      Remaining_Access_Discriminant_Consumer_Surface_Edge,
      Remaining_Gap_Unknown);

   type Lifetime_Context is
     (Context_Return_Object_Accessibility,
      Context_Access_Discriminant,
      Context_Allocator_Finalization,
      Context_Access_Assignment,
      Context_Generic_Lifetime_Substitution,
      Context_Unknown);

   type Lifetime_Form is
     (Lifetime_Compatible,
      Lifetime_Missing_Master_Evidence,
      Lifetime_Access_Discriminant_Escape,
      Lifetime_Return_Object_Component_Escape,
      Lifetime_Controlled_Finalization_Owner_Mismatch,
      Lifetime_Allocator_Finalization_Hazard,
      Lifetime_Static_Accessibility_Escape,
      Lifetime_Generic_Substitution_Lifetime_Mismatch,
      Lifetime_Runtime_Accessibility_Check,
      Lifetime_Alias_Consumer_Disagreement,
      Lifetime_Private_View_Indeterminate,
      Lifetime_Stale_Lifetime_Evidence,
      Lifetime_Unknown);
   ...
   type Remediation_Row is record
      ...
      Owner : Implementing_Slice := Matrix.Slice_Accessibility_Lifetime;
      ...
      Context : Lifetime_Context := Context_Return_Object_Accessibility;
      Form : Lifetime_Form := Lifetime_Compatible;
      ...
      Master_Evidence_Present : Boolean := True;
      Private_Full_View_Available : Boolean := True;
      Access_Discriminant_Escape : Boolean := False;
      Return_Object_Component_Escape : Boolean := False;
      Controlled_Finalization_Owner_Mismatch : Boolean := False;
      Allocator_Finalization_Hazard : Boolean := False;
      Static_Accessibility_Escape : Boolean := False;
      Generic_Substitution_Lifetime_Mismatch : Boolean := False;
      Runtime_Accessibility_Check : Boolean := False;
      Alias_Consumer_Disagreement : Boolean := False;
      Consumer_Surface_Agrees : Boolean := True;
      Stale_Lifetime_Evidence : Boolean := False;
