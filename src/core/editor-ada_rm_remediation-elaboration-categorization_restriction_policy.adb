...
package body Editor.Ada_RM_Remediation.Elaboration.Categorization_Restriction_Policy is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Legal_Policy_Agreement =>
            return Precision.Class_Legal;
         when Status_Runtime_Assertion_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Warning_Only_Restriction_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Configuration_Placement
            | Status_Illegal_Categorization_Conflict
            | Status_Illegal_No_Allocators_Violation
            | Status_Illegal_Preelaborate_Access_Initializer
            | Status_Illegal_Consumer_Surface_Disagreement =>
            return Precision.Class_Illegal;
         when Status_Indeterminate_Missing_Policy_Evidence
            | Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Policy_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Policy_Context'Pos (Row.Context)) * 5
        + Natural (Policy_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint + Row.AST_Fingerprint
        + Row.Type_Fingerprint + Row.Profile_Fingerprint
        + Row.Policy_Fingerprint + Row.Consumer_Fingerprint;
   end Row_Fingerprint;
      ...
      if not Row.Inventory_Row_From_Final_Burn_Down then
        ...
        or else Row.Matrix_Level_After /= Matrix.Coverage_Covered then
         ...
         Set_Status (Item, Status_Final_Gate_Still_Reports_Gap);
                 ...
                 and then Row.Warning_Only_Test_Present
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Policy_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Policy_Evidence);
      elsif not Row.Private_Full_View_Available then
         Set_Status (Item, Status_Indeterminate_Private_View);
      elsif Row.Stale_Policy_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Policy_Evidence);
      end if;
   ...
   begin
      ...
      elsif Row.Policy_Fingerprint /= Row.Expected_Policy_Fingerprint then
         Set_Status (Item, Status_Policy_Fingerprint_Mismatch);
   ...
   procedure Check_Policy_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Illegal_Configuration_Placement then
         Set_Status (Item, Status_Illegal_Configuration_Placement);
      elsif Row.Categorization_Conflict then
         Set_Status (Item, Status_Illegal_Categorization_Conflict);
      elsif Row.No_Allocators_Violation then
         Set_Status (Item, Status_Illegal_No_Allocators_Violation);
      elsif Row.Preelaborate_Access_Initializer then
         Set_Status (Item, Status_Illegal_Preelaborate_Access_Initializer);
      elsif not Row.Consumer_Surface_Agrees then
         Set_Status (Item, Status_Illegal_Consumer_Surface_Disagreement);
      elsif Row.Warning_Only_Allocator_Restriction then
         Set_Status (Item, Status_Warning_Only_Restriction_Preserved);
      elsif Row.Runtime_Assertion_Check then
         Set_Status (Item, Status_Runtime_Assertion_Check_Preserved);
      elsif Row.Form = Policy_Preelaborate_Restriction_Compatible then
         Set_Status (Item, Status_Legal_Policy_Agreement);
      end if;
   end Check_Policy_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Warning_Only_Restriction_Preserved then
...
end Editor.Ada_RM_Remediation.Elaboration.Categorization_Restriction_Policy;
