...
package Editor.Ada_Architecture_Cleanup is

   --  Project-scale architecture cleanup after the frozen RM remaining-gap
   --  closure.  It does not create another Ada RM semantic edge.  Instead it
   --  classifies editor architecture artifacts as canonical production
   --  surfaces, quarantined historical scaffolding, release documentation, or
   --  covered tests, and rejects alias, compatibility, render-parse,
   --  dirty-state, workspace/keybinding/render mutation, reopened gap,
   --  unowned API, and churn leaks.
   ...
   type Cleanup_Row is record
      ...
      Registered_In_Suite : Boolean := True;
