...
package Editor.Ada_End_To_End_Editor_Integration_Validation is

   --  Project-scale end-to-end editor integration validation.  It verifies
   --  that semantic analysis stays snapshot-owned while the ordinary editor
   --  workflow moves through project open, buffer editing, file-tree
   --  operations, search, outline, diagnostics, build, workspace restore, and
   --  project close/switch surfaces.
...
end Editor.Ada_End_To_End_Editor_Integration_Validation;
