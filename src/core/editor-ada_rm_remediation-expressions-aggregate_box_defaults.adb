...
package body Editor.Ada_RM_Remediation.Expressions.Aggregate_Box_Defaults is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Null_Record_Aggregate_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Runtime_Default_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Null_Aggregate_Not_Allowed
            | Status_Illegal_Box_Default_Missing
            | Status_Illegal_Discriminant_Default_Mismatch
            | Status_Illegal_Limited_Component_Default_Missing =>
            return Precision.Class_Illegal;
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Aggregate_Evidence
            ...
            | Status_Default_Fingerprint_Mismatch
   ...
   begin
      ...
      elsif Row.Default_Fingerprint /= Row.Expected_Default_Fingerprint then
         return Status_Default_Fingerprint_Mismatch;
   ...
   begin
      ...
      elsif not Row.Consumer_Reached or else not Row.Consumer_Aggregate_State_Agrees then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Aggregate_Evidence then
         return Status_Indeterminate_Stale_Aggregate_Evidence;
      elsif Row.Missing_Full_View
        or else Row.Missing_Component_Default_Evidence
        or else not Row.Complete_Aggregate_Evidence
      then
         return Status_Indeterminate_Private_View;
      elsif Row.Null_Aggregate_Not_Allowed then
         return Status_Illegal_Null_Aggregate_Not_Allowed;
      elsif Row.Box_Default_Missing then
         return Status_Illegal_Box_Default_Missing;
      elsif Row.Discriminant_Default_Mismatch then
         return Status_Illegal_Discriminant_Default_Mismatch;
      elsif Row.Limited_Component_Default_Missing then
         return Status_Illegal_Limited_Component_Default_Missing;
      elsif Row.Runtime_Default_Check_Preserved
        or else Row.Form = Form_Runtime_Default_Check_Preserved
      then
         return Status_Runtime_Default_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Gap = Remaining_Null_Record_Aggregate_Box_Default_Edge then
         return Status_Null_Record_Aggregate_Resolved;
      else
   ...
   begin
      return Row.Id * 41
        ...
        + Null_Record_Aggregate_Form'Pos (Row.Form) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Aggregate_Fingerprint
        + Row.Default_Fingerprint
...
end Editor.Ada_RM_Remediation.Expressions.Aggregate_Box_Defaults;
