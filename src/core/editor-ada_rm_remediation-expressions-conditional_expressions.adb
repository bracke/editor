...
package body Editor.Ada_RM_Remediation.Expressions.Conditional_Expressions is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Conditional_Result_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Missing_Branch_Expression
            | Status_Illegal_Branch_Type_Mismatch
            | Status_Illegal_Case_Choice_Overlap =>
         ...
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Conditional_Evidence
            ...
            | Status_Conditional_Fingerprint_Mismatch
            | Status_Branch_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Conditional_Result_Closure'Pos (Row.Closure)) * 5
        + Natural (Conditional_Result_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Conditional_Fingerprint
   ...
   begin
      ...
      elsif Row.Conditional_Fingerprint /= Row.Expected_Conditional_Fingerprint then
         Set_Status (Item, Status_Conditional_Fingerprint_Mismatch);
      elsif Row.Branch_Fingerprint /= Row.Expected_Branch_Fingerprint then
         Set_Status (Item, Status_Branch_Fingerprint_Mismatch);
   ...
   procedure Check_Conditional_Result_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Conditional_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Conditional_Evidence);
      elsif Row.Missing_Branch_Expression then
         Set_Status (Item, Status_Illegal_Missing_Branch_Expression);
      elsif Row.Branch_Type_Mismatch then
         Set_Status (Item, Status_Illegal_Branch_Type_Mismatch);
      elsif Row.Case_Choice_Overlap then
         Set_Status (Item, Status_Illegal_Case_Choice_Overlap);
      elsif Row.Missing_Full_View
        or else Row.Missing_Expected_Type_Evidence
        or else not Row.Complete_Conditional_Evidence
        or else not Row.Consumer_Conditional_State_Agrees then
      ...
      elsif Row.Form = Form_Conditional_Result_Resolved then
         Set_Status (Item, Status_Conditional_Result_Resolved);
      end if;
   end Check_Conditional_Result_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Conditional_Result_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Expressions.Conditional_Expressions;
