...
package Editor.Ada_RM_Remediation.Visibility.Limited_With_Generic_Formal is

   --  Case 1372 remediates a concrete remaining gap extracted by the final
   --  inventory: a limited-with generic formal entity reached through instantiated context and selected-name evidence
   --  must preserve canonical visibility, selected-name, profile, alias-cycle,
   --  and consumer evidence.  This closes a real edge where diagnostics and
   --  navigation can otherwise disagree about whether the renamed declaration is
   --  visible, hidden, ambiguous, or indeterminate.
   ...
   type Remediated_Gap_Family is
     (Remaining_Limited_With_Generic_Formal_View_Edge,
      Remaining_Limited_With_Generic_Formal_Consumer_Surface_Edge,
...
end Editor.Ada_RM_Remediation.Visibility.Limited_With_Generic_Formal;
