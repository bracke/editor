...
package body Editor.Ada_RM_Remediation.Expressions.Delta_Aggregate_Discriminant is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Current_Delta_Update_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Inactive_Variant_Update
            | Status_Illegal_Discriminant_Mismatch
            | Status_Illegal_Controlled_Component_Update =>
   ...
   begin
      return Row.Id
        ...
        + Natural (Aggregate_Closure'Pos (Row.Closure)) * 5
        + Natural (Aggregate_Form'Pos (Row.Form)) * 7
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         Set_Status (Item, Status_Aggregate_Fingerprint_Mismatch);
   ...
   procedure Check_Aggregate_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Aggregate_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Aggregate_Evidence);
      elsif Row.Inactive_Variant_Update then
         Set_Status (Item, Status_Illegal_Inactive_Variant_Update);
      elsif Row.Discriminant_Mismatch then
         Set_Status (Item, Status_Illegal_Discriminant_Mismatch);
      elsif Row.Controlled_Component_Update then
         Set_Status (Item, Status_Illegal_Controlled_Component_Update);
      elsif Row.Missing_Full_View
        or else Row.Missing_Component_Evidence
        or else not Row.Complete_Aggregate_Evidence
        or else not Row.Consumer_Aggregate_State_Agrees then
      ...
      elsif Row.Form = Form_Current_Delta_Update_Resolved then
         Set_Status (Item, Status_Current_Delta_Update_Resolved);
      end if;
   end Check_Aggregate_Rule;
...
end Editor.Ada_RM_Remediation.Expressions.Delta_Aggregate_Discriminant;
