...
package Editor.Ada_Real_Ada_Corpus_Validation is

   --  Project-scale real Ada corpus validation after architecture cleanup.  It
   --  does not reopen Remaining_* remediation.  It records source-shaped Ada
   --  corpus scenarios, expected semantic verdicts, warning/runtime-check
   --  preservation, diagnostic precision, consumer agreement, and stale
   --  evidence rejection.
