...
package Editor.Ada_RM_Remediation.Expressions.Membership_Range_Predicate is

   --  Case 1400 remediates a concrete remaining gap extracted by the final
   --  inventory: membership tests, range membership alternatives, subtype
   --  predicates, runtime membership checks, and semantic consumers must
   --  share one canonical membership/range/predicate result.
   ...
   type Remediated_Gap_Family is
     (Remaining_Membership_Range_Predicate_Edge,
      Remaining_Gap_Unknown);

   type Membership_Closure is
     (Closure_Membership_Test,
      Closure_Range_Alternative,
      Closure_Predicate_Result,
      Closure_Runtime_Membership_Check,
      Closure_Unknown);

   type Membership_Form is
     (Form_Membership_Result_Resolved,
      Form_Runtime_Membership_Check_Preserved,
      Form_Warning_Only_Preserved,
      Form_Illegal_Subject_Type_Mismatch,
      Form_Illegal_Non_Static_Range_In_Static_Context,
      Form_Illegal_Range_Bounds_Reversed,
      Form_Illegal_Static_Predicate_Failure,
      Form_Indeterminate_Private_View,
      Form_Indeterminate_Stale_Membership_Evidence,
      Form_Unknown);
   ...
   type Remediation_Status is
     (Status_Not_Checked,
      ...
      Status_Illegal_Subject_Type_Mismatch,
   ...
   type Remediation_Row is record
      ...
      Owner : Implementing_Slice := Matrix.Slice_Membership_Case_Choice;
      ...
      Closure : Membership_Closure := Closure_Membership_Test;
      Form : Membership_Form := Form_Membership_Result_Resolved;
      ...
      Subject_Type_Mismatch : Boolean := False;
      Non_Static_Range_In_Static_Context : Boolean := False;
      Range_Bounds_Reversed : Boolean := False;
      Static_Predicate_Failure : Boolean := False;
      Stale_Membership_Evidence : Boolean := False;
      Missing_Full_View : Boolean := False;
      Missing_Subject_Type_Evidence : Boolean := False;
      Runtime_Membership_Check_Preserved : Boolean := False;
      Warning_Only_Preserved : Boolean := False;
      Complete_Membership_Evidence : Boolean := True;
      Consumer_Membership_State_Agrees : Boolean := True;
      ...
      Membership_Fingerprint : Natural := 0;
      Expected_Membership_Fingerprint : Natural := 0;
