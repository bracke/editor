...
package body Editor.Ada_RM_Remediation.Expressions.Quantified_Iterator_Predicates is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Quantified_Filter_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Non_Iterable_Domain
   ...
   begin
      return Row.Id
        ...
        + Natural (Quantified_Closure'Pos (Row.Closure)) * 5
        + Natural (Quantified_Form'Pos (Row.Form)) * 7
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         Set_Status (Item, Status_Iterator_Fingerprint_Mismatch);
   ...
   procedure Check_Quantified_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Iterator_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Iterator_Evidence);
      elsif Row.Non_Iterable_Domain then
         Set_Status (Item, Status_Illegal_Non_Iterable_Domain);
        ...
        or else Row.Missing_Iterator_Evidence
        or else not Row.Complete_Iterator_Evidence
        or else not Row.Consumer_Iterator_State_Agrees then
      ...
      elsif Row.Form = Form_Quantified_Filter_Resolved then
         Set_Status (Item, Status_Quantified_Filter_Resolved);
      end if;
   end Check_Quantified_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Quantified_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Expressions.Quantified_Iterator_Predicates;
