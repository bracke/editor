...
package body Editor.Ada_RM_Remediation.Expressions.Fixed_Point_Conversion is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Static_Range_Overflow
         ...
         when Status_Indeterminate_Missing_Target_Fixed_Type
   ...
   begin
      return Row.Id
        ...
        + Natural (Fixed_Point_Context'Pos (Row.Context)) * 5
        + Natural (Fixed_Point_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint + Row.AST_Fingerprint
        + Row.Type_Fingerprint + Row.Static_Fingerprint
        + Row.Profile_Fingerprint + Row.Consumer_Fingerprint;
   end Row_Fingerprint;
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Target_Fixed_Type_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Target_Fixed_Type);
      elsif not Row.Delta_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Delta_Evidence);
      elsif not Row.Digits_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Digits_Evidence);
   ...
   procedure Check_Fixed_Point_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Static_Range_Overflow then
         Set_Status (Item, Status_Illegal_Static_Range_Overflow);
      elsif Row.Static_Delta_Mismatch then
         Set_Status (Item, Status_Illegal_Static_Delta_Mismatch);
      elsif Row.Static_Digits_Mismatch then
         Set_Status (Item, Status_Illegal_Static_Digits_Mismatch);
      elsif Row.Non_Static_Delta_Required then
         Set_Status (Item, Status_Illegal_Non_Static_Delta_Required);
      elsif not Row.Universal_Real_Resolution_Valid then
         Set_Status (Item, Status_Illegal_Universal_Real_Resolution);
      elsif not Row.Rounding_Mode_Agrees then
         Set_Status (Item, Status_Illegal_Rounding_Mode_Disagreement);
         ...
         Set_Status (Item, Status_Runtime_Predicate_Check_Preserved);
      elsif Row.Form = Fixed_Compatible then
         Set_Status (Item, Status_Legal_Fixed_Point_Conversion_Agreement);
      end if;
   end Check_Fixed_Point_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Fixed_Point_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Expressions.Fixed_Point_Conversion;
