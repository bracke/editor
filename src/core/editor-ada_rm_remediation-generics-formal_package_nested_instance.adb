...
package body Editor.Ada_RM_Remediation.Generics.Formal_Package_Nested_Instance is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Instance_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Formal_Package_Mismatch
   ...
   begin
      ...
      elsif Row.Instance_Fingerprint /= Row.Expected_Instance_Fingerprint then
         return Status_Instance_Fingerprint_Mismatch;
   ...
   begin
      ...
      elsif not Row.Consumer_Reached or else not Row.Consumer_Generic_State_Agrees then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Generic_Evidence then
         return Status_Indeterminate_Stale_Generic_Evidence;
      elsif Row.Missing_Full_View
        or else Row.Missing_Nested_Instance_Evidence
        or else not Row.Complete_Generic_Evidence
      then
         return Status_Indeterminate_Private_View;
      elsif Row.Formal_Package_Mismatch then
         return Status_Illegal_Formal_Package_Mismatch;
      elsif Row.Nested_Instance_Actual_Missing then
         return Status_Illegal_Nested_Instance_Actual_Missing;
      elsif Row.Formal_Package_Private_View_Leak then
         return Status_Illegal_Formal_Package_Private_View_Leak;
      elsif Row.Nested_Instance_Cycle then
         return Status_Illegal_Nested_Instance_Cycle;
      elsif Row.Runtime_Instance_Check_Preserved
        or else Row.Form = Form_Runtime_Instance_Check_Preserved
      then
         return Status_Runtime_Instance_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Gap = Remaining_Formal_Package_Nested_Instance_Edge then
         return Status_Formal_Package_Nested_Instance_Resolved;
      else
   ...
   begin
      return Row.Id * 41
        ...
        + Formal_Package_Nested_Instance_Form'Pos (Row.Form) * 7
...
end Editor.Ada_RM_Remediation.Generics.Formal_Package_Nested_Instance;
