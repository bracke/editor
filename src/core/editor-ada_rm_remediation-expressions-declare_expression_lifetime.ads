...
package Editor.Ada_RM_Remediation.Expressions.Declare_Expression_Lifetime is

   --  Case 1395 remediates a concrete remaining gap extracted by the final
   --  inventory: declare expression object lifetime and lifetime evidence must agree with
   --  declare object resolution, static lifetime evaluation,
   ...
   type Remediated_Gap_Family is
     (Remaining_Declare_Expression_Object_Lifetime_Edge,
      Remaining_Declare_Expression_Object_Default_Edge,
      Remaining_Gap_Unknown);

   type Declare_Closure is
     (Closure_Declare_Expression,
      Closure_Declare_Object,
      Closure_Declare_Result,
      Closure_Contract_Declare,
      Closure_Control_Flow_Declare,
      Closure_Unknown);

   type Declare_Form is
     (Form_Declare_Lifetime_Resolved,
      Form_Runtime_Check_Preserved,
      Form_Warning_Only_Preserved,
      Form_Illegal_Missing_Initializer,
      Form_Illegal_Result_Type_Mismatch,
      Form_Illegal_Accessibility_Escape,
      Form_Indeterminate_Private_View,
      Form_Indeterminate_Stale_Declare_Evidence,
      Form_Unknown);
   ...
   type Remediation_Status is
     (Status_Not_Checked,
      ...
      Status_Illegal_Missing_Initializer,
   ...
   type Remediation_Row is record
      ...
      Closure : Declare_Closure := Closure_Declare_Expression;
      Form : Declare_Form := Form_Declare_Lifetime_Resolved;
      ...
      Missing_Initializer : Boolean := False;
      Result_Type_Mismatch : Boolean := False;
      Accessibility_Escape : Boolean := False;
      Stale_Declare_Evidence : Boolean := False;
      Missing_Full_View : Boolean := False;
      Missing_Declare_Evidence : Boolean := False;
      Runtime_Check_Preserved : Boolean := False;
      Warning_Only_Preserved : Boolean := False;
      Complete_Declare_Evidence : Boolean := True;
      Consumer_Declare_State_Agrees : Boolean := True;
      ...
      Declare_Fingerprint : Natural := 0;
      Expected_Declare_Fingerprint : Natural := 0;
