...
package body Editor.Ada_RM_Remediation.Expressions.Reduction_Expression_Combiners is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Reduction_Combiner_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Non_Reducible_Domain
            | Status_Illegal_Seed_Type_Mismatch
            | Status_Illegal_Combiner_Profile_Mismatch =>
         ...
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Reduction_Evidence
            ...
            | Status_Combiner_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Reduction_Closure'Pos (Row.Closure)) * 5
        + Natural (Reduction_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Reduction_Fingerprint
        + Row.Combiner_Fingerprint
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         Set_Status (Item, Status_Reduction_Fingerprint_Mismatch);
      elsif Row.Combiner_Fingerprint /= Row.Expected_Combiner_Fingerprint then
         Set_Status (Item, Status_Combiner_Fingerprint_Mismatch);
   ...
   procedure Check_Reduction_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Reduction_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Reduction_Evidence);
      elsif Row.Non_Reducible_Domain then
         Set_Status (Item, Status_Illegal_Non_Reducible_Domain);
      elsif Row.Seed_Type_Mismatch then
         Set_Status (Item, Status_Illegal_Seed_Type_Mismatch);
      elsif Row.Combiner_Profile_Mismatch then
         Set_Status (Item, Status_Illegal_Combiner_Profile_Mismatch);
      elsif Row.Missing_Full_View
        or else Row.Missing_Reduction_Evidence
        or else not Row.Complete_Reduction_Evidence
        or else not Row.Consumer_Reduction_State_Agrees then
      ...
      elsif Row.Form = Form_Reduction_Combiner_Resolved then
         Set_Status (Item, Status_Reduction_Combiner_Resolved);
      end if;
   end Check_Reduction_Rule;
...
end Editor.Ada_RM_Remediation.Expressions.Reduction_Expression_Combiners;
