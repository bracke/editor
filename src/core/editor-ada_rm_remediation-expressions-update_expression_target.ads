...
package Editor.Ada_RM_Remediation.Expressions.Update_Expression_Target is

   --  Case 1396 remediates a concrete remaining gap extracted by the final
   --  inventory: update expression target-name and lifetime evidence must agree with
   --  update object resolution, static lifetime evaluation,
   ...
   type Remediated_Gap_Family is
     (Remaining_Update_Expression_Target_Name_Edge,
      Remaining_Update_Expression_Component_Default_Edge,
      Remaining_Gap_Unknown);

   type Update_Closure is
     (Closure_Update_Expression,
      Closure_Update_Target,
      Closure_Update_Result,
      Closure_Contract_Update,
      Closure_Control_Flow_Update,
      Closure_Unknown);

   type Update_Form is
     (Form_Update_Target_Resolved,
      Form_Runtime_Check_Preserved,
      Form_Warning_Only_Preserved,
      Form_Illegal_Missing_Target,
      Form_Illegal_Component_Type_Mismatch,
      Form_Illegal_Target_Name_Outside_Update,
      Form_Indeterminate_Private_View,
      Form_Indeterminate_Stale_Update_Evidence,
      Form_Unknown);
   ...
   type Remediation_Row is record
      ...
      Closure : Update_Closure := Closure_Update_Expression;
      Form : Update_Form := Form_Update_Target_Resolved;
      ...
      Missing_Update_Target : Boolean := False;
      Component_Type_Mismatch : Boolean := False;
      Target_Name_Outside_Update : Boolean := False;
      Stale_Update_Evidence : Boolean := False;
      Missing_Full_View : Boolean := False;
      Missing_Update_Evidence : Boolean := False;
      Runtime_Check_Preserved : Boolean := False;
      Warning_Only_Preserved : Boolean := False;
      Complete_Update_Evidence : Boolean := True;
      Consumer_Update_State_Agrees : Boolean := True;
      ...
      Update_Fingerprint : Natural := 0;
      Expected_Update_Fingerprint : Natural := 0;
