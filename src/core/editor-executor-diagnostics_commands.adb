...
package body Editor.Executor.Diagnostics_Commands is
   ...
   begin
      ...
      then
         Report_Info
           (S, Diagnostic_Target_Failure_Label
   ...
   procedure Execute_Problems_Move_Up
     (S : in out Editor.State.State_Type)
      renames Editor.Executor.Diagnostics_Problems_Commands
        .Execute_Problems_Move_Up;

   procedure Execute_Problems_Move_Down
     (S : in out Editor.State.State_Type)
      renames Editor.Executor.Diagnostics_Problems_Commands
        .Execute_Problems_Move_Down;

   procedure Execute_Problems_Page_Up
     (S : in out Editor.State.State_Type)
      renames Editor.Executor.Diagnostics_Problems_Commands
        .Execute_Problems_Page_Up;

   procedure Execute_Problems_Page_Down
     (S : in out Editor.State.State_Type)
      renames Editor.Executor.Diagnostics_Problems_Commands
        .Execute_Problems_Page_Down;

   procedure Execute_Problems_Open_Selected
     (S : in out Editor.State.State_Type)
      renames Editor.Executor.Diagnostics_Problems_Commands
        .Execute_Problems_Open_Selected;
   ...
   begin
      case Kind is
         when Diagnostics_Show =>
            Run (Command_Diagnostics_Show);

         when Diagnostics_Clear =>
            Run (Command_Diagnostics_Clear);

         when Diagnostics_Toggle_Info =>
            Run (Command_Diagnostics_Toggle_Info);

         when Diagnostics_Toggle_Warnings =>
            Run (Command_Diagnostics_Toggle_Warnings);

         when Diagnostics_Toggle_Errors =>
            Run (Command_Diagnostics_Toggle_Errors);

         when Diagnostics_Show_All =>
            Run (Command_Diagnostics_Show_All);

         when Diagnostics_Clear_Filter =>
            Run (Command_Diagnostics_Clear_Filter);

         when Diagnostics_Filter_Errors =>
            Run (Command_Diagnostics_Filter_Errors);

         when Diagnostics_Filter_Warnings =>
            Run (Command_Diagnostics_Filter_Warnings);

         when Diagnostics_Filter_Info_Notes =>
            Run (Command_Diagnostics_Filter_Info_Notes);

         when Diagnostics_Filter_Source =>
            Run (Command_Diagnostics_Filter_Source);

         when Diagnostics_Filter_Build =>
            Run (Command_Diagnostics_Filter_Build);

         when Diagnostics_Clear_Build =>
            Run (Command_Diagnostics_Clear_Build);

         when Diagnostics_Open_Selected =>
            Run (Command_Diagnostics_Open_Selected);
         ...
         when Diagnostic_Open_Source =>
            Run (Command_Diagnostic_Open_Source);

         when Diagnostic_Suppress_Selected =>
            Run (Command_Diagnostic_Suppress_Selected);

         when Diagnostic_Show_Suppressed =>
            Run (Command_Diagnostic_Show_Suppressed);

         when Diagnostic_Restore_Last_Suppressed =>
            Run (Command_Diagnostic_Restore_Last_Suppressed);

         when Diagnostic_Restore_Selected_Suppressed =>
            Run (Command_Diagnostic_Restore_Selected_Suppressed);

         when Diagnostic_Clear_Suppressed =>
            Run (Command_Diagnostic_Clear_Suppressed);

         when Diagnostic_Apply_Quick_Fix =>
            Run (Command_Diagnostic_Apply_Quick_Fix);

         when Diagnostics_Execute_Selected_Action =>
            Run (Command_Diagnostics_Execute_Selected_Action);

         when Diagnostics_Select_Next =>
            Run (Command_Diagnostics_Select_Next);

         when Diagnostics_Select_Previous =>
            Run (Command_Diagnostics_Select_Previous);

         when Diagnostics_Clear_Selected =>
            Run (Command_Diagnostics_Clear_Selected);

         when Diagnostics_Copy_Selected_Text =>
            Run (Command_Diagnostics_Copy_Selected_Text);

         when Diagnostics_Clear_Info =>
            Run (Command_Diagnostics_Clear_Info);

         when Diagnostics_Clear_Warnings =>
            Run (Command_Diagnostics_Clear_Warnings);

         when Diagnostics_Clear_Errors =>
            Run (Command_Diagnostics_Clear_Errors);

         when Diagnostics_Toggle_Editor_Source =>
            Run (Command_Diagnostics_Toggle_Editor_Source);

         when Diagnostics_Toggle_File_Source =>
            Run (Command_Diagnostics_Toggle_File_Source);

         when Diagnostics_Toggle_Project_Source =>
            Run (Command_Diagnostics_Toggle_Project_Source);

         when Diagnostics_Toggle_External_Source =>
            Run (Command_Diagnostics_Toggle_External_Source);

         when Diagnostics_Toggle_Unknown_Source =>
            Run (Command_Diagnostics_Toggle_Unknown_Source);
