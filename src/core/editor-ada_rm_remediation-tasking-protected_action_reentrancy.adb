...
package body Editor.Ada_RM_Remediation.Tasking.Protected_Action_Reentrancy is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Protected_Action_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Reentrant_Protected_Function_Call
   ...
   begin
      ...
      elsif Row.Protected_Action_Fingerprint /= Row.Expected_Protected_Action_Fingerprint then
         return Status_Protected_Action_Fingerprint_Mismatch;
      elsif Row.Call_Graph_Fingerprint /= Row.Expected_Call_Graph_Fingerprint then
         return Status_Call_Graph_Fingerprint_Mismatch;
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Reentrancy_Evidence then
         return Status_Indeterminate_Stale_Reentrancy_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_Protected_View;
      elsif Row.Missing_Protected_Action_Evidence
        or else not Row.Complete_Protected_Action_Evidence
      then
         return Status_Indeterminate_Missing_Protected_Action_Evidence;
      elsif Row.Missing_Call_Graph_Evidence
        or else not Row.Complete_Call_Graph_Evidence
      then
         return Status_Indeterminate_Missing_Call_Graph_Evidence;
      elsif Row.Reentrant_Protected_Function_Call then
         return Status_Illegal_Reentrant_Protected_Function_Call;
      elsif Row.Protected_Procedure_Barrier_Call then
         return Status_Illegal_Protected_Procedure_Barrier_Call;
      elsif Row.Entry_Call_During_Protected_Action then
         return Status_Illegal_Entry_Call_During_Protected_Action;
      elsif Row.Requeue_Target_Reenters_Protected_Object then
         return Status_Illegal_Requeue_Target_Reenters_Protected_Object;
      elsif Row.Select_Alternative_Reentrancy_Conflict then
         return Status_Illegal_Select_Alternative_Reentrancy_Conflict;
      elsif Row.Runtime_Protected_Action_Check_Preserved
        or else Row.Form = Form_Runtime_Protected_Action_Check_Preserved
      then
         return Status_Runtime_Protected_Action_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Protected_Self_Call_Resolved then
         return Status_Protected_Self_Call_Resolved;
      elsif Row.Gap = Remaining_Protected_Action_Reentrancy_Edge then
         return Status_Protected_Action_Reentrancy_Resolved;
      else
   ...
   begin
      return Row.Id * 79
        ...
        + Protected_Action_Reentrancy_Form'Pos (Row.Form) * 13
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Profile_Fingerprint
        + Row.Protected_Action_Fingerprint
        + Row.Call_Graph_Fingerprint
...
end Editor.Ada_RM_Remediation.Tasking.Protected_Action_Reentrancy;
