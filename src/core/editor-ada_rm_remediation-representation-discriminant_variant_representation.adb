...
package body Editor.Ada_RM_Remediation.Representation.Discriminant_Variant_Representation is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Variant_Component_Overlap
   ...
   begin
      return Row.Id
        ...
        + Natural (Representation_Closure'Pos (Row.Closure)) * 5
        + Natural (Representation_Form'Pos (Row.Form)) * 7
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         Set_Status (Item, Status_Discriminant_Fingerprint_Mismatch);
   ...
   procedure Check_Representation_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Representation_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Representation_Evidence);
      elsif Row.Variant_Component_Overlap then
         Set_Status (Item, Status_Illegal_Variant_Component_Overlap);
      elsif Row.Discriminant_Dependent_Layout then
         Set_Status (Item, Status_Illegal_Discriminant_Dependent_Layout);
      elsif Row.Controlled_Component_Layout then
         Set_Status (Item, Status_Illegal_Controlled_Component_Layout);
      elsif Row.Missing_Full_View
        or else Row.Missing_Component_Evidence
        or else not Row.Complete_Representation_Evidence
        or else not Row.Consumer_Representation_State_Agrees then
      ...
      elsif Row.Form = Form_Record_Component_Layout_Resolved then
         Set_Status (Item, Status_Record_Component_Layout_Resolved);
      end if;
   end Check_Representation_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Representation_Rule (Row, Item);
         end if;
