...
package body Editor.Fonts is
   ...
   --  Colour glyphs are packed by Textrender now, into a sheet of its own; what
   --  used to be here was a second shelf packer and a second map of which
   --  codepoint had been packed, both on top of the ones it already kept.
   function Colour_Sheet_Width return Natural is
     (Textrender.Colour_Sheet_Width (The_Backend));

   function Colour_Sheet_Height return Natural is
     (Textrender.Colour_Sheet_Height (The_Backend));

   function Colour_Sheet_Pixels return System.Address is
      Pixels : constant access constant Textrender.Rgba_Buffer :=
        Textrender.Colour_Sheet_Pixels (The_Backend);
   begin
      if Pixels = null then
         return System.Null_Address;
      end if;

      return Pixels.all'Address;
   end Colour_Sheet_Pixels;

   function Colour_Sheet_Dirty return Boolean is
     (Textrender.Colour_Sheet_Dirty (The_Backend));

   procedure Clear_Colour_Sheet_Dirty is
   begin
      Textrender.Clear_Colour_Sheet_Dirty (The_Backend);
   ...
   begin
      ...
      declare
         Code   : constant Textrender.Codepoint := Textrender.Codepoint (CP);
         Colour : Textrender.Colour_Glyph;
         use type Textrender.Status_Code;
      begin
         if Textrender.Has_Colour_Glyph (The_Backend, Code)
           and then Textrender.Get_Colour_Glyph (The_Backend, Code, Colour) = Textrender.Success
           and then Colour.Width > 0
         then
            Metric :=
              (W  => Float (Colour.Width),
               H  => Float (Colour.Height),
               U0 => Colour.U0,
               V0 => Colour.V0,
               U1 => Colour.U1,
               V1 => Colour.V1,
