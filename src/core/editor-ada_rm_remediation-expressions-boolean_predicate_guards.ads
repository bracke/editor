...
package Editor.Ada_RM_Remediation.Expressions.Boolean_Predicate_Guards is

   --  Case 1392 remediates a concrete remaining gap extracted by the final
   --  inventory: boolean short-circuit and predicate evidence must agree with
   --  Boolean expected-type resolution, static predicate evaluation,
   ...
   type Remediated_Gap_Family is
     (Remaining_Boolean_Short_Circuit_Predicate_Edge,
      Remaining_Boolean_Side_Effect_Order_Edge,
      Remaining_Gap_Unknown);

   type Boolean_Closure is
     (Closure_Boolean_Expression,
      Closure_Short_Circuit_Operator,
      Closure_Predicate_Guard,
      Closure_Contract_Condition,
      Closure_Control_Flow_Guard,
      Closure_Unknown);

   type Boolean_Form is
     (Form_Short_Circuit_Resolved,
      Form_Runtime_Check_Preserved,
      Form_Warning_Only_Preserved,
      Form_Illegal_Non_Boolean_Operand,
      Form_Illegal_Static_Predicate_Failure,
      Form_Illegal_Side_Effect_In_Static_Context,
      Form_Indeterminate_Private_View,
      Form_Indeterminate_Stale_Boolean_Evidence,
      Form_Unknown);
   ...
   type Remediation_Status is
     (Status_Not_Checked,
      ...
      Status_Illegal_Non_Boolean_Operand,
   ...
   type Remediation_Row is record
      ...
      Closure : Boolean_Closure := Closure_Boolean_Expression;
      Form : Boolean_Form := Form_Short_Circuit_Resolved;
      ...
      Non_Boolean_Operand : Boolean := False;
      Static_Predicate_Failure : Boolean := False;
      Side_Effect_In_Static_Context : Boolean := False;
      Stale_Boolean_Evidence : Boolean := False;
      Missing_Full_View : Boolean := False;
      Missing_Boolean_Evidence : Boolean := False;
      Runtime_Check_Preserved : Boolean := False;
      Warning_Only_Preserved : Boolean := False;
      Complete_Boolean_Evidence : Boolean := True;
      Consumer_Boolean_State_Agrees : Boolean := True;
      ...
      Boolean_Fingerprint : Natural := 0;
      Expected_Boolean_Fingerprint : Natural := 0;
