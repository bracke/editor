...
package body Editor.Ada_RM_Remediation.Expressions.Declare_Expression_Lifetime is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Declare_Lifetime_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Missing_Initializer
            | Status_Illegal_Result_Type_Mismatch
            | Status_Illegal_Accessibility_Escape =>
         ...
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Declare_Evidence
            ...
            | Status_Declare_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Declare_Closure'Pos (Row.Closure)) * 5
        + Natural (Declare_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Declare_Fingerprint
   ...
   begin
      ...
      elsif Row.Declare_Fingerprint /= Row.Expected_Declare_Fingerprint then
         Set_Status (Item, Status_Declare_Fingerprint_Mismatch);
   ...
   procedure Check_Declare_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Declare_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Declare_Evidence);
      elsif Row.Missing_Initializer then
         Set_Status (Item, Status_Illegal_Missing_Initializer);
      elsif Row.Result_Type_Mismatch then
         Set_Status (Item, Status_Illegal_Result_Type_Mismatch);
      elsif Row.Accessibility_Escape then
         Set_Status (Item, Status_Illegal_Accessibility_Escape);
      elsif Row.Missing_Full_View
        or else Row.Missing_Declare_Evidence
        or else not Row.Complete_Declare_Evidence
        or else not Row.Consumer_Declare_State_Agrees then
      ...
      elsif Row.Form = Form_Declare_Lifetime_Resolved then
         Set_Status (Item, Status_Declare_Lifetime_Resolved);
      end if;
   end Check_Declare_Rule;
...
end Editor.Ada_RM_Remediation.Expressions.Declare_Expression_Lifetime;
