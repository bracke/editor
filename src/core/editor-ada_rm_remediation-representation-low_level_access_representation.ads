...
package Editor.Ada_RM_Remediation.Representation.Low_Level_Access_Representation is

   --  Case 1383 remediates a concrete remaining gap extracted by the final
   --  inventory: unchecked conversion, address clauses, alignment evidence,
   --  storage-pool policy, and lifetime/finalization hazards must share one
   --  canonical low-level access/representation result.
   ...
   type Remediated_Gap_Family is
     (Remaining_Unchecked_Conversion_Address_Alignment_Edge,
      Remaining_Low_Level_Access_Consumer_Surface_Edge,
      Remaining_Gap_Unknown);

   type Low_Level_Context is
     (Context_Unchecked_Conversion_Instantiation,
      Context_Unchecked_Deallocation_Call,
      Context_Address_Clause_For_Access_Object,
      Context_Storage_Pool_Access_Type,
      Context_Alignment_Sensitive_View_Conversion,
      Context_Unknown);

   type Low_Level_Form is
     (Low_Level_Unchecked_Conversion_Agreement,
      Low_Level_Missing_Size_Evidence,
      Low_Level_Illegal_Size_Mismatch,
      Low_Level_Illegal_Address_Alignment,
      Low_Level_Illegal_Storage_Pool_Conflict,
      Low_Level_Illegal_Finalization_Hazard,
      Low_Level_Illegal_Restriction_Violation,
      Low_Level_Runtime_Accessibility_Check,
      Low_Level_Private_View_Indeterminate,
      Low_Level_Stale_Representation_Evidence,
      Low_Level_Consumer_Disagreement,
      Low_Level_Unknown);

   type Remediation_Status is
     (Status_Not_Checked,
      Status_Gap_Remediated,
      Status_Legal_Unchecked_Low_Level_Agreement,
      Status_Runtime_Accessibility_Check_Preserved,
      Status_Illegal_Size_Mismatch,
      Status_Illegal_Address_Alignment,
      Status_Illegal_Storage_Pool_Conflict,
      Status_Illegal_Finalization_Hazard,
      Status_Illegal_Restriction_Violation,
      Status_Illegal_Consumer_Surface_Disagreement,
      Status_Indeterminate_Missing_Size_Evidence,
   ...
   type Remediation_Row is record
      ...
      Context : Low_Level_Context := Context_Unchecked_Conversion_Instantiation;
      Form : Low_Level_Form := Low_Level_Unchecked_Conversion_Agreement;
      ...
      Size_Evidence_Present : Boolean := True;
      Private_Full_View_Available : Boolean := True;
      Size_Mismatch : Boolean := False;
      Address_Alignment_Mismatch : Boolean := False;
      Storage_Pool_Conflict : Boolean := False;
      Finalization_Hazard : Boolean := False;
...
end Editor.Ada_RM_Remediation.Representation.Low_Level_Access_Representation;
