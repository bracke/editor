...
package Editor.Ada_RM_Remediation.Expressions.Conditional_Expressions is

   --  Case 1398 remediates a concrete remaining gap extracted by the final
   --  inventory: conditional expressions must preserve expected-type evidence,
   --  branch result compatibility, Boolean guard legality, static case-choice
   --  evidence, runtime checks, and semantic consumers as one canonical result.
   ...
   type Conditional_Result_Closure is
     (Closure_Conditional_Expression,
      Closure_Branch_Result,
      Closure_Expected_Type_Result,
      Closure_Boolean_Guard,
      Closure_Case_Choice_Result,
      Closure_Unknown);

   type Conditional_Result_Form is
     (Form_Conditional_Result_Resolved,
      Form_Runtime_Check_Preserved,
      Form_Warning_Only_Preserved,
      Form_Illegal_Missing_Branch_Expression,
      Form_Illegal_Branch_Type_Mismatch,
      Form_Illegal_Case_Choice_Overlap,
      Form_Indeterminate_Private_View,
      Form_Indeterminate_Stale_Conditional_Evidence,
      Form_Unknown);
   ...
   type Remediation_Status is
     (Status_Not_Checked,
      ...
      Status_Illegal_Missing_Branch_Expression,
   ...
   type Remediation_Row is record
      ...
      Closure : Conditional_Result_Closure := Closure_Conditional_Expression;
      Form : Conditional_Result_Form := Form_Conditional_Result_Resolved;
      ...
      Missing_Branch_Expression : Boolean := False;
      Branch_Type_Mismatch : Boolean := False;
      Case_Choice_Overlap : Boolean := False;
      Stale_Conditional_Evidence : Boolean := False;
      ...
      Complete_Conditional_Evidence : Boolean := True;
      Consumer_Conditional_State_Agrees : Boolean := True;
      ...
      Conditional_Fingerprint : Natural := 0;
      Expected_Conditional_Fingerprint : Natural := 0;
      Branch_Fingerprint : Natural := 0;
      Expected_Branch_Fingerprint : Natural := 0;
