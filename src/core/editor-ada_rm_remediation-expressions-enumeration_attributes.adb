...
package body Editor.Ada_RM_Remediation.Expressions.Enumeration_Attributes is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Value_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Duplicate_Representation_Code
         ...
         when Status_Indeterminate_Missing_Literal_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Enumeration_Context'Pos (Row.Context)) * 5
        + Natural (Enumeration_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint + Row.AST_Fingerprint
        + Row.Type_Fingerprint + Row.Static_Fingerprint
        + Row.Representation_Fingerprint + Row.Consumer_Fingerprint;
   end Row_Fingerprint;
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Literal_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Literal_Evidence);
         ...
         Set_Status (Item, Status_Indeterminate_Stale_Representation_Evidence);
      end if;
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         Set_Status (Item, Status_Representation_Fingerprint_Mismatch);
   ...
   procedure Check_Enumeration_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Duplicate_Representation_Code then
         Set_Status (Item, Status_Illegal_Duplicate_Representation_Code);
      elsif Row.Incomplete_Representation_Clause then
         Set_Status (Item, Status_Illegal_Incomplete_Representation_Clause);
      elsif Row.Non_Static_Representation_Value then
         Set_Status (Item, Status_Illegal_Non_Static_Representation_Value);
      elsif Row.Attribute_Pos_Out_Of_Range then
         Set_Status (Item, Status_Illegal_Attribute_Pos_Out_Of_Range);
      elsif Row.Attribute_Val_Out_Of_Range then
         Set_Status (Item, Status_Illegal_Attribute_Val_Out_Of_Range);
      elsif Row.Attribute_Succ_Pred_Bounds_Error then
         Set_Status (Item, Status_Illegal_Attribute_Succ_Pred_Bounds);
      elsif Row.Ambiguous_Literal then
         Set_Status (Item, Status_Illegal_Ambiguous_Literal);
      elsif not Row.Consumer_Surface_Agrees then
         Set_Status (Item, Status_Illegal_Consumer_Surface_Disagreement);
      elsif Row.Runtime_Value_Check then
         Set_Status (Item, Status_Runtime_Value_Check_Preserved);
      elsif Row.Form = Enum_Compatible then
         Set_Status (Item, Status_Legal_Enumeration_Attribute_Agreement);
      end if;
   end Check_Enumeration_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Enumeration_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Expressions.Enumeration_Attributes;
