...
with Editor.Buffers.Audits;
with Editor.Buffers.Path_And_Labeling;
with Editor.Buffers.Registry_Metadata;
...
package body Editor.Buffers is
   ...
   begin
      ...
      Editor.File_Tree.Clear (State.Surface.File_Tree);
      Editor.File_Tree_View.Clear_View (State.Surface.File_Tree_View);
   ...
   function Metadata_Label_Max_Length return Positive
     renames Editor.Buffers.Path_And_Labeling.Metadata_Label_Max_Length;
   ...
   function Pure_Same_Or_Descendant_Path
     ...
     renames Editor.Buffers.Path_And_Labeling.Pure_Same_Or_Descendant_Path;
   ...
   function Classify_Buffer_Ownership
     ...
     renames Editor.Buffers.Path_And_Labeling.Classify_Buffer_Ownership;

   function Ownership_Label (Kind : Buffer_Ownership_Kind) return String
     renames Editor.Buffers.Path_And_Labeling.Ownership_Label;

   function Dirty_Category_Label (Kind : Buffer_Dirty_Category) return String
     renames Editor.Buffers.Path_And_Labeling.Dirty_Category_Label;

   function Close_Eligibility_Label (Kind : Buffer_Close_Eligibility) return String
     renames Editor.Buffers.Path_And_Labeling.Close_Eligibility_Label;

   function Workspace_Persistability_Label
     (Kind : Buffer_Workspace_Persistability) return String
     renames Editor.Buffers.Path_And_Labeling.Workspace_Persistability_Label;
   ...
   function Summary_At
     ...
     renames Editor.Buffers.Registry_Metadata.Summary_At;

   function Summary_For
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return Buffer_Summary
     renames Editor.Buffers.Registry_Metadata.Summary_For;
   ...
   function Metadata_For
     ...
     renames Editor.Buffers.Registry_Metadata.Metadata_For;

   function Project_Lifecycle_Buffer_Sets
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State) return Buffer_Project_Lifecycle_Sets
     renames Editor.Buffers.Audits.Project_Lifecycle_Buffer_Sets;

   function Audit_Buffers
     (Registry    : Buffer_Registry;
      Project     : Editor.Project.Project_State;
      Selected_Id : Buffer_Id := No_Buffer) return Buffer_Audit_Summary
     renames Editor.Buffers.Audits.Audit_Buffers;

   function Buffer_Metadata_Lifecycle_Audit_Coherent
     (Registry    : Buffer_Registry;
      Project     : Editor.Project.Project_State;
      Selected_Id : Buffer_Id := No_Buffer) return Boolean
     renames Editor.Buffers.Audits.Buffer_Metadata_Lifecycle_Audit_Coherent;

   function Project_Owned_Buffer_Count
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State) return Natural
     renames Editor.Buffers.Audits.Project_Owned_Buffer_Count;

   function Outside_Project_Buffer_Count
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State) return Natural
     renames Editor.Buffers.Audits.Outside_Project_Buffer_Count;

   function Scratch_Buffer_Count
     (Registry : Buffer_Registry) return Natural
     renames Editor.Buffers.Audits.Scratch_Buffer_Count;

   function Project_Owned_Dirty_Buffer_Count
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State) return Natural
     renames Editor.Buffers.Audits.Project_Owned_Dirty_Buffer_Count;

   function Outside_Project_Dirty_Buffer_Count
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State) return Natural
     renames Editor.Buffers.Audits.Outside_Project_Dirty_Buffer_Count;

   function Scratch_Dirty_Buffer_Count
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State) return Natural
     renames Editor.Buffers.Audits.Scratch_Dirty_Buffer_Count;

   function Categorized_Dirty_Buffer_Summary
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State)
      return Editor.Dirty_Guards.Dirty_Buffer_Summary
     renames Editor.Buffers.Audits.Categorized_Dirty_Buffer_Summary;

   function Project_Lifecycle_Dirty_Buffer_Summary
     (Registry : Buffer_Registry;
      Project  : Editor.Project.Project_State)
      return Editor.Dirty_Guards.Dirty_Buffer_Summary
     renames Editor.Buffers.Audits.Project_Lifecycle_Dirty_Buffer_Summary;
   ...
   begin
      if Registry.Has_Active_Group
        and then Editor.Buffers.Registry_Metadata.First_Buffer_In_Group
          (Registry, To_String (Registry.Active_Group)) = No_Buffer
      then
         Registry.Has_Active_Group := False;
         Registry.Active_Group := Null_Unbounded_String;
      end if;
   ...
   function Display_Name
     ...
     renames Editor.Buffers.Registry_Metadata.Display_Name;

   function Display_Label
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return String
     renames Editor.Buffers.Registry_Metadata.Display_Label;

   function Is_Dirty
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return Boolean
     renames Editor.Buffers.Registry_Metadata.Is_Dirty;

   function Is_File_Backed
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return Boolean
     renames Editor.Buffers.Registry_Metadata.Is_File_Backed;

   function Is_Buffer_Pinned
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return Boolean
     renames Editor.Buffers.Registry_Metadata.Is_Buffer_Pinned;

   function Has_Buffer_Label
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return Boolean
     renames Editor.Buffers.Registry_Metadata.Has_Buffer_Label;

   function Buffer_Label
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return String
     renames Editor.Buffers.Registry_Metadata.Buffer_Label;

   procedure Set_Buffer_Label
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id;
      Label    : String)
     renames Editor.Buffers.Registry_Metadata.Set_Buffer_Label;

   procedure Clear_Buffer_Label
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id)
     renames Editor.Buffers.Registry_Metadata.Clear_Buffer_Label;

   function Has_Buffer_Note
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return Boolean
     renames Editor.Buffers.Registry_Metadata.Has_Buffer_Note;

   function Buffer_Note
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return String
     renames Editor.Buffers.Registry_Metadata.Buffer_Note;

   procedure Set_Buffer_Note
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id;
      Note     : String)
     renames Editor.Buffers.Registry_Metadata.Set_Buffer_Note;

   procedure Clear_Buffer_Note
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id)
     renames Editor.Buffers.Registry_Metadata.Clear_Buffer_Note;

   function Has_Buffer_Group
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return Boolean
     renames Editor.Buffers.Registry_Metadata.Has_Buffer_Group;

   function Buffer_Group
     (Registry : Buffer_Registry;
      Id       : Buffer_Id) return String
     renames Editor.Buffers.Registry_Metadata.Buffer_Group;

   procedure Assign_Buffer_Group
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id;
      Name     : String)
     renames Editor.Buffers.Registry_Metadata.Assign_Buffer_Group;

   procedure Clear_Buffer_Group
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id)
     renames Editor.Buffers.Registry_Metadata.Clear_Buffer_Group;

   function Has_Buffer_Groups
     (Registry : Buffer_Registry) return Boolean
     renames Editor.Buffers.Registry_Metadata.Has_Buffer_Groups;

   function Has_Active_Buffer_Group
     (Registry : Buffer_Registry) return Boolean
     renames Editor.Buffers.Registry_Metadata.Has_Active_Buffer_Group;

   function Active_Buffer_Group
     (Registry : Buffer_Registry) return String
     renames Editor.Buffers.Registry_Metadata.Active_Buffer_Group;

   function First_Buffer_In_Group
     (Registry : Buffer_Registry;
      Name     : String) return Buffer_Id
     renames Editor.Buffers.Registry_Metadata.First_Buffer_In_Group;

   procedure Set_Active_Buffer_Group
     (Registry : in out Buffer_Registry;
      Name     : String)
     renames Editor.Buffers.Registry_Metadata.Set_Active_Buffer_Group;

   procedure Clear_Active_Buffer_Group
     (Registry : in out Buffer_Registry)
     renames Editor.Buffers.Registry_Metadata.Clear_Active_Buffer_Group;

   procedure Cycle_Active_Buffer_Group
     (Registry : in out Buffer_Registry;
      Forward  : Boolean)
     renames Editor.Buffers.Registry_Metadata.Cycle_Active_Buffer_Group;

   procedure Pin_Buffer
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id)
     renames Editor.Buffers.Registry_Metadata.Pin_Buffer;

   procedure Unpin_Buffer
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id)
     renames Editor.Buffers.Registry_Metadata.Unpin_Buffer;

   procedure Toggle_Buffer_Pin
     (Registry : in out Buffer_Registry;
      Id       : Buffer_Id)
     renames Editor.Buffers.Registry_Metadata.Toggle_Buffer_Pin;

   function Unpinned_Clean_Buffer_Count
     (Registry : Buffer_Registry) return Natural
     renames Editor.Buffers.Audits.Unpinned_Clean_Buffer_Count;

   function Dirty_Buffer_Count
     (Registry : Buffer_Registry) return Natural
     renames Editor.Buffers.Audits.Dirty_Buffer_Count;

   function Dirty_File_Backed_Buffer_Count
     (Registry : Buffer_Registry) return Natural
     renames Editor.Buffers.Audits.Dirty_File_Backed_Buffer_Count;

   function Dirty_Untitled_Buffer_Count
     (Registry : Buffer_Registry) return Natural
     renames Editor.Buffers.Audits.Dirty_Untitled_Buffer_Count;

   function Clean_Buffer_Count
     (Registry : Buffer_Registry) return Natural
     renames Editor.Buffers.Audits.Clean_Buffer_Count;

   function Dirty_Buffer_Display_Name
     (Registry : Buffer_Registry;
      Index    : Positive) return String
     renames Editor.Buffers.Audits.Dirty_Buffer_Display_Name;

   function Dirty_Buffer_Summary
     (Registry : Buffer_Registry)
      return Editor.Dirty_Guards.Dirty_Buffer_Summary
     renames Editor.Buffers.Audits.Dirty_Buffer_Summary;

   function Closeable_Unpinned_Clean_Outside_Active_Group_Count
     (Registry : Buffer_Registry) return Natural
     renames Editor.Buffers.Audits.Closeable_Unpinned_Clean_Outside_Active_Group_Count;
