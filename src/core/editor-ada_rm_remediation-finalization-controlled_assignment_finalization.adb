...
package body Editor.Ada_RM_Remediation.Finalization.Controlled_Assignment_Finalization is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Adjust_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Adjust_Profile
         ...
         when Status_Indeterminate_Missing_Finalization_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Controlled_Context'Pos (Row.Context)) * 5
        + Natural (Controlled_Form'Pos (Row.Form)) * 7
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Finalization_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Finalization_Evidence);
   ...
   procedure Check_Controlled_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Adjust_Profile_Mismatch then
         Set_Status (Item, Status_Illegal_Adjust_Profile);
      elsif Row.Finalize_Profile_Mismatch then
         Set_Status (Item, Status_Illegal_Finalize_Profile);
      elsif Row.Writable_Alias_Hazard then
         Set_Status (Item, Status_Illegal_Writable_Alias);
      elsif Row.Assignment_Finalization_Order_Hazard then
         Set_Status (Item, Status_Illegal_Assignment_Finalization_Order);
      elsif Row.Exception_Path_Finalization_Hazard then
         Set_Status (Item, Status_Illegal_Exception_Path_Finalization);
      elsif not Row.Consumer_Surface_Agrees then
         Set_Status (Item, Status_Illegal_Consumer_Surface_Disagreement);
      elsif Row.Runtime_Adjust_Check then
         Set_Status (Item, Status_Runtime_Adjust_Check_Preserved);
      elsif Row.Form = Controlled_Assignment_Adjust_Finalize_Compatible then
         Set_Status (Item, Status_Legal_Controlled_Assignment_Agreement);
      end if;
   end Check_Controlled_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Controlled_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Finalization.Controlled_Assignment_Finalization;
