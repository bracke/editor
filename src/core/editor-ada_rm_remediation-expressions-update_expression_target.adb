...
package body Editor.Ada_RM_Remediation.Expressions.Update_Expression_Target is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Update_Target_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Missing_Target
            | Status_Illegal_Component_Type_Mismatch
            | Status_Illegal_Target_Name_Outside_Update =>
         ...
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Update_Evidence
            ...
            | Status_Update_Fingerprint_Mismatch
            | Status_Target_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Update_Closure'Pos (Row.Closure)) * 5
        + Natural (Update_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Update_Fingerprint
   ...
   begin
      ...
      elsif Row.Update_Fingerprint /= Row.Expected_Update_Fingerprint then
         Set_Status (Item, Status_Update_Fingerprint_Mismatch);
      elsif Row.Target_Fingerprint /= Row.Expected_Target_Fingerprint then
         Set_Status (Item, Status_Target_Fingerprint_Mismatch);
   ...
   procedure Check_Update_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Update_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Update_Evidence);
      elsif Row.Missing_Update_Target then
         Set_Status (Item, Status_Illegal_Missing_Target);
      elsif Row.Component_Type_Mismatch then
         Set_Status (Item, Status_Illegal_Component_Type_Mismatch);
      elsif Row.Target_Name_Outside_Update then
         Set_Status (Item, Status_Illegal_Target_Name_Outside_Update);
      elsif Row.Missing_Full_View
        or else Row.Missing_Update_Evidence
        or else not Row.Complete_Update_Evidence
        or else not Row.Consumer_Update_State_Agrees then
      ...
      elsif Row.Form = Form_Update_Target_Resolved then
         Set_Status (Item, Status_Update_Target_Resolved);
      end if;
   end Check_Update_Rule;
...
end Editor.Ada_RM_Remediation.Expressions.Update_Expression_Target;
