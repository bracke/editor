...
package body Editor.Ada_RM_Remediation.Expressions.Boolean_Predicate_Guards is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Short_Circuit_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Non_Boolean_Operand
            | Status_Illegal_Static_Predicate_Failure
            | Status_Illegal_Side_Effect_In_Static_Context =>
         ...
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Boolean_Evidence
            ...
            | Status_Boolean_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Boolean_Closure'Pos (Row.Closure)) * 5
        + Natural (Boolean_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Boolean_Fingerprint
   ...
   begin
      ...
      elsif Row.Boolean_Fingerprint /= Row.Expected_Boolean_Fingerprint then
         Set_Status (Item, Status_Boolean_Fingerprint_Mismatch);
      elsif Row.Predicate_Fingerprint /= Row.Expected_Predicate_Fingerprint then
         Set_Status (Item, Status_Predicate_Fingerprint_Mismatch);
   ...
   procedure Check_Boolean_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Boolean_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Boolean_Evidence);
      elsif Row.Non_Boolean_Operand then
         Set_Status (Item, Status_Illegal_Non_Boolean_Operand);
      elsif Row.Static_Predicate_Failure then
         Set_Status (Item, Status_Illegal_Static_Predicate_Failure);
      elsif Row.Side_Effect_In_Static_Context then
         Set_Status (Item, Status_Illegal_Side_Effect_In_Static_Context);
      elsif Row.Missing_Full_View
        or else Row.Missing_Boolean_Evidence
        or else not Row.Complete_Boolean_Evidence
        or else not Row.Consumer_Boolean_State_Agrees then
      ...
      elsif Row.Form = Form_Short_Circuit_Resolved then
         Set_Status (Item, Status_Short_Circuit_Resolved);
      end if;
   end Check_Boolean_Rule;
...
end Editor.Ada_RM_Remediation.Expressions.Boolean_Predicate_Guards;
