...
package body Editor.Ada_RM_Remediation.Generics.Formal_Derived_Interface is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Formal_Derived_Interface_Resolved
         ...
         when Status_Illegal_Formal_Derived_Parent_Mismatch
            | Status_Illegal_Interface_Primitive_Missing
            | Status_Illegal_Abstract_Primitive_Unimplemented
            | Status_Illegal_Private_View_Leak
            | Status_Illegal_Dispatching_Profile_Mismatch =>
   ...
   begin
      ...
      elsif not Row.Consumer_Reached or else not Row.Consumer_Interface_State_Agrees then
   ...
   begin
      if Status /= Status_Not_Checked then
        ...
        or else Row.Missing_Interface_Evidence
        or else not Row.Complete_Interface_Evidence
      then
         return Status_Indeterminate_Private_View;
      elsif Row.Formal_Derived_Parent_Mismatch then
         return Status_Illegal_Formal_Derived_Parent_Mismatch;
      elsif Row.Interface_Primitive_Missing then
         return Status_Illegal_Interface_Primitive_Missing;
      elsif Row.Abstract_Primitive_Unimplemented then
         return Status_Illegal_Abstract_Primitive_Unimplemented;
      elsif Row.Private_View_Leak then
         return Status_Illegal_Private_View_Leak;
      elsif Row.Dispatching_Profile_Mismatch then
         return Status_Illegal_Dispatching_Profile_Mismatch;
      ...
      elsif Row.Gap = Remaining_Formal_Derived_Interface_Private_View_Edge then
         return Status_Formal_Derived_Interface_Resolved;
      else
   ...
   begin
      return Row.Id * 43
        + Remediation_Status'Pos (Status) * 17
        + Remediated_Gap_Family'Pos (Row.Gap) * 13
        + Formal_Derived_Interface_Form'Pos (Row.Form) * 7
...
end Editor.Ada_RM_Remediation.Generics.Formal_Derived_Interface;
