...
package Editor.Ada_RM_Remediation.Calls.Expression_Function_Contracts is

   --  Case 1397 remediates a concrete remaining gap extracted by the final
   --  inventory: expression-function return and contract evidence must agree with
   --  result type resolution, Pre/Post contracts, No_Return flow,
   --  runtime checks, control-flow guards, and semantic consumers as
   ...
   type Remediated_Gap_Family is
     (Remaining_Expression_Function_Contract_Return_Edge,
      Remaining_Expression_Function_No_Return_Edge,
      Remaining_Gap_Unknown);

   type Contract_Return_Closure is
     (Closure_Expression_Function,
      Closure_Return_Object,
      Closure_Contract_Result,
      Closure_Pre_Post_Contract,
      Closure_No_Return_Control_Flow,
      Closure_Unknown);

   type Contract_Return_Form is
     (Form_Expression_Function_Return_Resolved,
      Form_Runtime_Check_Preserved,
      Form_Warning_Only_Preserved,
      Form_Illegal_Missing_Return_Expression,
      Form_Illegal_Return_Type_Mismatch,
      Form_Illegal_No_Return_Normal_Return,
      Form_Indeterminate_Private_View,
      Form_Indeterminate_Stale_Contract_Evidence,
      Form_Unknown);
   ...
   type Remediation_Status is
     (Status_Not_Checked,
      ...
      Status_Illegal_Missing_Return_Expression,
   ...
   type Remediation_Row is record
      ...
      Closure : Contract_Return_Closure := Closure_Expression_Function;
      Form : Contract_Return_Form := Form_Expression_Function_Return_Resolved;
      ...
      Missing_Return_Expression : Boolean := False;
      Return_Type_Mismatch : Boolean := False;
      No_Return_Normal_Return_Path : Boolean := False;
      Stale_Contract_Evidence : Boolean := False;
      Missing_Full_View : Boolean := False;
      Missing_Contract_Evidence : Boolean := False;
      Runtime_Check_Preserved : Boolean := False;
      Warning_Only_Preserved : Boolean := False;
      Complete_Contract_Evidence : Boolean := True;
      Consumer_Contract_State_Agrees : Boolean := True;
      ...
      Expected_Contract_Fingerprint : Natural := 0;
      Return_Fingerprint : Natural := 0;
      Expected_Return_Fingerprint : Natural := 0;
