...
with Editor.Executor.File_Save_As_Commands;
...
package body Editor.Executor.File_Save_Basic_Commands is
   ...
   begin
      ...
      declare
         ...
         Conflict_Kind : constant Editor.State_Buffer.File_Conflict_Kind :=
           Conflict_Kind_For_Status (External_Status, S.Buffer_Lifecycle.File_Info.Dirty);
      begin
      ...
      if not S.Buffer_Lifecycle.File_Info.Dirty then
      ...
      if Editor.Files.Is_Success (Result) then
         ...
         Editor.Buffers.Sync_Global_Active_From_State (S);
         if S.Buffer_Lifecycle.File_Info.Has_Path and then Editor.Executor.Visible_Restore_Message_In_History (S) then
            Editor.Executor.Shared_Services.Report_Success_Append (S, "Saved " & To_String (S.Buffer_Lifecycle.File_Info.Display_Name));
         else
         ...
         S.Buffer_Lifecycle.File_Info.Unwritable_Target_Surfaced :=
      ...
      if not Resolve_Active_Buffer_Reload_Target (S) then
         ...
         Editor.Executor.Pending_Transition_Policy.Set_Pending_Dirty_Transition (S,
            Editor.Executor.Pending_Transition_Policy.Pending_Target_For (Editor.Pending_Transitions.Pending_Reload_Active_Buffer,
               Path => To_String (S.Buffer_Lifecycle.File_Info.Path),
               Display => To_String (S.Buffer_Lifecycle.File_Info.Display_Name),
               Buffer_Id => Editor.Buffers.Global_Active_Buffer),
            Editor.Dirty_Guards.Blocked
              ((Dirty_Count       => 1,
                Untitled_Count    => (if S.Buffer_Lifecycle.File_Info.Has_Path then 0 else 1),
                File_Backed_Count => (if S.Buffer_Lifecycle.File_Info.Has_Path then 1 else 0)),
      ...
      begin
         ...
         declare
            External_Status : constant Editor.Files.File_External_Change_Status :=
              Active_File_External_Status (S);
         begin
            if Conflict_Kind_For_Status (External_Status, S.Buffer_Lifecycle.File_Info.Dirty) /=
                  ...
                  then
                     ...
                     begin
                        if Save_Current_File_Backed_Buffer (Status) then
                          ...
                          and then
                            (S.Buffer_Lifecycle.File_Info.External_Change_Surfaced
                             or else S.Buffer_Lifecycle.File_Info.Missing_Target_Surfaced
                             or else S.Buffer_Lifecycle.File_Info.Unreadable_Target_Surfaced)
                        then
   ...
   is
   begin
      Editor.Executor.File_Save_As_Commands.Execute_Save_As (S, Path);
   end Execute_Save_As;
