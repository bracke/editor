...
package body Editor.Ada_RM_Remediation.Expressions.Container_Aggregate_Indexing is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Iterator_Profile_Overlap
   ...
   begin
      return Row.Id
        ...
        + Natural (Container_Closure'Pos (Row.Closure)) * 5
        + Natural (Container_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Container_Fingerprint
        + Row.Indexing_Fingerprint
   ...
   begin
      if not Row.Inventory_Row_From_Final_Burn_Down then
        ...
        or else Length (Row.Candidate_Case) = 0 then
   ...
   begin
      ...
      elsif Row.Container_Fingerprint /= Row.Expected_Container_Fingerprint then
         Set_Status (Item, Status_Container_Fingerprint_Mismatch);
      elsif Row.Indexing_Fingerprint /= Row.Expected_Indexing_Fingerprint then
         Set_Status (Item, Status_Indexing_Fingerprint_Mismatch);
   ...
   procedure Check_Container_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Container_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Container_Evidence);
      elsif Row.Iterator_Profile_Overlap then
         Set_Status (Item, Status_Illegal_Iterator_Profile_Overlap);
      elsif Row.Container_Key_Dependent_Association then
         Set_Status (Item, Status_Illegal_Container_Key_Dependent_Association);
      elsif Row.Controlled_Element_Association then
         Set_Status (Item, Status_Illegal_Controlled_Element_Association);
      elsif Row.Missing_Full_View
        or else Row.Missing_Component_Evidence
        or else not Row.Complete_Container_Evidence
        or else not Row.Consumer_Container_State_Agrees then
         Set_Status (Item, Status_Indeterminate_Private_View);
      elsif Row.Runtime_Check_Preserved
        or else Row.Form = Form_Runtime_Check_Preserved then
         Set_Status (Item, Status_Runtime_Check_Preserved);
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved then
         Set_Status (Item, Status_Warning_Only_Preserved);
      elsif Row.Form = Form_Container_Element_Association_Resolved then
         Set_Status (Item, Status_Container_Element_Association_Resolved);
      end if;
   end Check_Container_Rule;
