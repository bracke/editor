...
package body Editor.Ada_RM_Remediation.Expressions.Universal_Numeric_Expected_Context is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Numeric_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Ambiguous_Universal_Expression
         ...
         when Status_Indeterminate_Private_Numeric_View
   ...
   begin
      ...
      elsif Row.Expected_Context_Fingerprint /= Row.Expected_Expected_Context_Fingerprint then
         return Status_Expected_Context_Fingerprint_Mismatch;
      elsif Row.Numeric_Fingerprint /= Row.Expected_Numeric_Fingerprint then
         return Status_Numeric_Fingerprint_Mismatch;
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Expected_Context_Evidence then
         return Status_Indeterminate_Stale_Expected_Context_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_Numeric_View;
      elsif Row.Missing_Expected_Type_Evidence
        or else not Row.Complete_Expected_Type_Evidence
      then
         return Status_Indeterminate_Missing_Expected_Type_Evidence;
      elsif Row.Missing_Static_Numeric_Evidence
        or else not Row.Complete_Static_Numeric_Evidence
      then
         return Status_Indeterminate_Missing_Static_Numeric_Evidence;
      elsif Row.Ambiguous_Universal_Expression then
         return Status_Illegal_Ambiguous_Universal_Expression;
      elsif Row.Universal_Integer_Real_Mismatch then
         return Status_Illegal_Universal_Integer_Real_Mismatch;
      elsif Row.Modular_Expected_Context_Overflow then
         return Status_Illegal_Modular_Expected_Context_Overflow;
      elsif Row.Stateful_Context_Changes_Resolution then
         return Status_Illegal_Stateful_Context_Changes_Resolution;
      elsif Row.Static_Numeric_Range_Violation then
         return Status_Illegal_Static_Numeric_Range_Violation;
      elsif Row.Runtime_Numeric_Check_Preserved
        or else Row.Form = Form_Runtime_Numeric_Check_Preserved
      then
         return Status_Runtime_Numeric_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Universal_Operand_Context_Resolved then
         return Status_Universal_Operand_Context_Resolved;
      elsif Row.Gap = Remaining_Universal_Numeric_Stateful_Expected_Context_Edge then
         return Status_Universal_Numeric_Expected_Context_Resolved;
      else
   ...
   begin
      return Row.Id * 79
        ...
        + Universal_Numeric_Expected_Context_Form'Pos (Row.Form) * 13
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Profile_Fingerprint
        + Row.Expected_Context_Fingerprint
        + Row.Numeric_Fingerprint
...
end Editor.Ada_RM_Remediation.Expressions.Universal_Numeric_Expected_Context;
