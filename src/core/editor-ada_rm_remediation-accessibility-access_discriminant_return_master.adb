...
package body Editor.Ada_RM_Remediation.Accessibility.Access_Discriminant_Return_Master is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Legal_Access_Discriminant_Return_Object_Agreement =>
         ...
         when Status_Illegal_Access_Discriminant_Escape
            | Status_Illegal_Return_Object_Component_Escape
            | Status_Illegal_Controlled_Finalization_Owner_Mismatch
            | Status_Illegal_Allocator_Finalization_Hazard
            | Status_Illegal_Static_Accessibility_Escape
            | Status_Illegal_Generic_Substitution_Lifetime_Mismatch
            | Status_Illegal_Alias_Consumer_Disagreement
            | Status_Illegal_Consumer_Surface_Disagreement =>
            return Precision.Class_Illegal;
         when Status_Indeterminate_Missing_Master_Evidence
            | Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Lifetime_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Lifetime_Context'Pos (Row.Context)) * 5
        + Natural (Lifetime_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint + Row.AST_Fingerprint
        + Row.Type_Fingerprint + Row.Accessibility_Fingerprint
        + Row.Lifetime_Fingerprint + Row.Consumer_Fingerprint;
   end Row_Fingerprint;
   ...
   begin
      if not Row.Inventory_Row_From_Final_Burn_Down then
         Set_Status (Item, Status_Missing_Final_Inventory_Row);
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Master_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Master_Evidence);
      elsif not Row.Private_Full_View_Available then
         Set_Status (Item, Status_Indeterminate_Private_View);
      elsif Row.Stale_Lifetime_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Lifetime_Evidence);
      end if;
   ...
   begin
      ...
      elsif Row.Accessibility_Fingerprint /= Row.Expected_Accessibility_Fingerprint then
         Set_Status (Item, Status_Accessibility_Fingerprint_Mismatch);
      elsif Row.Lifetime_Fingerprint /= Row.Expected_Lifetime_Fingerprint then
         Set_Status (Item, Status_Lifetime_Fingerprint_Mismatch);
   ...
   procedure Check_Access_Lifetime_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Access_Discriminant_Escape then
         Set_Status (Item, Status_Illegal_Access_Discriminant_Escape);
      elsif Row.Return_Object_Component_Escape then
         Set_Status (Item, Status_Illegal_Return_Object_Component_Escape);
      elsif Row.Controlled_Finalization_Owner_Mismatch then
         Set_Status (Item, Status_Illegal_Controlled_Finalization_Owner_Mismatch);
      elsif Row.Allocator_Finalization_Hazard then
         Set_Status (Item, Status_Illegal_Allocator_Finalization_Hazard);
      elsif Row.Static_Accessibility_Escape then
         Set_Status (Item, Status_Illegal_Static_Accessibility_Escape);
      elsif Row.Generic_Substitution_Lifetime_Mismatch then
         Set_Status (Item, Status_Illegal_Generic_Substitution_Lifetime_Mismatch);
      elsif Row.Alias_Consumer_Disagreement then
         Set_Status (Item, Status_Illegal_Alias_Consumer_Disagreement);
         ...
         Set_Status (Item, Status_Runtime_Accessibility_Check_Preserved);
      elsif Row.Form = Lifetime_Compatible then
         Set_Status (Item, Status_Legal_Access_Discriminant_Return_Object_Agreement);
      end if;
   end Check_Access_Lifetime_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Access_Lifetime_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Accessibility.Access_Discriminant_Return_Master;
