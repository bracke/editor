...
package body Editor.Ada_RM_Remediation.Finalization.Controlled_Discriminant_Finalization is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Controlled_Discriminant_Component_Resolved
            | Status_Finalization_Order_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Runtime_Finalization_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Controlled_Component_Depends_On_Mutable_Discriminant
            | Status_Illegal_Discriminant_Default_Finalized_Object
            | Status_Illegal_Limited_Controlled_Discriminant_Copy
            | Status_Illegal_Premature_Finalization_During_Assignment
            | Status_Illegal_Component_Finalization_Order_Conflict =>
            return Precision.Class_Illegal;
         when Status_Indeterminate_Private_Controlled_View
            | Status_Indeterminate_Missing_Discriminant_Evidence
            ...
            | Status_Discriminant_Fingerprint_Mismatch
   ...
   begin
      ...
      elsif Row.Discriminant_Fingerprint /= Row.Expected_Discriminant_Fingerprint then
         return Status_Discriminant_Fingerprint_Mismatch;
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Finalization_Evidence then
         return Status_Indeterminate_Stale_Finalization_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_Controlled_View;
      elsif Row.Missing_Discriminant_Evidence
        or else not Row.Complete_Discriminant_Evidence
      then
         return Status_Indeterminate_Missing_Discriminant_Evidence;
      ...
      elsif Row.Controlled_Component_Depends_On_Mutable_Discriminant then
         return Status_Illegal_Controlled_Component_Depends_On_Mutable_Discriminant;
      elsif Row.Discriminant_Default_Finalized_Object then
         return Status_Illegal_Discriminant_Default_Finalized_Object;
      elsif Row.Limited_Controlled_Discriminant_Copy then
         return Status_Illegal_Limited_Controlled_Discriminant_Copy;
      elsif Row.Premature_Finalization_During_Assignment then
         return Status_Illegal_Premature_Finalization_During_Assignment;
      elsif Row.Component_Finalization_Order_Conflict then
         return Status_Illegal_Component_Finalization_Order_Conflict;
      elsif Row.Runtime_Finalization_Check_Preserved
        or else Row.Form = Form_Runtime_Finalization_Check_Preserved
      then
         return Status_Runtime_Finalization_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Finalization_Order_Resolved then
         return Status_Finalization_Order_Resolved;
      elsif Row.Gap = Remaining_Controlled_Finalized_Discriminant_Component_Edge then
         return Status_Controlled_Discriminant_Component_Resolved;
      else
   ...
   begin
      return Row.Id * 79
        ...
        + Controlled_Finalized_Discriminant_Form'Pos (Row.Form) * 13
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Profile_Fingerprint
        + Row.Discriminant_Fingerprint
...
end Editor.Ada_RM_Remediation.Finalization.Controlled_Discriminant_Finalization;
