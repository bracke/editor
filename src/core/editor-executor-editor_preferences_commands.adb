...
package body Editor.Executor.Editor_Preferences_Commands is
   ...
   begin
      case Kind is
         when Toggle_Theme =>
            Run (Command_Toggle_Theme);

         when Toggle_Minimap =>
            Run (Command_Toggle_Minimap);

         when Toggle_Scrollbars =>
            Run (Command_Toggle_Scrollbars);

         when Toggle_Format_On_Save =>
            Run (Command_Toggle_Format_On_Save);

         when Toggle_Line_Number_Mode =>
            Run (Command_Toggle_Line_Number_Mode);

         when Toggle_Cursor_Blink =>
            Run (Command_Toggle_Cursor_Blink);

         when Set_Theme_Light =>
            Run (Command_Set_Theme_Light);

         when Set_Theme_Dark =>
            Run (Command_Set_Theme_Dark);

         when others =>
            raise Program_Error with "unsupported editor preferences command kind";
      end case;
   end Execute_Editor_Preferences_Kind;
