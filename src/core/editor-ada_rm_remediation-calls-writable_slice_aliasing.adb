...
package body Editor.Ada_RM_Remediation.Calls.Writable_Slice_Aliasing is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Alias_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Writable_Alias_Overlap
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         return Status_Alias_Fingerprint_Mismatch;
   ...
   begin
      ...
      elsif not Row.Consumer_Reached or else not Row.Consumer_Alias_Call_State_Agrees then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Alias_Call_Evidence then
         return Status_Indeterminate_Stale_Alias_Call_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_View;
      elsif Row.Missing_Alias_Evidence or else not Row.Complete_Alias_Call_Evidence then
         return Status_Indeterminate_Missing_Alias_Evidence;
      elsif Row.Writable_Alias_Overlap then
         return Status_Illegal_Writable_Alias_Overlap;
      elsif Row.Constant_View_Writable_Actual then
         return Status_Illegal_Constant_View_Writable_Actual;
      elsif Row.Mode_Mismatch then
         return Status_Illegal_Mode_Mismatch;
      elsif Row.Static_Slice_Out_Of_Range then
         return Status_Illegal_Static_Slice_Out_Of_Range;
      elsif Row.Volatile_Atomic_Order_Conflict then
         return Status_Illegal_Volatile_Atomic_Order_Conflict;
      elsif Row.Runtime_Alias_Check_Preserved
        or else Row.Form = Form_Runtime_Alias_Check_Preserved
      then
         return Status_Runtime_Alias_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Gap = Remaining_Array_Slice_Writable_Alias_Call_Edge then
         return Status_Writable_Alias_Call_Resolved;
      else
   ...
   begin
      return Row.Id * 47
        ...
        + Writable_Alias_Call_Form'Pos (Row.Form) * 7
...
end Editor.Ada_RM_Remediation.Calls.Writable_Slice_Aliasing;
