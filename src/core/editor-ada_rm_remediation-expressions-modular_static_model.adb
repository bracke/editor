...
package body Editor.Ada_RM_Remediation.Expressions.Modular_Static_Model is
   ...
   begin
      case Status is
         ...
         when Status_Illegal_Non_Static_Modulus
   ...
   begin
      return Row.Id
        ...
        + Natural (Modular_Closure'Pos (Row.Closure)) * 5
        + Natural (Modular_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Modulus_Fingerprint
   ...
   begin
      ...
      elsif Row.Modulus_Fingerprint /= Row.Expected_Modulus_Fingerprint then
         Set_Status (Item, Status_Modulus_Fingerprint_Mismatch);
   ...
   procedure Check_Modular_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Modular_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Modular_Evidence);
      elsif Row.Non_Static_Modulus then
         Set_Status (Item, Status_Illegal_Non_Static_Modulus);
      elsif Row.Modulus_Size_Conflict then
         Set_Status (Item, Status_Illegal_Modulus_Size_Conflict);
      elsif Row.Static_Overflow then
         Set_Status (Item, Status_Illegal_Static_Overflow);
      elsif Row.Missing_Full_View
        or else Row.Missing_Modulus_Evidence
        or else not Row.Complete_Modular_Evidence
        or else not Row.Consumer_Modular_State_Agrees then
      ...
      elsif Row.Form = Form_Modular_Operator_Resolved then
         Set_Status (Item, Status_Modular_Operator_Resolved);
      end if;
   end Check_Modular_Rule;
...
end Editor.Ada_RM_Remediation.Expressions.Modular_Static_Model;
