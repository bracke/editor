...
package body Editor.Build_Command is
   ...
   begin
      ...
      then
         ...
         --  is the Windows CI hang). Native_Process_Control_Is_POSIX now asks
         --  Hostkit.Host.Current, so it correctly reports non-POSIX on Windows
         --  and this skips there. release_check still guards that this
         --  integration assertion exists.
         return True;
      end if;
