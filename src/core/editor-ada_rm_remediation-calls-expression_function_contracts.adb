...
package body Editor.Ada_RM_Remediation.Calls.Expression_Function_Contracts is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Expression_Function_Return_Resolved
            | Status_Warning_Only_Preserved =>
            return Precision.Class_Legal;
         when Status_Illegal_Missing_Return_Expression
            | Status_Illegal_Return_Type_Mismatch
            | Status_Illegal_No_Return_Normal_Return =>
         ...
         when Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Contract_Evidence
            ...
            | Status_Contract_Fingerprint_Mismatch
            | Status_Return_Fingerprint_Mismatch
   ...
   begin
      return Row.Id
        ...
        + Natural (Contract_Return_Closure'Pos (Row.Closure)) * 5
        + Natural (Contract_Return_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Contract_Fingerprint
        + Row.Return_Fingerprint
   ...
   begin
      ...
      elsif Row.Contract_Fingerprint /= Row.Expected_Contract_Fingerprint then
         Set_Status (Item, Status_Contract_Fingerprint_Mismatch);
      elsif Row.Return_Fingerprint /= Row.Expected_Return_Fingerprint then
         Set_Status (Item, Status_Return_Fingerprint_Mismatch);
   ...
   procedure Check_Contract_Return_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Stale_Contract_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Contract_Evidence);
      elsif Row.Missing_Return_Expression then
         Set_Status (Item, Status_Illegal_Missing_Return_Expression);
      elsif Row.Return_Type_Mismatch then
         Set_Status (Item, Status_Illegal_Return_Type_Mismatch);
      elsif Row.No_Return_Normal_Return_Path then
         Set_Status (Item, Status_Illegal_No_Return_Normal_Return);
      elsif Row.Missing_Full_View
        or else Row.Missing_Contract_Evidence
        or else not Row.Complete_Contract_Evidence
        or else not Row.Consumer_Contract_State_Agrees then
      ...
      elsif Row.Form = Form_Expression_Function_Return_Resolved then
         Set_Status (Item, Status_Expression_Function_Return_Resolved);
      end if;
   end Check_Contract_Return_Rule;
   ...
   begin
      ...
      for Row of Input.Rows loop
         ...
         if Item.Status = Status_Not_Checked then
            Check_Contract_Return_Rule (Row, Item);
         end if;
...
end Editor.Ada_RM_Remediation.Calls.Expression_Function_Contracts;
