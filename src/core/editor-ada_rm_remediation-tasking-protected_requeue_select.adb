...
package body Editor.Ada_RM_Remediation.Tasking.Protected_Requeue_Select is
   ...
   begin
      case Status is
         when Status_Gap_Remediated
            | Status_Legal_Tasking_Agreement =>
            return Precision.Class_Legal;
         when Status_Runtime_Entry_Queue_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Protected_Barrier_Side_Effect
            | Status_Illegal_Requeue_Target_Profile
            | Status_Illegal_Select_Coverage
            | Status_Illegal_Terminate_Dependency
            | Status_Illegal_Abort_Finalization_Hazard
            | Status_Illegal_Consumer_Surface_Disagreement =>
            return Precision.Class_Illegal;
         when Status_Indeterminate_Missing_Effect_Evidence
            | Status_Indeterminate_Private_View
            | Status_Indeterminate_Stale_Effect_Evidence
   ...
   begin
      return Row.Id
        ...
        + Natural (Tasking_Context'Pos (Row.Context)) * 5
        + Natural (Tasking_Form'Pos (Row.Form)) * 7
        + Row.Source_Fingerprint + Row.AST_Fingerprint
        + Row.Type_Fingerprint + Row.Profile_Fingerprint
        + Row.Effect_Fingerprint + Row.Consumer_Fingerprint;
   end Row_Fingerprint;
   ...
   procedure Check_Indeterminate_Evidence
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if not Row.Effect_Evidence_Present then
         Set_Status (Item, Status_Indeterminate_Missing_Effect_Evidence);
      elsif not Row.Private_Full_View_Available then
         Set_Status (Item, Status_Indeterminate_Private_View);
      elsif Row.Stale_Effect_Evidence then
         Set_Status (Item, Status_Indeterminate_Stale_Effect_Evidence);
      end if;
   ...
   begin
      if Row.Source_Fingerprint /= Row.Expected_Source_Fingerprint then
         ...
         Set_Status (Item, Status_Effect_Fingerprint_Mismatch);
   ...
   procedure Check_Tasking_Rule
     (Row : Remediation_Row;
      Item : in out Remediation_Entry) is
   begin
      if Row.Protected_Barrier_Side_Effect then
         Set_Status (Item, Status_Illegal_Protected_Barrier_Side_Effect);
      elsif Row.Requeue_Target_Profile_Mismatch then
         Set_Status (Item, Status_Illegal_Requeue_Target_Profile);
      elsif Row.Select_Coverage_Missing then
         Set_Status (Item, Status_Illegal_Select_Coverage);
      elsif Row.Terminate_Dependency_Unsafe then
         Set_Status (Item, Status_Illegal_Terminate_Dependency);
      elsif Row.Abort_Finalization_Hazard then
         Set_Status (Item, Status_Illegal_Abort_Finalization_Hazard);
      elsif not Row.Consumer_Surface_Agrees then
         Set_Status (Item, Status_Illegal_Consumer_Surface_Disagreement);
      elsif Row.Runtime_Entry_Queue_Check then
         Set_Status (Item, Status_Runtime_Entry_Queue_Check_Preserved);
      elsif Row.Form = Tasking_Protected_Requeue_Select_Compatible then
         Set_Status (Item, Status_Legal_Tasking_Agreement);
      end if;
   end Check_Tasking_Rule;
...
end Editor.Ada_RM_Remediation.Tasking.Protected_Requeue_Select;
