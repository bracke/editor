...
package body Editor.Ada_RM_Remediation.Tasking.Entry_Family_Barrier is
   ...
   begin
      case Status is
         ...
         when Status_Runtime_Barrier_Check_Preserved =>
            return Precision.Class_Legal_With_Runtime_Check;
         when Status_Illegal_Non_Discrete_Entry_Family_Index
   ...
   begin
      ...
      elsif Row.Entry_Family_Fingerprint /= Row.Expected_Entry_Family_Fingerprint then
         return Status_Entry_Family_Fingerprint_Mismatch;
      elsif Row.Barrier_Fingerprint /= Row.Expected_Barrier_Fingerprint then
         return Status_Barrier_Fingerprint_Mismatch;
   ...
   begin
      ...
      then
        ...
        or else not Row.Consumer_Entry_State_Agrees
      then
   ...
   begin
      if Status /= Status_Not_Checked then
         return Status;
      elsif Row.Stale_Entry_Queue_Evidence then
         return Status_Indeterminate_Stale_Entry_Queue_Evidence;
      elsif Row.Missing_Full_View then
         return Status_Indeterminate_Private_Protected_View;
      elsif Row.Missing_Entry_Family_Evidence
        or else not Row.Complete_Entry_Family_Evidence
      then
         return Status_Indeterminate_Missing_Entry_Family_Evidence;
      elsif Row.Missing_Barrier_Evidence
        or else not Row.Complete_Barrier_Evidence
      then
         return Status_Indeterminate_Missing_Barrier_Evidence;
      elsif Row.Non_Discrete_Entry_Family_Index then
         return Status_Illegal_Non_Discrete_Entry_Family_Index;
      elsif Row.Barrier_Not_Boolean then
         return Status_Illegal_Barrier_Not_Boolean;
      elsif Row.Index_Constraint_Mismatch then
         return Status_Illegal_Index_Constraint_Mismatch;
      elsif Row.Requeue_Target_Index_Mismatch then
         return Status_Illegal_Requeue_Target_Index_Mismatch;
      elsif Row.Queue_Policy_Conflict then
         return Status_Illegal_Queue_Policy_Conflict;
      elsif Row.Runtime_Barrier_Check_Preserved
        or else Row.Form = Form_Runtime_Barrier_Check_Preserved
      then
         return Status_Runtime_Barrier_Check_Preserved;
      elsif Row.Warning_Only_Preserved
        or else Row.Form = Form_Warning_Only_Preserved
      then
         return Status_Warning_Only_Preserved;
      elsif Row.Form = Form_Entry_Queue_Requeue_Target_Resolved then
         return Status_Entry_Queue_Requeue_Target_Resolved;
      elsif Row.Gap = Remaining_Entry_Family_Barrier_Index_Edge then
         return Status_Entry_Family_Barrier_Index_Resolved;
      else
   ...
   begin
      return Row.Id * 79
        ...
        + Entry_Family_Barrier_Form'Pos (Row.Form) * 13
        + Row.Source_Fingerprint
        + Row.AST_Fingerprint
        + Row.Type_Fingerprint
        + Row.Profile_Fingerprint
        + Row.Entry_Family_Fingerprint
        + Row.Barrier_Fingerprint
