...
procedure Product_Smoke_Build_Diagnostics is
   Tool : constant String := "product_smoke_build_diagnostics";
...
begin
   ...
   Status := Run1 ("tools/bin/product_smoke", "build_diagnostics");
   Unit_Test_Build_Lock.Release;
   if Status /= 0 then
      Fail (Tool, "product smoke failed");
   end if;
   Require_File (Tool, Report);
   if not File_Contains (Report, "build_diagnostics_navigation=confirmed") then
      Fail (Tool, "build/diagnostics marker missing");
   end if;
   ...
   then
      Fail (Tool, "build/diagnostics smoke must not rely on combined scenario markers");
   end if;
   Info (Tool, "build/diagnostics smoke passed");
exception
   when others =>
      Unit_Test_Build_Lock.Release;
      raise;
end Product_Smoke_Build_Diagnostics;
