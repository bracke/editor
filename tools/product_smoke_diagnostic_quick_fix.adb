...
procedure Product_Smoke_Diagnostic_Quick_Fix is
   Tool : constant String := "product_smoke_diagnostic_quick_fix";
...
begin
   ...
   Status := Run1 ("tools/bin/product_smoke", "diagnostic_quick_fix");
   Unit_Test_Build_Lock.Release;
   if Status /= 0 then
      Fail (Tool, "product smoke failed");
   end if;
   Require_File (Tool, Report);
   if not File_Contains (Report, "diagnostic_quick_fix=confirmed") then
      Fail (Tool, "diagnostic quick-fix marker missing");
   end if;
   if File_Contains (Report, "workspace_save_restore_clear=confirmed")
     ...
     or else File_Contains (Report, "build_ui_interaction=confirmed")
     or else File_Contains (Report, "diagnostics_problems_filters=confirmed")
   ...
   then
      Fail (Tool, "diagnostic quick-fix smoke must not rely on combined scenario markers");
   end if;
   Info (Tool, "diagnostic quick-fix smoke passed");
exception
   when others =>
      Unit_Test_Build_Lock.Release;
      raise;
end Product_Smoke_Diagnostic_Quick_Fix;
