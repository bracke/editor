...
procedure Product_Smoke_Render_Packet is
   Tool : constant String := "product_smoke_render_packet";
...
begin
   ...
   Status := Run1 ("tools/bin/product_smoke", "render_packet");
   Unit_Test_Build_Lock.Release;
   if Status /= 0 then
      Fail (Tool, "product smoke failed");
   end if;
   Require_File (Tool, Report);
   if not File_Contains (Report, "render_packet_nonempty=confirmed") then
      Fail (Tool, "render-packet marker missing");
   end if;
   ...
   then
      Fail (Tool, "render-packet smoke must not rely on combined scenario markers");
   end if;
   Info (Tool, "render-packet smoke passed");
exception
   when others =>
      Unit_Test_Build_Lock.Release;
      raise;
end Product_Smoke_Render_Packet;
