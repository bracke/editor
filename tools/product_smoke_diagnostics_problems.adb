...
procedure Product_Smoke_Diagnostics_Problems is
   Tool : constant String := "product_smoke_diagnostics_problems";
...
begin
   ...
   Status := Run1 ("tools/bin/product_smoke", "diagnostics_problems");
   Unit_Test_Build_Lock.Release;
   if Status /= 0 then
      Fail (Tool, "product smoke failed");
   end if;
   Require_File (Tool, Report);
   if not File_Contains (Report, "diagnostics_problems_filters=confirmed") then
      Fail (Tool, "diagnostics/problems marker missing");
   end if;
   ...
   then
      Fail (Tool, "diagnostics/problems smoke must not rely on combined scenario markers");
   end if;
   Info (Tool, "diagnostics/problems smoke passed");
exception
   when others =>
      Unit_Test_Build_Lock.Release;
      raise;
end Product_Smoke_Diagnostics_Problems;
