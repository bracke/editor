...
procedure Product_Smoke_Edit_Save is
   Tool : constant String := "product_smoke_edit_save";
...
begin
   ...
   Status := Run1 ("tools/bin/product_smoke", "edit_save");
   Unit_Test_Build_Lock.Release;
   if Status /= 0 then
      Fail (Tool, "product smoke failed");
   end if;
   Require_File (Tool, Report);
   if not File_Contains (Report, "editing_save_conflict_free=confirmed") then
      Fail (Tool, "edit/save marker missing");
   end if;
   ...
   then
      Fail (Tool, "edit/save smoke must not rely on combined scenario markers");
   end if;
   Info (Tool, "edit/save smoke passed");
exception
   when others =>
      Unit_Test_Build_Lock.Release;
      raise;
end Product_Smoke_Edit_Save;
