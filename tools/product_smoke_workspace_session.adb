...
procedure Product_Smoke_Workspace_Session is
   Tool : constant String := "product_smoke_workspace_session";
...
begin
   ...
   Status := Run1 ("tools/bin/product_smoke", "workspace_session");
   Unit_Test_Build_Lock.Release;
   if Status /= 0 then
      Fail (Tool, "product smoke failed");
   end if;
   Require_File (Tool, Report);
   if not File_Contains (Report, "workspace_save_restore_clear=confirmed")
     or else not File_Contains (Report, "workspace_persistence_roundtrip=confirmed")
   then
      Fail (Tool, "workspace/session marker missing");
   end if;
   if File_Contains (Report, "quick_open_file_tree_navigation=confirmed")
   ...
   then
      Fail (Tool, "workspace/session smoke must not rely on combined scenario markers");
   end if;
   Info (Tool, "workspace/session smoke passed");
exception
   when others =>
      Unit_Test_Build_Lock.Release;
      raise;
end Product_Smoke_Workspace_Session;
