...
procedure Product_Smoke_Quick_Open_File_Tree is
   Tool : constant String := "product_smoke_quick_open_file_tree";
...
begin
   ...
   Status := Run1 ("tools/bin/product_smoke", "quick_open_file_tree");
   Unit_Test_Build_Lock.Release;
   if Status /= 0 then
      Fail (Tool, "product smoke failed");
   end if;
   Require_File (Tool, Report);
   if not File_Contains (Report, "quick_open_file_tree_navigation=confirmed") then
      Fail (Tool, "quick-open/file-tree marker missing");
   end if;
   ...
   then
      Fail (Tool, "quick-open/file-tree smoke must not rely on combined scenario markers");
   end if;
   Info (Tool, "quick-open/file-tree smoke passed");
exception
   when others =>
      Unit_Test_Build_Lock.Release;
      raise;
end Product_Smoke_Quick_Open_File_Tree;
