...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1404;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1404 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1404;
   ...
   use type Audit.Access_Subprogram_Convention_Default_Closure;
   use type Audit.Access_Subprogram_Convention_Default_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1404");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Access_Subprogram_Convention_Default_Form := Audit.Form_Access_Subprogram_Convention_Default_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Access_Subprogram_Convention_Default_Edge;
      ...
      Row.Closure := Audit.Closure_Access_Subprogram_Convention_Default;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/access-subprogram-convention-default.ads");
      ...
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1404");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1404");
      Row.Blocker_Family :=
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1404 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1404 precision classification");
   end Expect_Status;

   procedure Test_Access_Subprogram_Convention_Default_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Access_Subprogram_Convention_Default_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Convention_Mismatch);
      ...
      Assert (Audit.Gap_Remediated (Results),
              "pass1404 should close the access-subprogram/convention/default gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Access_Subprogram_Convention_Default_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Convention_Mismatch,
   ...
   end Test_Access_Subprogram_Convention_Default_Gap_Remediated;

   procedure Test_Access_Subprogram_Convention_Default_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Default_Null_Exclusion := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (11);
      Row.Default_Profile_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Protected_Access_Profile_Mismatch := True;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 10, Audit.Status_Illegal_Default_Null_Exclusion,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Default_Profile_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Protected_Access_Profile_Mismatch,
                     Precision.Class_Illegal);
      ...
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 15, Audit.Status_Indeterminate_Stale_Access_Subprogram_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Access_Subprogram_Convention_Default_Rejections;
   ...
   begin
      ...
      Expect_Status (Results, 25, Audit.Status_Semantic_Result_Unconsumed,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Convention_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Access_Subprogram_Convention_Default_Gap_Remediated'Access,
         "access-subprogram convention/default gap remediated");
      Register_Routine
        (T, Test_Access_Subprogram_Convention_Default_Rejections'Access,
         "missing default null-exclusion, default profile, protected access profile mismatch, static accessibility mismatch, missing evidence, and stale evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1404;
