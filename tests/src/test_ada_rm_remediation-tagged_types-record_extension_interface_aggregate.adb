...
with Editor.Ada_RM_Remediation.Tagged_Types.Record_Extension_Interface_Aggregate;

package body Test_Ada_RM_Remediation.Tagged_Types.Record_Extension_Interface_Aggregate is
   package Audit renames Editor.Ada_RM_Remediation.Tagged_Types.Record_Extension_Interface_Aggregate;
   ...
   use type Audit.Record_Extension_Interface_Closure;
   use type Audit.Record_Extension_Interface_Form;
   ...
   function Base_Row
     (Id : Natural;
      ...
      Form : Audit.Record_Extension_Interface_Form :=
        Audit.Form_Record_Extension_Aggregate_Interface_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Record_Extension_Aggregate_Interface_Edge;
      ...
      Row.Closure := Audit.Closure_Record_Extension_Aggregate;
      Row.Form := Form;
      Row.Source_File :=
        To_Unbounded_String ("src/record-extension-aggregate-interface.adb");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("record extension aggregates, ancestor parts, extension component "
           & "associations, interface progenitor/full-view evidence, abstract "
           & "primitive availability, runtime tag checks, warnings, and "
           & "consumers must preserve one canonical aggregate/interface result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remediation.Tagged_Types.Record_Extension_Interface_Aggregate");
      Row.Candidate_Case := To_Unbounded_String ("Case 1414");
      Row.Blocker_Family :=
        To_Unbounded_String ("RM.Aggregate.Record_Extension.Interface");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected case 1414 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected case 1414 precision classification");
   end Expect_Status;
   ...
   begin
      ...
      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Ancestor_Type_Incompatible);
      Row.Ancestor_Type_Incompatible := True;
      ...
      Row := Base_Row (5, Precision.Class_Indeterminate,
                       Audit.Form_Indeterminate_Missing_Interface_Evidence);
      ...
      Assert (Audit.Gap_Remediated (Results),
              "case 1414 should close the record extension aggregate interface gap");
      ...
      Expect_Status
        (Results, 1,
         Audit.Status_Record_Extension_Aggregate_Interface_Resolved,
         Precision.Class_Legal);
      Expect_Status
        (Results, 2,
         Audit.Status_Illegal_Ancestor_Type_Incompatible,
      ...
      Expect_Status
        (Results, 5,
         Audit.Status_Indeterminate_Missing_Interface_Evidence,
   ...
   begin
      Row := Base_Row (10);
      Row.Nonextension_Target := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (11);
      Row.Missing_Ancestor_Part := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Interface_Progenitor_Incomplete := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Abstract_Interface_Primitive_Unimplemented := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Extension_Component_Association_Mismatch := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (17);
      Row.Missing_Aggregate_Evidence := True;
      ...
      Expect_Status
        (Results, 10,
         Audit.Status_Illegal_Nonextension_Target,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 11,
         Audit.Status_Illegal_Missing_Ancestor_Part,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 12,
         Audit.Status_Illegal_Interface_Progenitor_Incomplete,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 13,
         Audit.Status_Illegal_Abstract_Interface_Primitive_Unimplemented,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 14,
         Audit.Status_Illegal_Extension_Component_Association_Mismatch,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 15,
      ...
      Expect_Status
        (Results, 16,
         Audit.Status_Indeterminate_Stale_Interface_Evidence,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 17,
         Audit.Status_Indeterminate_Missing_Aggregate_Evidence,
         Precision.Class_Indeterminate);
   end Test_Rejections;
   ...
   begin
      ...
      Row := Base_Row (25);
      Row.Consumer_Interface_State_Agrees := False;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 26,
         Audit.Status_Aggregate_Fingerprint_Mismatch,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 27,
         Audit.Status_Interface_Fingerprint_Mismatch,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 28,
   ...
   begin
      Register_Routine
        (T, Test_Gap_Remediated'Access,
         "record extension aggregate interface gap remediated");
      Register_Routine
        (T, Test_Rejections'Access,
         "record extension aggregate interface rejection cases");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remediation.Tagged_Types.Record_Extension_Interface_Aggregate;
