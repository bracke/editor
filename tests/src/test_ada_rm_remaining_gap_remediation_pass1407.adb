...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1407;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1407 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1407;
   ...
   use type Audit.Writable_Alias_Call_Closure;
   use type Audit.Writable_Alias_Call_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1407");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Writable_Alias_Call_Form := Audit.Form_Writable_Alias_Call_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Array_Slice_Writable_Alias_Call_Edge;
      ...
      Row.Closure := Audit.Closure_Call_Actual_Association;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/array-slice-writable-alias-call.adb");
      ...
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1407");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1407");
      Row.Blocker_Family :=
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1407 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1407 precision classification");
   end Expect_Status;

   procedure Test_Writable_Alias_Call_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Writable_Alias_Call_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Writable_Alias_Overlap);
      Row.Writable_Alias_Overlap := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Alias_Check_Preserved);
      Row.Runtime_Alias_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (5, Precision.Class_Indeterminate,
                       Audit.Form_Indeterminate_Missing_Alias_Evidence);
      Row.Missing_Alias_Evidence := True;
      Row.Complete_Alias_Call_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1407 should close the array slice writable alias call gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Writable_Alias_Call_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Writable_Alias_Overlap,
                     Precision.Class_Illegal);
      Expect_Status (Results, 3, Audit.Status_Runtime_Alias_Check_Preserved,
                     Precision.Class_Legal_With_Runtime_Check);
      Expect_Status (Results, 4, Audit.Status_Warning_Only_Preserved,
                     Precision.Class_Legal);
      Expect_Status (Results, 5, Audit.Status_Indeterminate_Missing_Alias_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Writable_Alias_Call_Gap_Remediated;

   procedure Test_Writable_Alias_Call_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Constant_View_Writable_Actual := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (11);
      Row.Mode_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Static_Slice_Out_Of_Range := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Volatile_Atomic_Order_Conflict := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (15);
      Row.Stale_Alias_Call_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Constant_View_Writable_Actual,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Mode_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Static_Slice_Out_Of_Range,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Illegal_Volatile_Atomic_Order_Conflict,
                     Precision.Class_Illegal);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 15, Audit.Status_Indeterminate_Stale_Alias_Call_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Writable_Alias_Call_Rejections;
   ...
   begin
      ...
      Row := Base_Row (25);
      Row.Consumer_Alias_Call_State_Agrees := False;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (27);
      Row.Alias_Fingerprint := 1;
      Row.Expected_Alias_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 25, Audit.Status_Consumer_Not_Reached,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Profile_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Alias_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Writable_Alias_Call_Gap_Remediated'Access,
         "array slice writable alias call gap remediated");
      Register_Routine
        (T, Test_Writable_Alias_Call_Rejections'Access,
         "writable alias, constant view, mode, static slice, volatile/atomic, private view, and stale alias evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1407;
