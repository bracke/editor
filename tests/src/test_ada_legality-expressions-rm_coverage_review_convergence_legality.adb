...
package body Test_Ada_Legality.Expressions.RM_Coverage_Review_Convergence_Legality is
   ...
   begin
      return AUnit.Format ("Ada remaining RM edge recheck convergence legality");
   end Name;
   ...
   procedure Application_Rows_Are_Classified_For_Convergence
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Apps : constant AP.Remaining_RM_Edge_Application_Model := Build_Applications;
      Convergence : constant CV.Remaining_RM_Edge_Convergence_Model := CV.Build (Apps, 0);
   begin
      Assert (CV.Row_Count (Convergence) = AP.Row_Count (Apps),
              "convergence should preserve remaining-edge application row count");
      Assert (CV.Converged_Count (Convergence) = AP.Current_Count (Apps),
              "current remaining-edge application evidence should converge");
      Assert (CV.Stable_Withheld_Count (Convergence) = AP.Withheld_Count (Apps),
              "withheld application rows should become stable withheld convergence rows");
      Assert (CV.Stable_Fingerprint (Convergence) /= 0,
              "convergence fingerprint should be deterministic");
   end Application_Rows_Are_Classified_For_Convergence;
   ...
   begin
      Assert (CV.Count_Status (Convergence, CV.Remaining_RM_Edge_Converged_Not_Required) = 1,
              "accepted non-diagnostic remaining-edge evidence should converge as not-required");
      Assert (CV.Count_Status (Convergence, CV.Remaining_RM_Edge_Stable_Withheld_Remaining_Edge) = 1,
              "remaining-edge blockers should remain a distinct stable withheld family");
      Assert (CV.Count_Status (Convergence, CV.Remaining_RM_Edge_Stable_Withheld_Stabilized_Closure) = 2,
              "stabilized closure blockers should remain distinct stable withheld rows");
      Assert (CV.Count_Action (Convergence, CV.Remaining_RM_Edge_Convergence_Action_Retain_Remaining_Edge_Blocker) = 1,
              "remaining-edge convergence action should stay explicit");
      Assert (CV.Count_Action (Convergence, CV.Remaining_RM_Edge_Convergence_Action_Retain_Stabilized_Closure_Blocker) = 2,
              "stabilized-closure convergence action should stay explicit");
   end Preserves_Remaining_Edge_And_Closure_Blockers;

   procedure Detects_Changed_Application_Fingerprint
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Apps : constant AP.Remaining_RM_Edge_Application_Model := Build_Applications;
      Prior_Fingerprint : constant Natural := AP.Stable_Fingerprint (Apps) + 1;
      Convergence : constant CV.Remaining_RM_Edge_Convergence_Model := CV.Build (Apps, Prior_Fingerprint);
   begin
      Assert (CV.Changed_Count (Convergence) = CV.Row_Count (Convergence),
              "changed application fingerprints should force bounded recheck for all rows");
      Assert (CV.Count_Action (Convergence, CV.Remaining_RM_Edge_Convergence_Action_Recheck_Again) = CV.Row_Count (Convergence),
              "changed rows should request another recheck");
   end Detects_Changed_Application_Fingerprint;

   procedure Query_Surface_Finds_Convergence_Rows
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Convergence : constant CV.Remaining_RM_Edge_Convergence_Model := Build_Convergence;
      Row : constant CV.Remaining_RM_Edge_Convergence_Row := CV.Row_At (Convergence, 3);
   begin
      Assert (CV.Query_Count (CV.Query_Node (Convergence, Row.Node)) >= 1,
              "node query should find remaining-edge convergence rows");
      Assert (CV.Query_Count (CV.Query_Source_Fingerprint (Convergence, Row.Source_Fingerprint)) >= 1,
              "source fingerprint query should find remaining-edge convergence rows");
      Assert (Row.Blocks_Downstream,
              "stable withheld convergence row should block downstream trust");
   end Query_Surface_Finds_Convergence_Rows;

   overriding procedure Register_Tests (T : in out Test_Case) is
      use AUnit.Test_Cases.Registration;
   begin
      Register_Routine
        (T, Application_Rows_Are_Classified_For_Convergence'Access,
         "application rows are classified for convergence");
      Register_Routine
        (T, Preserves_Remaining_Edge_And_Closure_Blockers'Access,
         "preserves remaining-edge and closure blockers");
      Register_Routine
        (T, Detects_Changed_Application_Fingerprint'Access,
         "detects changed application fingerprint");
      Register_Routine
        (T, Query_Surface_Finds_Convergence_Rows'Access,
         "query surface finds convergence rows");
   end Register_Tests;

end Test_Ada_Legality.Expressions.RM_Coverage_Review_Convergence_Legality;
