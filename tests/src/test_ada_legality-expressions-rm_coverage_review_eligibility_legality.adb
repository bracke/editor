...
package body Test_Ada_Legality.Expressions.RM_Coverage_Review_Eligibility_Legality is
   ...
   begin
      return AUnit.Format ("Ada remaining RM edge recheck eligibility legality");
   end Name;
   ...
   procedure Worklist_Is_Converted_To_Bounded_Recheck_Eligibility
   ...
   begin
      Assert (EL.Row_Count (Rechecks) = WL.Count (Worklist),
              "recheck eligibility should preserve remaining-edge worklist row count");
      Assert (EL.Current_Evidence_Count (Rechecks) = WL.Current_Evidence_Count (Worklist),
              "current evidence should remain non-recheck evidence");
      Assert (EL.Blocked_Count (Rechecks) = WL.Ready_For_Recheck_Count (Worklist),
              "unresolved worklist items should remain bounded recheck blockers");
      Assert (EL.Stable_Fingerprint (Rechecks) /= 0,
              "eligibility fingerprint should be deterministic");
   end Worklist_Is_Converted_To_Bounded_Recheck_Eligibility;

   procedure Preserves_Remaining_Edge_And_Closure_Prerequisites
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Rechecks : constant EL.Remaining_RM_Edge_Recheck_Model := Build_Rechecks;
   begin
      Assert (EL.Count_Status (Rechecks, EL.Remaining_RM_Edge_Recheck_Not_Required_Current) = 1,
              "accepted remaining-edge rows should not require recheck");
      Assert (EL.Remaining_Edge_Blocked_Count (Rechecks) = 1,
              "remaining-edge blockers should remain distinct prerequisites");
      Assert (EL.Stabilized_Closure_Blocked_Count (Rechecks) = 2,
              "missing or blocked stabilized closure should remain closure prerequisites");
      Assert (EL.Count_Action (Rechecks, EL.Remaining_RM_Edge_Recheck_Action_Wait_For_Remaining_Edge) = 1,
              "remaining-edge action should wait for remaining-edge evidence");
      Assert (EL.Count_Action (Rechecks, EL.Remaining_RM_Edge_Recheck_Action_Wait_For_Stabilized_Closure) = 2,
              "closure action should wait for stabilized closure evidence");
   end Preserves_Remaining_Edge_And_Closure_Prerequisites;
   ...
   procedure Preserves_Fingerprint_And_Family_Blockers
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      ...
      Rechecks : constant EL.Remaining_RM_Edge_Recheck_Model := Build_Rechecks;
   begin
      Assert (EL.Count_Family (Rechecks, D.Remaining_RM_Edge_Stabilized_Diagnostic_Remaining_Edge) = 1,
              "remaining-edge diagnostic family should survive recheck eligibility");
      Assert (EL.Count_Family (Rechecks, D.Remaining_RM_Edge_Stabilized_Diagnostic_Stabilized_Closure) = 2,
              "stabilized-closure diagnostic family should survive recheck eligibility");
      Assert (EL.Fingerprint_Blocked_Count (Rechecks) = 1,
              "source or substitution fingerprint blockers should remain separate");
      Assert (EL.Count_Action (Rechecks, EL.Remaining_RM_Edge_Recheck_Action_Wait_For_Source_Fingerprint) = 1,
   ...
   procedure Query_Surface_Finds_Node_And_Source_Fingerprint
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Rechecks : constant EL.Remaining_RM_Edge_Recheck_Model := Build_Rechecks;
      Row : constant EL.Remaining_RM_Edge_Recheck_Row := EL.Row_At (Rechecks, 3);
   begin
      Assert (EL.Query_Count (EL.Query_Node (Rechecks, Row.Node)) >= 1,
              "node query should find remaining-edge recheck rows");
      Assert (EL.Query_Count (EL.Query_Source_Fingerprint (Rechecks, Row.Source_Fingerprint)) >= 1,
              "source fingerprint query should find remaining-edge recheck rows");
      Assert (EL.Blocks_Downstream (Row),
              "blocked remaining-edge recheck row should block downstream trust");
   ...
   begin
      Register_Routine
        (T, Worklist_Is_Converted_To_Bounded_Recheck_Eligibility'Access,
         "worklist is converted to bounded recheck eligibility");
      Register_Routine
        (T, Preserves_Remaining_Edge_And_Closure_Prerequisites'Access,
         "preserves remaining-edge and closure prerequisites");
      Register_Routine
...
end Test_Ada_Legality.Expressions.RM_Coverage_Review_Eligibility_Legality;
