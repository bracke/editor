...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1406;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1406 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1406;
   ...
   use type Audit.Array_Slice_Index_Closure;
   use type Audit.Array_Slice_Index_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1406");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Array_Slice_Index_Form := Audit.Form_Array_Slice_Index_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Array_Slice_Index_Constraint_Edge;
      ...
      Row.Closure := Audit.Closure_Array_Index_Constraint;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/array-slice-index-constraint.adb");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("array indexes, slices, multidimensional index counts, runtime index checks, aggregates, assignments, and diagnostics must share one canonical index/subtype result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1406");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1406");
      Row.Blocker_Family :=
        To_Unbounded_String ("RM.Arrays.Slice_Index_Constraint");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1406 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1406 precision classification");
   end Expect_Status;

   procedure Test_Array_Slice_Index_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Array_Slice_Index_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Index_Count_Mismatch);
      Row.Index_Count_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Index_Check_Preserved);
      Row.Runtime_Index_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (5, Precision.Class_Indeterminate,
                       Audit.Form_Indeterminate_Missing_Index_Subtype_Evidence);
      Row.Missing_Index_Subtype_Evidence := True;
      Row.Complete_Array_Index_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1406 should close the array slice/index constraint gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Array_Slice_Index_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Index_Count_Mismatch,
      ...
      Expect_Status (Results, 4, Audit.Status_Warning_Only_Preserved,
                     Precision.Class_Legal);
      Expect_Status (Results, 5, Audit.Status_Indeterminate_Missing_Index_Subtype_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Array_Slice_Index_Gap_Remediated;

   procedure Test_Array_Slice_Index_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Non_Array_Prefix := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (11);
      Row.Index_Type_Mismatch := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (13);
      Row.Slice_Bounds_Not_Discrete := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (15);
      Row.Stale_Array_Index_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Non_Array_Prefix,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Index_Type_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Static_Index_Out_Of_Range,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Illegal_Slice_Bounds_Not_Discrete,
                     Precision.Class_Illegal);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 15, Audit.Status_Indeterminate_Stale_Array_Index_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Array_Slice_Index_Rejections;
   ...
   begin
      ...
      Row := Base_Row (25);
      Row.Consumer_Array_Index_State_Agrees := False;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (27);
      Row.Index_Fingerprint := 1;
      Row.Expected_Index_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 26, Audit.Status_Static_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Index_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Array_Slice_Index_Gap_Remediated'Access,
         "array slice/index constraint gap remediated");
      Register_Routine
        (T, Test_Array_Slice_Index_Rejections'Access,
         "non-array prefix, index count/type mismatch, static bounds, nondiscrete slice bounds, private view, and stale index evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1406;
