...
with Editor.Ada_RM_Remediation.Expressions.Update_Expression_Target;

package body Test_Ada_RM_Remediation.Expressions.Update_Expression_Target is

   package Audit renames Editor.Ada_RM_Remediation.Expressions.Update_Expression_Target;
   ...
   use type Audit.Update_Closure;
   use type Audit.Update_Form;
   ...
   function Base_Row
     (Id : Natural;
      ...
      Form : Audit.Update_Form := Audit.Form_Update_Target_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Update_Expression_Target_Name_Edge;
      ...
      Row.Closure := Audit.Closure_Update_Expression;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/update-expression-target-name.ads");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("update expressions, target-name @ usage, component update associations, expected target type, runtime checks, and diagnostic consumers must share one canonical update-expression result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remediation.Expressions.Update_Expression_Target");
      Row.Candidate_Case := To_Unbounded_String ("Case 1396");
      Row.Blocker_Family :=
        To_Unbounded_String ("RM.Expressions.Update_Target_Name");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected case 1396 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected case 1396 precision classification");
   end Expect_Status;

   procedure Test_Update_Target_Name_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Update_Target_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Component_Type_Mismatch);
      ...
      Row.Complete_Update_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "case 1396 should close the update expression target-name gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Update_Target_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Component_Type_Mismatch,
   ...
   end Test_Update_Target_Name_Gap_Remediated;

   procedure Test_Update_Target_Name_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Missing_Update_Target := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (12);
      Row.Target_Name_Outside_Update := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Missing_Update_Evidence := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Stale_Update_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Missing_Target,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Component_Type_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Target_Name_Outside_Update,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Stale_Update_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Update_Target_Name_Rejections;
   ...
   begin
      ...
      Row := Base_Row (26);
      Row.Update_Fingerprint := 1;
      Row.Expected_Update_Fingerprint := 2;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (27);
      Row.Target_Fingerprint := 1;
      Row.Expected_Target_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 25, Audit.Status_Semantic_Result_Unconsumed,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Update_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Target_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Update_Target_Name_Gap_Remediated'Access,
         "update expression target-name gap remediated");
      Register_Routine
        (T, Test_Update_Target_Name_Rejections'Access,
         "missing-initializer update object, result type mismatch, lifetime profile mismatch, missing update, and stale update evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remediation.Expressions.Update_Expression_Target;
