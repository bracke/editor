...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1371;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1371 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1371;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1371");
   end Name;
   ...
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Private_Child_Renaming_Visibility_Edge;
      ...
      Row.Source_File := To_Unbounded_String ("src/private-child-renaming-visibility.adb");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("private-child renaming visibility must preserve canonical selected-name, alias, and consumer evidence");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1371");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1371");
      Row.Blocker_Family :=
        To_Unbounded_String ("RM.Names.Visibility.Private_Child_Renaming");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1371 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1371 precision classification");
   end Expect_Status;

   procedure Test_Private_Child_Renaming_Gap_Remediated
   ...
   begin
      ...
      Assert (Audit.Gap_Remediated (Results),
              "pass1371 should close the selected private-child renaming visibility gap");
   ...
   end Test_Private_Child_Renaming_Gap_Remediated;

   procedure Test_Renaming_Visibility_Rejections
   ...
   begin
      ...
      Expect_Status (Results, 15, Audit.Status_Illegal_Renamed_Type_View_Mismatch,
                     Precision.Class_Illegal);
   end Test_Renaming_Visibility_Rejections;
   ...
   begin
      Register_Routine
        (T, Test_Private_Child_Renaming_Gap_Remediated'Access,
         "private child renaming visibility gap remediated");
      Register_Routine
        (T, Test_Renaming_Visibility_Rejections'Access,
         "renaming visibility rejections");
      Register_Routine
...
end Test_Ada_RM_Remaining_Gap_Remediation_Pass1371;
