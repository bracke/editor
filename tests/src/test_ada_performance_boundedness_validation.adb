...
with Editor.Ada_Performance_Boundedness_Validation;

package body Test_Ada_Performance_Boundedness_Validation is
   package Perf renames Editor.Ada_Performance_Boundedness_Validation;
   ...
   begin
      return AUnit.Format ("Test_Ada_Performance_Boundedness_Validation");
   end Name;
   ...
   begin
      Assert (Item.Status = Status, "unexpected boundedness status");
      Assert (Item.Result_Class = Result_Class,
              "unexpected boundedness result class");
   ...
   begin
      Perf.Add_Row (Input, Base_Row
        (40, Perf.Scenario_Large_File,
         "large file semantic analysis stays within budget"));
      Perf.Add_Row (Input, Base_Row
        (41, Perf.Scenario_Large_File,
         "duplicate large-file row must not substitute for stale rejection"));
      Perf.Add_Row (Input, Base_Row
        (42, Perf.Scenario_Multi_Buffer_Project,
         "multi-buffer project closure stays bounded"));
      Perf.Add_Row (Input, Base_Row
        (43, Perf.Scenario_Cross_Unit_Index,
         "cross-unit index traversal stays bounded"));
      Perf.Add_Row (Input, Base_Row
        (44, Perf.Scenario_Cancellation,
         "cancellation is acknowledged before publishing semantic consumers"));
      Perf.Add_Row (Input, Base_Row
        (45, Perf.Scenario_Deterministic_Replay,
         "deterministic replay preserves semantic fingerprints"));
      Perf.Add_Row (Input, Base_Row
        (46, Perf.Scenario_Budget_Exhaustion,
         "budget exhaustion degrades deterministically"));

      Model := Perf.Build (Input);

      Assert (not Perf.Performance_Boundedness_Complete (Model),
              "boundedness completion must require each scenario exactly once");
      Assert (Model.Total_Rows = 7,
              "regression keeps the previous accepted-row-count shape");
      Assert (Model.Required_Scenario_Count = 7,
              "all required boundedness scenarios are known to the model");
      Assert (Model.Duplicate_Scenario_Count = 1,
              "duplicate boundedness scenarios are counted");
      Assert (Model.Missing_Scenario_Count = 1,
              "omitted boundedness scenarios are counted");
      Expect_Status (Model, 41, Perf.Status_Rejected_Duplicate_Scenario,
                     Perf.Class_Rejected);
