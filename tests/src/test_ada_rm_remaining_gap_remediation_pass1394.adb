...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1394;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1394 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1394;
   ...
   use type Audit.Reduction_Closure;
   use type Audit.Reduction_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1394");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Reduction_Form := Audit.Form_Reduction_Combiner_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Reduction_Expression_Combiner_Profile_Edge;
      ...
      Row.Closure := Audit.Closure_Reduction_Expression;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/reduction-expression-combiner-profile.ads");
      ...
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1394");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1394");
      Row.Blocker_Family :=
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1394 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1394 precision classification");
   end Expect_Status;

   procedure Test_Reduction_Combiner_Profile_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Reduction_Combiner_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Seed_Type_Mismatch);
      Row.Seed_Type_Mismatch := True;
      ...
      Row.Complete_Reduction_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1394 should close the reduction expression combiner/profile gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Reduction_Combiner_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Seed_Type_Mismatch,
   ...
   end Test_Reduction_Combiner_Profile_Gap_Remediated;

   procedure Test_Reduction_Combiner_Profile_Rejections
   ...
   begin
      ...
      Row := Base_Row (12);
      Row.Combiner_Profile_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Missing_Reduction_Evidence := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Stale_Reduction_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Non_Reducible_Domain,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Seed_Type_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Combiner_Profile_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Stale_Reduction_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Reduction_Combiner_Profile_Rejections;
   ...
   begin
      ...
      Row := Base_Row (26);
      Row.Reduction_Fingerprint := 1;
      Row.Expected_Reduction_Fingerprint := 2;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (27);
      Row.Combiner_Fingerprint := 1;
      Row.Expected_Combiner_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 25, Audit.Status_Semantic_Result_Unconsumed,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Reduction_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Combiner_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Reduction_Combiner_Profile_Gap_Remediated'Access,
         "reduction expression combiner/profile gap remediated");
      Register_Routine
        (T, Test_Reduction_Combiner_Profile_Rejections'Access,
         "non-reducible reduction domain, seed type mismatch, combiner profile mismatch, missing reduction, and stale reduction evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1394;
