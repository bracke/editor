...
package body Test_Ada_RM_Remediation.Expressions.Quantified_Iterator_Predicates is
   ...
   begin
      Assert (Item.Status = Status, "unexpected case 1393 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected case 1393 precision classification");
   end Expect_Status;
   ...
   begin
      ...
      Assert (Audit.Gap_Remediated (Results),
              "case 1393 should close the quantified expression/iterator filter gap");
...
end Test_Ada_RM_Remediation.Expressions.Quantified_Iterator_Predicates;
