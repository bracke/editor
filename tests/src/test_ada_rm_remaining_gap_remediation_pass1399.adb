...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1399;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1399 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1399;
   ...
   use type Audit.Case_Result_Closure;
   use type Audit.Case_Result_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1399");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Case_Result_Form := Audit.Form_Case_Result_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Case_Expression_Others_Coverage_Edge;
      ...
      Row.Closure := Audit.Closure_Case_Expression;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/case-expression-others-coverage.ads");
      ...
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1399");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1399");
      Row.Blocker_Family :=
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1399 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1399 precision classification");
   end Expect_Status;

   procedure Test_Case_Expression_Others_Coverage_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Case_Result_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Alternative_Type_Mismatch);
      Row.Alternative_Type_Mismatch := True;
      ...
      Row.Complete_Case_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1399 should close the case expression others coverage gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Case_Result_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Alternative_Type_Mismatch,
   ...
   end Test_Case_Expression_Others_Coverage_Gap_Remediated;

   procedure Test_Case_Expression_Others_Coverage_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Missing_Case_Alternative := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (14);
      Row.Stale_Case_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Missing_Case_Alternative,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Alternative_Type_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Choice_Overlap,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Stale_Case_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Case_Expression_Others_Coverage_Rejections;
   ...
   begin
      ...
      Row := Base_Row (26);
      Row.Case_Fingerprint := 1;
      Row.Expected_Case_Fingerprint := 2;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (27);
      Row.Alternative_Fingerprint := 1;
      Row.Expected_Alternative_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 25, Audit.Status_Semantic_Result_Unconsumed,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Case_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Alternative_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Case_Expression_Others_Coverage_Gap_Remediated'Access,
         "case expression others coverage gap remediated");
      Register_Routine
        (T, Test_Case_Expression_Others_Coverage_Rejections'Access,
         "missing case alternative, case alternative type mismatch, overlapping case choices, missing expected-type evidence, and stale case evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1399;
