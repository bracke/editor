...
with Editor.Ada_Documentation_Handoff;

package body Test_Ada_Documentation_Handoff is
   package Handoff renames Editor.Ada_Documentation_Handoff;
   ...
   begin
      return AUnit.Format ("Test_Ada_Documentation_Handoff");
   end Name;
   ...
   begin
      Assert (Item.Status = Status, "unexpected documentation handoff status");
      Assert (Item.Result_Class = Result_Class,
              "unexpected documentation handoff result class");
   ...
   begin
      Handoff.Add_Row (Input, Base_Row
        (1, Handoff.Section_Final_Status,
         "final status",
         "Remaining gap closure is frozen; project-scale gates continue as validation."));
      Handoff.Add_Row (Input, Base_Row
      ...
      Handoff.Add_Row (Input, Base_Row
        (5, Handoff.Section_Operational_Handoff,
         "Operational handoff",
         "Next validation step consumes release evidence, not broad audit churn."));
      ...
      Assert (Model.Required_Section_Count = 5, "all required handoff sections tracked");
      Assert (Model.Missing_Section_Count = 0, "no handoff section missing");
      Assert (Model.Duplicate_Section_Count = 0, "no duplicate handoff section expected");
   ...
   procedure Test_Duplicate_And_Missing_Sections_Block_Completion
   ...
   begin
      Handoff.Add_Row (Input, Base_Row
        (101, Handoff.Section_Final_Status,
         "final status",
         "Remaining gap closure is frozen; project-scale gates continue as validation."));
      Handoff.Add_Row (Input, Base_Row
        (102, Handoff.Section_Guarantees,
         "Guaranteed semantic invariants",
         "Snapshot-owned bounded analysis, stable consumers, and stale evidence rejection are documented."));
      Handoff.Add_Row (Input, Base_Row
        (103, Handoff.Section_Intentional_Approximation,
         "Intentional approximation boundary",
         "Real corpus failures may create evidence-driven fixes, but not speculative Remaining_* edges."));
      Handoff.Add_Row (Input, Base_Row
        (104, Handoff.Section_Future_Work_Rule,
         "Future work rule",
         "No new Remaining_* edge is allowed without a source-shaped failing case or RM contradiction."));
      Handoff.Add_Row (Input, Base_Row
        (105, Handoff.Section_Future_Work_Rule,
         "Duplicate future work rule",
         "A duplicate future work rule cannot replace the operational handoff section."));

      Model := Handoff.Build (Input);

      Assert (not Handoff.Documentation_Handoff_Complete (Model),
              "duplicate and missing handoff sections must block completion");
      Assert (Model.Required_Section_Count = 5,
              "required handoff section count remains stable");
      Assert (Model.Missing_Section_Count = 1,
              "missing operational handoff section detected");
      Assert (Model.Duplicate_Section_Count = 1,
              "duplicate future work rule section detected");
      Expect_Status
        (Model, 105, Handoff.Status_Rejected_Duplicate_Section,
         Handoff.Class_Rejected);
   end Test_Duplicate_And_Missing_Sections_Block_Completion;
   ...
   begin
      Register_Routine (T, Test_Final_Handoff_Documentation_Accepted'Access,
                        "final documentation handoff accepted");
      Register_Routine (T, Test_Duplicate_And_Missing_Sections_Block_Completion'Access,
                        "documentation handoff requires each named section exactly once");
      Register_Routine (T, Test_Missing_Section_Rejections'Access,
                        "rejects incomplete handoff sections");
      Register_Routine (T, Test_Reopened_Gap_And_Speculative_Edge_Rejections'Access,
                        "rejects reopened finite-gap closure");
      Register_Routine (T, Test_Stale_And_Missing_Evidence_Rejections'Access,
                        "rejects stale or missing handoff evidence");
   end Register_Tests;

end Test_Ada_Documentation_Handoff;
