...
with Editor.Ada_RM_Remediation.Expressions.Membership_Range_Predicate;

package body Test_Ada_RM_Remediation.Expressions.Membership_Range_Predicate is

   package Audit renames Editor.Ada_RM_Remediation.Expressions.Membership_Range_Predicate;
   ...
   use type Audit.Membership_Closure;
   use type Audit.Membership_Form;
   ...
   function Base_Row
     (Id : Natural;
      ...
      Form : Audit.Membership_Form := Audit.Form_Membership_Result_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Membership_Range_Predicate_Edge;
      Row.Family := Matrix.Family_Static_Expressions_Choices;
      Row.Owner := Matrix.Slice_Membership_Case_Choice;
      ...
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/membership-range-predicate.ads");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("membership tests, range alternatives, subtype predicates, runtime membership checks, and diagnostic consumers must share one canonical membership/range/predicate result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remediation.Expressions.Membership_Range_Predicate");
      Row.Candidate_Case := To_Unbounded_String ("Case 1400");
      Row.Blocker_Family :=
        To_Unbounded_String ("RM.Expressions.Membership_Range_Predicate");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected case 1400 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected case 1400 precision classification");
   end Expect_Status;

   procedure Test_Membership_Range_Predicate_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Membership_Result_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Subject_Type_Mismatch);
      Row.Subject_Type_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Membership_Check_Preserved);
      Row.Runtime_Membership_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Row.Complete_Membership_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "case 1400 should close the membership/range/predicate gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Membership_Result_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Subject_Type_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 3, Audit.Status_Runtime_Membership_Check_Preserved,
   ...
   end Test_Membership_Range_Predicate_Gap_Remediated;

   procedure Test_Membership_Range_Predicate_Rejections
   ...
   begin
      ...
      Row := Base_Row (11);
      Row.Non_Static_Range_In_Static_Context := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Range_Bounds_Reversed := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (14);
      Row.Missing_Subject_Type_Evidence := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (15);
      Row.Stale_Membership_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Subject_Type_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Non_Static_Range_In_Static_Context,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Range_Bounds_Reversed,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Illegal_Static_Predicate_Failure,
                     Precision.Class_Illegal);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 15, Audit.Status_Indeterminate_Stale_Membership_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Membership_Range_Predicate_Rejections;
   ...
   begin
      ...
      Row := Base_Row (26);
      Row.Membership_Fingerprint := 1;
      Row.Expected_Membership_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 25, Audit.Status_Semantic_Result_Unconsumed,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Membership_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Membership_Range_Predicate_Gap_Remediated'Access,
         "membership range predicate gap remediated");
      Register_Routine
        (T, Test_Membership_Range_Predicate_Rejections'Access,
         "subject mismatch, non-static range, reversed range, static predicate failure, missing evidence, and stale evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remediation.Expressions.Membership_Range_Predicate;
