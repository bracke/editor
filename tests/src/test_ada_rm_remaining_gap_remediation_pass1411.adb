...
package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1411 is
   ...
   use type Audit.Task_Termination_Abort_Closure;
   use type Audit.Task_Termination_Abort_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1411");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Task_Termination_Abort_Form :=
        Audit.Form_Task_Termination_Abort_Finalization_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Task_Termination_Abort_Finalization_Edge;
      ...
      Row.Closure := Audit.Closure_Task_Termination;
      Row.Form := Form;
      Row.Source_File :=
        To_Unbounded_String ("src/task-termination-abort-finalization.adb");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("task termination, abort completion, dependent masters, controlled "
   ...
   begin
      ...
      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Abort_Finalization_Order);
      Row.Abort_Finalization_Order_Hazard := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Abort_Finalization_Check_Preserved);
      Row.Runtime_Abort_Finalization_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (5, Precision.Class_Indeterminate,
                       Audit.Form_Indeterminate_Missing_Finalization_Evidence);
      ...
      Expect_Status
        (Results, 1,
         Audit.Status_Task_Termination_Abort_Finalization_Resolved,
         Precision.Class_Legal);
      Expect_Status
        (Results, 2,
         Audit.Status_Illegal_Abort_Finalization_Order,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 3,
         Audit.Status_Runtime_Abort_Finalization_Check_Preserved,
   ...
   begin
      Row := Base_Row (10);
      Row.Termination_Dependent_Master_Hazard := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (11);
      Row.Async_Select_Finalization_Hazard := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Controlled_Finalization_Double_Finalize := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Protected_Finalization_Blocking := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (15);
      Row.Stale_Lifecycle_Evidence := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (16);
      Row.Missing_Termination_Evidence := True;
      Row.Complete_Task_Termination_Evidence := False;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 10,
         Audit.Status_Illegal_Termination_Dependent_Master,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 11,
         Audit.Status_Illegal_Asynchronous_Select_Finalization_Hazard,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 12,
         Audit.Status_Illegal_Controlled_Finalization_Double_Finalize,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 13,
         Audit.Status_Illegal_Protected_Finalization_Blocking,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 14,
      ...
      Expect_Status
        (Results, 15,
         Audit.Status_Indeterminate_Stale_Lifecycle_Evidence,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 16,
         Audit.Status_Indeterminate_Missing_Termination_Evidence,
         Precision.Class_Indeterminate);
   end Test_Rejections;
   ...
   begin
      ...
      Row := Base_Row (25);
      Row.Consumer_Finalization_State_Agrees := False;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (26);
      Row.Task_Fingerprint := 1;
      Row.Expected_Task_Fingerprint := 2;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (27);
      Row.Master_Fingerprint := 1;
      Row.Expected_Master_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 26,
         Audit.Status_Task_Fingerprint_Mismatch,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 27,
   ...
   begin
      Register_Routine
        (T, Test_Gap_Remediated'Access,
         "task termination abort finalization gap remediated");
      Register_Routine
        (T, Test_Rejections'Access,
         "termination master, async select, controlled finalization, protected finalization, private view, and stale lifecycle rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1411;
