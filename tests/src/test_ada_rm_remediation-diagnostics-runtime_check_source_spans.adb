...
package body Test_Ada_RM_Remediation.Diagnostics.Runtime_Check_Source_Spans is
   ...
   begin
      Assert (Item.Status = Status, "unexpected case 1385 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected case 1385 precision classification");
   end Expect_Status;
   ...
   begin
      ...
      Assert (Audit.Gap_Remediated (Results),
              "case 1385 should close the diagnostic/runtime/warning source-span gap");
...
end Test_Ada_RM_Remediation.Diagnostics.Runtime_Check_Source_Spans;
