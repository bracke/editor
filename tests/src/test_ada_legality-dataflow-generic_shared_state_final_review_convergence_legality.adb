...
package body Test_Ada_Legality.Dataflow.Generic_Shared_State_Final_Review_Convergence_Legality is
   ...
   begin
      return AUnit.Format ("Ada generic shared-state final recheck convergence legality");
   end Name;
   ...
   begin
      ...
      Result.Source_Fingerprint := 1243 * Id;
      Result.Expected_Source_Fingerprint := 1243 * Id;
      Result.Substitution_Fingerprint := 4213 * Id;
      Result.Expected_Substitution_Fingerprint := 4213 * Id;
      return Result;
   ...
   procedure Application_Rows_Converge_Or_Remain_Stable_Withheld
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Model : constant C.Generic_Shared_State_Final_Convergence_Model := Build_Convergence;
      Row   : constant C.Generic_Shared_State_Final_Convergence_Row := C.Row_At (Model, 1);
   begin
      Assert (C.Count (Model) = 7, "seven convergence rows expected");
      Assert (C.Converged_Count (Model) = 1, "current non-diagnostic row should converge as not required");
      Assert (C.Stable_Withheld_Count (Model) = 6, "blocked rows should remain stably withheld");
      Assert (C.Current_Count (Model) = 0, "fixture has no freshly accepted current recheck row");
      Assert (C.Changed_Count (Model) = 0, "fresh convergence should not be marked changed");
      Assert
        (Row.Status = C.Generic_Shared_State_Final_Converged_Not_Required,
         "accepted diagnostic-free generic/shared-state evidence should converge as not required");
      Assert
        (Row.Action = C.Generic_Shared_State_Final_Convergence_Action_Skip_Not_Required,
         "converged non-diagnostic evidence should skip fresh recheck work");
   end Application_Rows_Converge_Or_Remain_Stable_Withheld;

   procedure Convergence_Preserves_Generic_Shared_State_Blocker_Families
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Model : constant C.Generic_Shared_State_Final_Convergence_Model := Build_Convergence;
   begin
      Assert
        (C.Count_By_Status (Model, C.Generic_Shared_State_Final_Stable_Withheld_Generic_Replay) = 1,
         "generic replay blocker should remain distinct through convergence");
      Assert
        (C.Count_By_Status (Model, C.Generic_Shared_State_Final_Stable_Withheld_Abstract_Or_Shared_State) = 1,
         "shared-state blocker should remain distinct through convergence");
      Assert
        (C.Count_By_Status (Model, C.Generic_Shared_State_Final_Stable_Withheld_Representation) = 1,
         "representation blocker should remain distinct through convergence");
      Assert
        (C.Count_By_Status (Model, C.Generic_Shared_State_Final_Stable_Withheld_Predicate_Invariant) = 1,
         "predicate blocker should remain distinct through convergence");
      Assert
        (C.Count_By_Status (Model, C.Generic_Shared_State_Final_Stable_Withheld_Stale_Or_Fingerprint) = 1,
         "fingerprint blocker should remain distinct through convergence");
      Assert
        (C.Count_By_Status (Model, C.Generic_Shared_State_Final_Stable_Withheld_Dataflow) = 1,
         "dataflow blocker should remain distinct through convergence");
   end Convergence_Preserves_Generic_Shared_State_Blocker_Families;
   ...
   procedure Convergence_Detects_Changed_Application_Fingerprint
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      ...
      Model   : constant C.Generic_Shared_State_Final_Convergence_Model := C.Build (Apps, Current + 1);
   begin
      Assert (C.Count (Model) = 7, "changed convergence still mirrors application rows");
      Assert (C.Changed_Count (Model) = 7, "all rows should force recheck when the model fingerprint changes");
      Assert
        (C.Count_By_Status (Model, C.Generic_Shared_State_Final_Changed_Since_Previous) = 7,
         "changed fingerprint should classify every row as changed since previous");
      Assert
        (C.Row_At (Model, 1).Action = C.Generic_Shared_State_Final_Convergence_Action_Recheck_Again,
   ...
   begin
      Assert
        (C.Query_Count (C.Find_By_Node (Model, Node)) = 1,
         "node lookup should find the fingerprint blocker convergence row");
      Assert
        (C.Query_Count (C.Find_By_Source_Fingerprint (Model, 1243 * 6)) = 1,
         "source fingerprint lookup should find the fingerprint blocker convergence row");
      Assert
        (C.Query_Count (C.Find_By_Substitution_Fingerprint (Model, 4213 * 6)) = 1,
   ...
   begin
      Register_Routine
        (T, Application_Rows_Converge_Or_Remain_Stable_Withheld'Access,
         "generic/shared-state application rows converge or remain stably withheld");
      Register_Routine
        (T, Convergence_Preserves_Generic_Shared_State_Blocker_Families'Access,
         "generic/shared-state convergence preserves blocker families");
      Register_Routine
        (T, Convergence_Detects_Changed_Application_Fingerprint'Access,
         "generic/shared-state convergence detects changed fingerprints");
      Register_Routine
        (T, Convergence_Lookups_And_Fingerprints_Are_Deterministic'Access,
         "generic/shared-state convergence lookups and fingerprints are deterministic");
   end Register_Tests;

end Test_Ada_Legality.Dataflow.Generic_Shared_State_Final_Review_Convergence_Legality;
