...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1421;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1421 is
   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1421;
   ...
   use type Audit.Controlled_Finalized_Discriminant_Closure;
   use type Audit.Controlled_Finalized_Discriminant_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1421");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Controlled_Finalized_Discriminant_Form := Audit.Form_Controlled_Discriminant_Component_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Controlled_Finalized_Discriminant_Component_Edge;
      ...
      Row.Closure := Audit.Closure_Controlled_Component_Evidence;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/controlled-finalized-discriminant-component.adb");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("controlled components governed by discriminants, finalized"
           & "discriminant defaults, assignment/finalization ordering, runtime"
           & "finalization checks, warning diagnostics, stale finalization"
           & "evidence, and semantic consumers must share one source-shaped"
           & "legality result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1421");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1421");
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1421 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1421 precision classification");
   end Expect_Status;
   ...
   begin
      Audit.Add_Row
        (Input,
         Base_Row (1, Precision.Class_Legal,
                   Audit.Form_Controlled_Discriminant_Component_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Controlled_Component_Depends_On_Mutable_Discriminant);
      Row.Controlled_Component_Depends_On_Mutable_Discriminant := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Finalization_Check_Preserved);
      Row.Runtime_Finalization_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (5, Precision.Class_Indeterminate,
                       Audit.Form_Indeterminate_Missing_Discriminant_Evidence);
      Row.Missing_Discriminant_Evidence := True;
      Row.Complete_Discriminant_Evidence := False;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (6, Precision.Class_Legal,
                       Audit.Form_Finalization_Order_Resolved);
      Row.Closure := Audit.Closure_Discriminant_Default_Evidence;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1421 should close the controlled finalized discriminant component gap");
      ...
      Expect_Status
        (Results, 1,
         Audit.Status_Controlled_Discriminant_Component_Resolved,
         Precision.Class_Legal);
      Expect_Status
        (Results, 2,
         Audit.Status_Illegal_Controlled_Component_Depends_On_Mutable_Discriminant,
      ...
      Expect_Status
        (Results, 5,
         Audit.Status_Indeterminate_Missing_Discriminant_Evidence,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 6,
         Audit.Status_Finalization_Order_Resolved,
         Precision.Class_Legal);
   ...
   begin
      ...
      Row := Base_Row (11);
      Row.Discriminant_Default_Finalized_Object := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Limited_Controlled_Discriminant_Copy := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Premature_Finalization_During_Assignment := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Component_Finalization_Order_Conflict := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (17);
      Row.Missing_Finalization_Evidence := True;
      Row.Complete_Finalization_Evidence := False;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 11,
         Audit.Status_Illegal_Discriminant_Default_Finalized_Object,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 12,
         Audit.Status_Illegal_Limited_Controlled_Discriminant_Copy,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 13,
         Audit.Status_Illegal_Premature_Finalization_During_Assignment,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 14,
         Audit.Status_Illegal_Component_Finalization_Order_Conflict,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 15,
         Audit.Status_Indeterminate_Private_Controlled_View,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 16,
         Audit.Status_Indeterminate_Stale_Finalization_Evidence,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 17,
   ...
   begin
      ...
      Expect_Status
        (Results, 26,
         Audit.Status_Discriminant_Fingerprint_Mismatch,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 27,
   ...
   begin
      Register_Routine
        (T, Test_Gap_Remediated'Access,
         "controlled finalized discriminant component gap remediated");
      Register_Routine
        (T, Test_Rejections'Access,
         "controlled finalized discriminant component rejection cases");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1421;
