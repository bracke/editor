...
with Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Worklist;
with Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Review_Eligibility;
with Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Review_Application;
with Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Review_Convergence;
with Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Stabilization;
with Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Closure;
with Editor.Ada_Syntax_Tree;

package body Test_Ada_Legality.Dataflow.Generic_Shared_State_Final_Stable_Closure_Legality is
   ...
   package Diag renames Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Diagnostics;
   ...
   package W renames Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Worklist;
   ...
   package R renames Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Review_Eligibility;
   ...
   package A renames Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Review_Application;
   ...
   package C renames Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Review_Convergence;
   ...
   package S renames Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Stabilization;
   ...
   package Z renames Editor.Ada_Legality.Dataflow.Generic_Shared_State_Repair_Final_Closure;
   use type Z.Generic_Shared_State_Final_Stabilization_Status;
   use type Z.Generic_Shared_State_Final_Stabilization_Action;
   use type Z.Generic_Shared_State_Final_Closure_Family;
   use type Z.Generic_Shared_State_Final_Stabilized_Closure_Id;
   use type Z.Generic_Shared_State_Final_Stabilized_Closure_Status;
   use type Z.Generic_Shared_State_Final_Stabilized_Closure_Action;
   use type Z.Generic_Shared_State_Final_Stabilized_Closure_Row;
   use type Z.Generic_Shared_State_Final_Stabilized_Closure_Model;
   use type Z.Generic_Shared_State_Final_Stabilized_Closure_Set;
   ...
   begin
      return AUnit.Format ("Ada generic shared-state final stabilized closure legality");
   end Name;
   ...
   begin
      ...
      Result.Source_Fingerprint := 1245 * Id;
      Result.Expected_Source_Fingerprint := 1245 * Id;
      Result.Substitution_Fingerprint := 4215 * Id;
      Result.Expected_Substitution_Fingerprint := 4215 * Id;
      return Result;
   ...
   function Build_Stabilized_Closure
     (Previous : Natural := 0) return Z.Generic_Shared_State_Final_Stabilized_Closure_Model is
   begin
      return Z.Build (Build_Stabilization_Gate (Previous));
   end Build_Stabilized_Closure;

   procedure Stabilized_Closure_Promotes_Accepted_And_Blocks_Prerequisites
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Model : constant Z.Generic_Shared_State_Final_Stabilized_Closure_Model := Build_Stabilized_Closure;
      Row   : constant Z.Generic_Shared_State_Final_Stabilized_Closure_Row := Z.Row_At (Model, 1);
   begin
      Assert (Z.Count (Model) = 7, "seven stabilized closure rows expected");
      Assert (Z.Accepted_Count (Model) = 1, "one accepted closure evidence row expected");
      Assert (Z.Blocked_Count (Model) = 6, "six closure blockers should be retained");
      Assert (Z.Current_Count (Model) = 0, "fixture has no current accepted recheck row");
      Assert (Z.Recheck_Required_Count (Model) = 0, "fresh closure should not require another recheck");
      Assert
        (Row.Status = Z.Generic_Shared_State_Final_Stabilized_Closure_Accepted_Not_Required,
         "diagnostic-free generic/shared-state row should become accepted closure evidence");
      Assert
        (Row.Action = Z.Generic_Shared_State_Final_Stabilized_Closure_Action_Accept_Not_Required,
         "not-required row should be accepted without re-emitting diagnostics");
      Assert (not Row.Blocks_Downstream, "accepted closure evidence must not block downstream consumers");
   end Stabilized_Closure_Promotes_Accepted_And_Blocks_Prerequisites;

   procedure Stabilized_Closure_Preserves_Generic_Shared_State_Blocker_Families
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Model : constant Z.Generic_Shared_State_Final_Stabilized_Closure_Model := Build_Stabilized_Closure;
   begin
      Assert
        (Z.Count_By_Status (Model, Z.Generic_Shared_State_Final_Stabilized_Closure_Blocker_Generic_Replay) = 1,
         "generic replay blocker should remain distinct in stabilized closure");
      Assert
        (Z.Count_By_Status (Model, Z.Generic_Shared_State_Final_Stabilized_Closure_Blocker_Abstract_Or_Shared_State) = 1,
         "shared-state blocker should remain distinct in stabilized closure");
      Assert
        (Z.Count_By_Status (Model, Z.Generic_Shared_State_Final_Stabilized_Closure_Blocker_Representation) = 1,
         "representation blocker should remain distinct in stabilized closure");
      Assert
        (Z.Count_By_Status (Model, Z.Generic_Shared_State_Final_Stabilized_Closure_Blocker_Predicate_Invariant) = 1,
         "predicate blocker should remain distinct in stabilized closure");
      Assert
        (Z.Count_By_Status (Model, Z.Generic_Shared_State_Final_Stabilized_Closure_Blocker_Stale_Or_Fingerprint) = 1,
         "fingerprint blocker should remain distinct in stabilized closure");
      Assert
        (Z.Count_By_Status (Model, Z.Generic_Shared_State_Final_Stabilized_Closure_Blocker_Dataflow) = 1,
         "dataflow blocker should remain distinct in stabilized closure");
   end Stabilized_Closure_Preserves_Generic_Shared_State_Blocker_Families;

   procedure Stabilized_Closure_Retains_Recheck_Required_Rows
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Apps    : constant A.Generic_Shared_State_Final_Application_Model := Build_Application;
      ...
      Gate_Model : constant S.Generic_Shared_State_Final_Stabilization_Gate_Model := S.Build (Conv);
      Model   : constant Z.Generic_Shared_State_Final_Stabilized_Closure_Model := Z.Build (Gate_Model);
   begin
      Assert (Z.Count (Model) = 7, "changed stabilized closure still mirrors application rows");
      Assert (Z.Recheck_Required_Count (Model) = 7, "all rows should retain recheck-required closure state");
      Assert
        (Z.Count_By_Status (Model, Z.Generic_Shared_State_Final_Stabilized_Closure_Recheck_Required) = 7,
         "changed fingerprint should keep every row out of stable closure");
      Assert
        (Z.Row_At (Model, 1).Action = Z.Generic_Shared_State_Final_Stabilized_Closure_Action_Recheck,
         "changed rows should request another bounded recheck");
   end Stabilized_Closure_Retains_Recheck_Required_Rows;

   procedure Stabilized_Closure_Lookups_And_Fingerprints_Are_Deterministic
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Model : constant Z.Generic_Shared_State_Final_Stabilized_Closure_Model := Build_Stabilized_Closure;
      Node  : constant Editor.Ada_Syntax_Tree.Node_Id := Editor.Ada_Syntax_Tree.Node_Id (124506);
   begin
      Assert
        (Z.Query_Count (Z.Find_By_Node (Model, Node)) = 1,
         "node lookup should find the fingerprint blocker stabilized closure row");
      Assert
        (Z.Query_Count (Z.Find_By_Source_Fingerprint (Model, 1245 * 6)) = 1,
         "source fingerprint lookup should find the fingerprint blocker stabilized closure row");
      Assert
        (Z.Query_Count (Z.Find_By_Substitution_Fingerprint (Model, 4215 * 6)) = 1,
      ...
      Assert (Z.Indeterminate_Count (Model) = 0, "fixture should not produce indeterminate closure rows");
   ...
   begin
      Register_Routine
        (T, Stabilized_Closure_Promotes_Accepted_And_Blocks_Prerequisites'Access,
         "generic/shared-state stabilized closure promotes accepted evidence and blocks prerequisites");
      Register_Routine
        (T, Stabilized_Closure_Preserves_Generic_Shared_State_Blocker_Families'Access,
         "generic/shared-state stabilized closure preserves blocker families");
      Register_Routine
        (T, Stabilized_Closure_Retains_Recheck_Required_Rows'Access,
         "generic/shared-state stabilized closure retains recheck-required rows");
      Register_Routine
        (T, Stabilized_Closure_Lookups_And_Fingerprints_Are_Deterministic'Access,
         "generic/shared-state stabilized closure lookups and fingerprints are deterministic");
   end Register_Tests;

end Test_Ada_Legality.Dataflow.Generic_Shared_State_Final_Stable_Closure_Legality;
