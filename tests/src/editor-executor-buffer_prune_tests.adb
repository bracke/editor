...
package body Editor.Executor.Buffer_Prune_Tests is
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 1) = B_Id,
      ...
      Assert (Editor.Buffer_Switcher.Has_Pending_Marked_Review (S.Surface.Buffer_Switcher)
              ...
              and then Editor.Buffer_Switcher.Row_At (S.Surface.Buffer_Switcher, 1).Id = B_Id,
   ...
   begin
      ...
      Assert (Editor.Buffers.Global_Contains (A_Id)
              and then Editor.Buffer_Switcher.Is_Marked (S.Surface.Buffer_Switcher, A_Id),
   ...
   begin
      ...
      Cmd := Editor.Commands.Payloads.Command_For_Id (Editor.Command_Ids.Command_Buffer_Switcher_Pending_Mark_Remove_Selected);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Editor.Buffer_Switcher.Select_Buffer_Or_Row (S.Surface.Buffer_Switcher, C_Id, 1);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 1) = A_Id,
      ...
      Assert (Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 2
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 1) = A_Id
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 2) = C_Id,
              "restore-last restores the most recently pruned still-open target");
      Assert (Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 1,
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 3
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 1) = A_Id
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 2) = B_Id
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 3) = C_Id,
              "restored pending targets return to original captured order");
      Assert (Editor.Buffer_Switcher.Has_Pending_Marked_Review (S.Surface.Buffer_Switcher)
              and then Editor.Buffer_Switcher.Row_Count (S.Surface.Buffer_Switcher) = 3,
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then Editor.Buffer_Switcher.Pending_Marked_Target_At (S.Surface.Buffer_Switcher, 1) = A_Id
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 1,
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Row_Count (S.Surface.Buffer_Switcher) = 2
              and then Editor.Buffer_Switcher.Row_At (S.Surface.Buffer_Switcher, 1).Id = B_Id
              and then Editor.Buffer_Switcher.Row_At (S.Surface.Buffer_Switcher, 2).Id = C_Id,
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Selected_Row_Index (S.Surface.Buffer_Switcher) = 2
              and then Editor.Buffer_Switcher.Row_At (S.Surface.Buffer_Switcher, 2).Id = C_Id,
              "pruned-next follows current pruned review order");
      Assert (not Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
      ...
      Assert (Editor.Buffer_Switcher.Pending_Marked_Action (S.Surface.Buffer_Switcher) = Editor.Buffer_Switcher.Reviews.No_Pending_Marked_Action
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 0
              and then not Editor.Buffer_Switcher.Has_Pruned_Pending_Marked_Review (S.Surface.Buffer_Switcher),
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Row_At
                (S.Surface.Buffer_Switcher, Editor.Buffer_Switcher.Selected_Row_Index (S.Surface.Buffer_Switcher)).Id = B_Id,
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Row_At
                (S.Surface.Buffer_Switcher, Editor.Buffer_Switcher.Selected_Row_Index (S.Surface.Buffer_Switcher)).Id = C_Id,
              "dirty-previous selects the previous dirty pending target in switcher order");
      Assert (Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Row_At
                (S.Surface.Buffer_Switcher,
                 Editor.Buffer_Switcher.Selected_Row_Index (S.Surface.Buffer_Switcher)).Id = B_Id,
      ...
      Assert (Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 2
              and then not Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 1,
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then not Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Pending_Marked_Open_Dirty_Count
                (S.Surface.Buffer_Switcher, Editor.Buffers.Global_Registry_For_UI) = 1,
      ...
      Cmd := Editor.Commands.Payloads.Command_For_Id
        (Editor.Command_Ids.Command_Buffer_Switcher_Pending_Mark_Dirty_Remove_Selected);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (not Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id),
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 0
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 0,
      ...
      Assert (Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, A_Id)
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 0
              and then Editor.Buffers.Global_Contains (C_Id)
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 2
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 0
      ...
      Assert (Latest_Message_Text (S) = "Dirty prune cancelled"
              and then not Editor.Buffer_Switcher.Has_Dirty_Pending_Marked_Close_Prune (S.Surface.Buffer_Switcher)
              and then Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 3
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 0,
      ...
      Assert (Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, A_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then not Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then not Editor.Buffer_Switcher.Has_Dirty_Pending_Marked_Close_Prune (S.Surface.Buffer_Switcher),
      ...
      Cmd := Editor.Commands.Payloads.Command_For_Id (Editor.Command_Ids.Command_Buffer_Switcher_Pending_Mark_Restore_Last_Pruned);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Pending_Marked_Open_Dirty_Count
                (S.Surface.Buffer_Switcher, Editor.Buffers.Global_Registry_For_UI) = 2,
   ...
   begin
      ...
      Assert (Latest_Message_Text (S) = "Dirty-prune preview review shown"
              and then Editor.Buffer_Switcher.Has_Dirty_Prune_Review (S.Surface.Buffer_Switcher)
      ...
      Row := Editor.Buffer_Switcher.Row_At (S.Surface.Buffer_Switcher, 1);
      Assert (Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, Row.Id),
      ...
      Editor.Buffer_Switcher.Set_Filter_Text (S.Surface.Buffer_Switcher, "beta");
      Editor.Buffer_Switcher.Recompute_Rows
      ...
      Assert (Latest_Message_Text (S) = "Dirty-prune preview review hidden"
              and then not Editor.Buffer_Switcher.Has_Dirty_Prune_Review (S.Surface.Buffer_Switcher)
              and then Editor.Buffer_Switcher.Filter_Text (S.Surface.Buffer_Switcher) = "beta",
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Latest_Message_Text (S) = "Dirty prune preview targets: 2; 1 still applicable"
              and then Editor.Buffer_Switcher.Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 2
              and then Editor.Buffer_Switcher.Row_Count (S.Surface.Buffer_Switcher) = 1,
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (not Editor.Buffer_Switcher.Has_Dirty_Prune_Review (S.Surface.Buffer_Switcher)
              and then not Editor.Buffer_Switcher.Has_Dirty_Pending_Marked_Close_Prune (S.Surface.Buffer_Switcher)
              and then Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id),
   ...
   begin
      ...
      Row := Editor.Buffer_Switcher.Selected_Row (S.Surface.Buffer_Switcher, Found);
      Assert (Found and then Row.Id = B_Id
              and then Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, B_Id),
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then not Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Pending_Marked_Target_Count (S.Surface.Buffer_Switcher) = 3
              and then Editor.Buffer_Switcher.Pending_Marked_Open_Dirty_Count
      ...
      Assert (Editor.Buffer_Switcher.Has_Dirty_Prune_Review (S.Surface.Buffer_Switcher)
              and then Editor.Buffer_Switcher.Row_Count (S.Surface.Buffer_Switcher) = 1
              and then Editor.Buffer_Switcher.Preview_Target (S.Surface.Buffer_Switcher) = C_Id,
              "review rows and preview target normalize to the remaining captured target");

      Cmd := Editor.Commands.Payloads.Command_For_Id
        (Editor.Command_Ids.Command_Buffer_Switcher_Pending_Mark_Dirty_Prune_Apply);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (not Editor.Buffer_Switcher.Has_Dirty_Pending_Marked_Close_Prune (S.Surface.Buffer_Switcher)
              and then Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then not Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then not Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id),
      ...
      Cmd := Editor.Commands.Payloads.Command_For_Id
        (Editor.Command_Ids.Command_Buffer_Switcher_Pending_Mark_Dirty_Prune_Preview);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, B_Id)
      ...
      Assert (not Editor.Buffer_Switcher.Has_Dirty_Pending_Marked_Close_Prune (S.Surface.Buffer_Switcher)
              and then not Editor.Buffer_Switcher.Has_Dirty_Prune_Review (S.Surface.Buffer_Switcher)
              and then not Editor.Buffer_Switcher.Has_Removed_Dirty_Pending_Marked_Close_Prune_Targets (S.Surface.Buffer_Switcher),
   ...
   begin
      ...
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then Editor.Buffer_Switcher.Has_Removed_Dirty_Pending_Marked_Close_Prune_Targets (S.Surface.Buffer_Switcher)
              and then Editor.Buffer_Switcher.Removed_Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then Editor.Buffer_Switcher.Last_Removed_Dirty_Pending_Marked_Close_Prune_Target_Name (S.Surface.Buffer_Switcher) = "beta.adb"
              and then not Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, C_Id),
      ...
      Assert (not Editor.Buffer_Switcher.Has_Dirty_Pending_Marked_Close_Prune (S.Surface.Buffer_Switcher)
              ...
              and then not Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then not Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Last_Pruned_Pending_Marked_Close_Target_Name (S.Surface.Buffer_Switcher) = "gamma.adb",
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Removed_Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 2
      ...
      Editor.Buffer_Switcher.Hide_Dirty_Prune_Review (S.Surface.Buffer_Switcher);
      Editor.Buffer_Switcher.Recompute_Rows
        (S.Surface.Buffer_Switcher, Editor.Buffers.Global_Registry_For_UI, (others => <>));
      Editor.Buffer_Switcher.Select_Buffer_Or_Row (S.Surface.Buffer_Switcher, B_Id, 1);
      Recent_Before := Editor.Recent_Buffers.Count (S.Navigation.Recent_Buffers);
      ...
      Assert (Latest_Message_Text (S) = "Selected previous removed dirty-prune preview target"
              ...
              and then not Editor.Buffer_Switcher.Row_Is_Dirty_Prune_Target (S.Surface.Buffer_Switcher, A_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, A_Id),
              "removed-previous selects the previous still-open removed target without restoring it");

      Cmd := Editor.Commands.Payloads.Command_For_Id
        (Editor.Command_Ids.Command_Buffer_Switcher_Pending_Mark_Dirty_Prune_Apply);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (not Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, A_Id)
              and then Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then not Editor.Buffer_Switcher.Is_Pruned_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then not Editor.Buffer_Switcher.Has_Removed_Dirty_Pending_Marked_Close_Prune_Targets (S.Surface.Buffer_Switcher),
   ...
   begin
      ...
      Assert (Editor.Buffer_Switcher.Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 1
              and then Editor.Buffer_Switcher.Applicable_Dirty_Pending_Marked_Close_Prune_Target_Count
                (S.Surface.Buffer_Switcher, Editor.Buffers.Global_Registry_For_UI) = 1
              and then Editor.Buffer_Switcher.Is_Dirty_Pending_Marked_Close_Prune_Target (S.Surface.Buffer_Switcher, C_Id)
              and then not Editor.Buffer_Switcher.Is_Dirty_Pending_Marked_Close_Prune_Target (S.Surface.Buffer_Switcher, B_Id),
              "clear-stale leaves only still-applicable preview targets");
      Assert (Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, B_Id)
              and then Editor.Buffer_Switcher.Is_Pending_Marked_Close_Target (S.Surface.Buffer_Switcher, C_Id)
              and then Editor.Buffer_Switcher.Pruned_Pending_Marked_Close_Target_Count (S.Surface.Buffer_Switcher) = 0
              and then Editor.Buffer_Switcher.Removed_Dirty_Pending_Marked_Close_Prune_Target_Count (S.Surface.Buffer_Switcher) = 0,
      ...
      Cmd := Editor.Commands.Payloads.Command_For_Id
        (Editor.Command_Ids.Command_Buffer_Switcher_Pending_Mark_Dirty_Prune_Preview);
      Editor.Executor.Execute_No_Log (S, Cmd);
      Assert (Editor.Buffer_Switcher.Is_Dirty_Pending_Marked_Close_Prune_Target (S.Surface.Buffer_Switcher, B_Id),
