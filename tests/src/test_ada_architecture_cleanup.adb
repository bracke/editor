...
with Editor.Ada_Architecture_Cleanup;

package body Test_Ada_Architecture_Cleanup is
   package Cleanup renames Editor.Ada_Architecture_Cleanup;
   ...
   begin
      return AUnit.Format ("Test_Ada_Architecture_Cleanup");
   end Name;
   ...
   begin
      ...
      Row.Canonical_Owner := To_Unbounded_String ("Editor..Final_Architecture");
      Row.Final_Intent :=
        To_Unbounded_String
          ("canonical production surface, quarantined historical scaffold, "
           & "release document, or registered AUnit test after closure");
      Row.Quarantine_Reason := To_Unbounded_String ("not quarantined");
      Row.Blocker_Family := To_Unbounded_String ("Architecture.Cleanup");
   ...
   begin
      Assert (Feed_Item.Status = Status, "unexpected cleanup status");
      Assert (Feed_Item.Result_Class = Result_Class,
              "unexpected cleanup class");
   ...
   begin
      ...
      Cleanup.Add_Row
        (Input,
         Base_Row (3, Cleanup.Surface_Project_Index,
                   "Editor.Ada_Project_Index",
                   "src/core/editor-ada_project_index.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (4, Cleanup.Surface_Diagnostic_Consumer,
                   "Editor.Ada_Diagnostic_Service",
                   "src/core/editor-ada_diagnostic_service.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (5, Cleanup.Surface_Command_Surface,
                   "Editor.Commands",
                   "src/core/editor-commands.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (6, Cleanup.Surface_Render_Surface,
                   "Editor.Render_Model",
                   "src/core/editor-render_model.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (7, Cleanup.Surface_Workspace_Surface,
      ...
      Row := Base_Row
        (9, Cleanup.Surface_Release_Document,
      ...
      Row := Base_Row
        (10, Cleanup.Surface_Test_Harness,
         "Test_Ada_Architecture_Cleanup",
         "tests/src/test_ada_architecture_cleanup.ads");
      Row.Is_Production_Surface := False;
      ...
      Assert (Cleanup.Final_Cleanup_Achieved (Model),
              "architecture cleanup should be closed");
      Assert (Model.Canonical_Count = 7, "canonical architecture surfaces");
      ...
      Assert (Model.Required_Surface_Count = 10,
              "all architecture cleanup surfaces tracked");
      Assert (Model.Missing_Surface_Count = 0,
              "no architecture cleanup surface missing");
      Assert (Model.Duplicate_Surface_Count = 0,
              "no duplicate architecture cleanup surface expected");

      Expect_Status
        (Model, 1, Cleanup.Status_Canonical_Production_Surface,
         Cleanup.Class_Accepted);
      Expect_Status
        (Model, 8, Cleanup.Status_Quarantined_Historical_Scaffold,
         Cleanup.Class_Quarantined);
      Expect_Status
        (Model, 10, Cleanup.Status_Test_Harness_Covered,
         Cleanup.Class_Accepted);
   ...
   begin
      Cleanup.Add_Row
        (Input,
         Base_Row (101, Cleanup.Surface_Semantic_Engine,
                   "Editor.Ada_Language_Model",
                   "src/core/editor-ada_language_model.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (102, Cleanup.Surface_Syntax_Parser,
                   "Editor.Ada_Declaration_Parser",
                   "src/core/editor-ada_declaration_parser.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (103, Cleanup.Surface_Project_Index,
                   "Editor.Ada_Project_Index",
                   "src/core/editor-ada_project_index.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (104, Cleanup.Surface_Diagnostic_Consumer,
                   "Editor.Ada_Diagnostic_Service",
                   "src/core/editor-ada_diagnostic_service.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (105, Cleanup.Surface_Command_Surface,
                   "Editor.Commands",
                   "src/core/editor-commands.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (106, Cleanup.Surface_Render_Surface,
                   "Editor.Render_Model",
                   "src/core/editor-render_model.ads"));
      Cleanup.Add_Row
        (Input,
         Base_Row (107, Cleanup.Surface_Render_Surface,
                   "Editor.Render_Model.Duplicate",
                   "src/core/editor-render_model.ads"));
      Cleanup.Add_Row
        (Input,
         Historical_Row
      ...
      Row := Base_Row
        (109, Cleanup.Surface_Release_Document,
         "docs.release.ARCHITECTURE_CLEANUP_PASS1429",
         "docs/release/ARCHITECTURE_CLEANUP_PASS1429.md");
      Row.Is_Production_Surface := False;
      Row.Is_Release_Document := True;
      ...
      Row := Base_Row
        (110, Cleanup.Surface_Test_Harness,
         "Test_Ada_Architecture_Cleanup",
         "tests/src/test_ada_architecture_cleanup.ads");
      Row.Is_Production_Surface := False;
      Row.Is_Test_Surface := True;
      ...
      Assert (not Cleanup.Final_Cleanup_Achieved (Model),
              "duplicate and missing cleanup surfaces must block completion");
      Assert (Model.Required_Surface_Count = 10,
              "required architecture cleanup surface count remains stable");
      Assert (Model.Missing_Surface_Count = 1,
              "missing workspace surface detected");
      Assert (Model.Duplicate_Surface_Count = 1,
              "duplicate render surface detected");
      Expect_Status
        (Model, 107, Cleanup.Status_Rejected_Duplicate_Surface,
   ...
   begin
      ...
      Row := Base_Row
        (21, Cleanup.Surface_Semantic_Engine,
         "Editor.Ada_RM_Remaining_Gap_Remediation",
         "src/core/editor-ada_rm_remaining_gap_remediation.ads");
   ...
   begin
      Register_Routine
        (T, Test_Final_Architecture_Cleanup'Access,
         "architecture cleanup closes canonical/quarantine surfaces");
      Register_Routine
        (T, Test_Duplicate_And_Missing_Surfaces_Block_Completion'Access,
         "architecture cleanup requires each named surface exactly once");
      Register_Routine
...
end Test_Ada_Architecture_Cleanup;
