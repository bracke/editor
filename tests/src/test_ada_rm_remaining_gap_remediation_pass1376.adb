...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1376;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1376 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1376;
   ...
   use type Audit.Enumeration_Context;
   use type Audit.Enumeration_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1376");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Enumeration_Form := Audit.Enum_Compatible)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Enumeration_Attribute_Representation_Edge;
      Row.Family := Matrix.Family_Static_Expressions_Choices;
      Row.Owner := Matrix.Slice_Enumeration_Representation;
      ...
      Row.Context := Audit.Context_Enumeration_Attribute;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/enumeration-attribute-representation.adb");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("enumeration representation and attributes Pos Val Succ Pred Value Image must preserve canonical literal, static choice, runtime-check, and consumer evidence");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1376");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1376");
      Row.Blocker_Family :=
        To_Unbounded_String ("RM.Enumeration.Attribute_Representation_Literal");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1376 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1376 precision classification");
   end Expect_Status;

   procedure Test_Enumeration_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Enum_Compatible));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Enum_Duplicate_Representation_Code);
      Row.Duplicate_Representation_Code := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Enum_Attribute_Value_Runtime_Check);
      Row.Runtime_Value_Check := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (4, Precision.Class_Indeterminate,
                       Audit.Enum_Missing_Literal_Evidence);
      Row.Literal_Evidence_Present := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1376 should close the selected enumeration attribute gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Legal_Enumeration_Attribute_Agreement,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Duplicate_Representation_Code,
                     Precision.Class_Illegal);
      Expect_Status (Results, 3, Audit.Status_Runtime_Value_Check_Preserved,
                     Precision.Class_Legal_With_Runtime_Check);
      Expect_Status (Results, 4, Audit.Status_Indeterminate_Missing_Literal_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Enumeration_Gap_Remediated;

   procedure Test_Enumeration_Static_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Incomplete_Representation_Clause := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (11);
      Row.Non_Static_Representation_Value := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Attribute_Pos_Out_Of_Range := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Attribute_Val_Out_Of_Range := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Attribute_Succ_Pred_Bounds_Error := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (15);
      Row.Ambiguous_Literal := True;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 10, Audit.Status_Illegal_Incomplete_Representation_Clause,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Non_Static_Representation_Value,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Attribute_Pos_Out_Of_Range,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Illegal_Attribute_Val_Out_Of_Range,
                     Precision.Class_Illegal);
      Expect_Status (Results, 14, Audit.Status_Illegal_Attribute_Succ_Pred_Bounds,
                     Precision.Class_Illegal);
      Expect_Status (Results, 15, Audit.Status_Illegal_Ambiguous_Literal,
                     Precision.Class_Illegal);
      Expect_Status (Results, 16, Audit.Status_Illegal_Consumer_Surface_Disagreement,
                     Precision.Class_Illegal);
   end Test_Enumeration_Static_Rejections;

   procedure Test_Runtime_And_Indeterminate_Enumeration
   ...
   begin
      ...
      Row := Base_Row (21, Precision.Class_Indeterminate,
                       Audit.Enum_Private_View_Indeterminate);
      Row.Private_Full_View_Available := False;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (22, Precision.Class_Indeterminate,
                       Audit.Enum_Stale_Representation_Evidence);
      Row.Stale_Representation_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 20, Audit.Status_Runtime_Value_Check_Preserved,
                     Precision.Class_Legal_With_Runtime_Check);
      ...
      Expect_Status (Results, 22, Audit.Status_Indeterminate_Stale_Representation_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Runtime_And_Indeterminate_Enumeration;
   ...
   begin
      Register_Routine
        (T, Test_Enumeration_Gap_Remediated'Access,
         "enumeration attribute gap remediated");
      Register_Routine
        (T, Test_Enumeration_Static_Rejections'Access,
         "enumeration static rejections");
      Register_Routine
        (T, Test_Runtime_And_Indeterminate_Enumeration'Access,
         "runtime and indeterminate enumeration cases");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1376;
