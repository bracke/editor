...
package body Test_Ada_Shared_State_Stabilization_Gate_Legality_Pass1222 is
   ...
   begin
      return AUnit.Format ("Ada shared-state stabilization gate legality pass1222");
   end Name;
   ...
   begin
      ...
      C.Source_Fingerprint := Natural (Id) * 1222;
      C.Expected_Source_Fingerprint := Natural (Id) * 1222;
      return C;
   ...
   begin
      ...
      declare
         ...
         Model      : constant Gate.Shared_State_Stabilization_Gate_Model := Gate.Build (Conv_Model);
         Row        : constant Gate.Shared_State_Stabilization_Gate_Row := Gate.Row_At (Model, 1);
      begin
         Assert (Gate.Count (Model) = 1, "one stabilization-gate row expected");
         Assert (Gate.Promoted_Count (Model) = 1, "stable current evidence should be promoted");
         Assert (Gate.Current_Count (Model) = 1, "promoted current row should remain current");
         Assert (Gate.Withheld_Count (Model) = 0, "promoted current row should not be withheld");
         Assert (Row.Status = Gate.Shared_State_Stabilization_Gate_Promoted_Current,
                 "current convergence rows should cross the stabilization gate");
         Assert (Row.Action = Gate.Shared_State_Stabilization_Gate_Action_Promote_Current,
   ...
   begin
      ...
      declare
         Model : constant Gate.Shared_State_Stabilization_Gate_Model := Gate.Build (Convergence_From (Contexts));
      begin
         Assert (Gate.Count (Model) = 3, "three stabilization-gate rows expected");
         Assert (Gate.Promoted_Count (Model) = 0, "blocked evidence must not be promoted");
         Assert (Gate.Withheld_Count (Model) = 3, "stable blockers should remain withheld");
         Assert (Gate.Recheck_Required_Count (Model) = 0, "stable blockers should not request recheck");
         Assert
           (Gate.Count_By_Status (Model, Gate.Shared_State_Stabilization_Gate_Withheld_Abstract_State) = 1,
            "abstract/refined-state blocker should remain explicit at stabilization gate");
         Assert
           (Gate.Count_By_Status (Model, Gate.Shared_State_Stabilization_Gate_Withheld_Tasking_Protected) = 1,
            "tasking/protected blocker should remain explicit at stabilization gate");
         Assert
           (Gate.Count_By_Status (Model, Gate.Shared_State_Stabilization_Gate_Withheld_View_Barrier) = 1,
            "view-barrier blocker should remain explicit at stabilization gate");
      end;
   ...
   begin
      ...
      declare
         ...
         Model   : constant Gate.Shared_State_Stabilization_Gate_Model := Gate.Build (Changed);
         Row     : constant Gate.Shared_State_Stabilization_Gate_Row := Gate.Row_At (Model, 1);
      begin
         Assert (Gate.Count (Model) = 1, "one changed stabilization-gate row expected");
         Assert (Gate.Promoted_Count (Model) = 0, "changed evidence must not be promoted");
         Assert (Gate.Recheck_Required_Count (Model) = 1, "changed convergence evidence should force recheck");
         Assert (Row.Status = Gate.Shared_State_Stabilization_Gate_Recheck_Required,
                 "changed convergence rows should be held for another bounded recheck");
         Assert (Row.Action = Gate.Shared_State_Stabilization_Gate_Action_Recheck,
   ...
   begin
      ...
      declare
         ...
         Row   : constant Gate.Shared_State_Stabilization_Gate_Row := Gate.Row_At (Model, 1);
      begin
         Assert (Gate.Query_Count (Gate.Find_By_Node (Model, Node)) = 1,
                 "node lookup should preserve shared-state stabilization evidence");
         Assert (Gate.Query_Count (Gate.Find_By_Source_Fingerprint (Model, Row.Source_Fingerprint)) = 1,
                 "source fingerprint lookup should preserve stabilization evidence");
         Assert (Gate.Count_By_Blocker_Family (Model, Row.Blocker_Family) = 1,
                 "blocker-family lookup should preserve the original family");
         Assert (Gate.Stable_Fingerprint (Model) /= 0,
                 "stabilization-gate model should have deterministic non-zero fingerprint");
      end;
   ...
   begin
      ...
      Register_Routine
        (T, Stable_Blockers_Remain_Withheld'Access,
         "stable shared-state blockers remain withheld at stabilization gate");
      Register_Routine
