...
with Editor.Ada_RM_Remediation.Representation.Volatile_Atomic_Representation;

package body Test_Ada_RM_Remediation.Representation.Volatile_Atomic_Representation is
   package Audit renames Editor.Ada_RM_Remediation.Representation.Volatile_Atomic_Representation;
   ...
   use type Audit.Volatile_Atomic_Representation_Closure;
   use type Audit.Volatile_Atomic_Representation_Form;
   ...
   function Base_Row
     (Id : Natural;
      ...
      Form : Audit.Volatile_Atomic_Representation_Form := Audit.Form_Volatile_Atomic_Representation_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Volatile_Atomic_Representation_Clause_Edge;
      Row.Family := Matrix.Family_Representation_Aspects_Freezing;
      Row.Owner := Matrix.Slice_Freezing_Representation;
      ...
      Row.Closure := Audit.Closure_Atomic_Size_Alignment_Evidence;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/volatile-atomic-representation-clause.ads");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("volatile and atomic representation clauses, size/alignment"
           & "evidence, full-access and independent-addressability evidence,"
           & "freezing, runtime representation checks, warning diagnostics,"
           & "stale representation evidence, and semantic consumers must share"
           & "one source-shaped legality result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remediation.Representation.Volatile_Atomic_Representation");
      Row.Candidate_Case := To_Unbounded_String ("Case 1420");
      Row.Blocker_Family := To_Unbounded_String ("RM.Representation.Volatile_Atomic.Clause");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected case 1420 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected case 1420 precision classification");
   end Expect_Status;
   ...
   begin
      Audit.Add_Row
        (Input,
         Base_Row (1, Precision.Class_Legal,
                   Audit.Form_Volatile_Atomic_Representation_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Atomic_Size_Too_Small);
      Row.Atomic_Size_Too_Small := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Representation_Check_Preserved);
      Row.Runtime_Representation_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (5, Precision.Class_Indeterminate,
                       Audit.Form_Indeterminate_Missing_Representation_Evidence);
      Row.Missing_Representation_Evidence := True;
      Row.Complete_Representation_Evidence := False;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (6, Precision.Class_Legal,
                       Audit.Form_Atomic_Size_Alignment_Resolved);
      Row.Closure := Audit.Closure_Volatile_Representation_Evidence;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "case 1420 should close the volatile atomic representation clause gap");
      ...
      Expect_Status
        (Results, 1,
         Audit.Status_Volatile_Atomic_Representation_Resolved,
         Precision.Class_Legal);
      Expect_Status
        (Results, 2,
         Audit.Status_Illegal_Atomic_Size_Too_Small,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 3,
         Audit.Status_Runtime_Representation_Check_Preserved,
      ...
      Expect_Status
        (Results, 5,
         Audit.Status_Indeterminate_Missing_Representation_Evidence,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 6,
         Audit.Status_Atomic_Size_Alignment_Resolved,
         Precision.Class_Legal);
   ...
   begin
      ...
      Row := Base_Row (11);
      Row.Atomic_Alignment_Conflict := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Volatile_Full_Access_Conflict := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Late_Representation_After_Freezing := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Independent_Addressability_Conflict := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (17);
      Row.Missing_Freezing_Evidence := True;
      Row.Complete_Freezing_Evidence := False;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 11,
         Audit.Status_Illegal_Atomic_Alignment_Conflict,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 12,
         Audit.Status_Illegal_Volatile_Full_Access_Conflict,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 13,
         Audit.Status_Illegal_Late_Representation_After_Freezing,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 14,
         Audit.Status_Illegal_Independent_Addressability_Conflict,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 15,
         Audit.Status_Indeterminate_Private_Object_View,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 16,
         Audit.Status_Indeterminate_Stale_Representation_Evidence,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 17,
         Audit.Status_Indeterminate_Missing_Freezing_Evidence,
         Precision.Class_Indeterminate);
   end Test_Rejections;
   ...
   begin
      ...
      Row := Base_Row (27);
      Row.Freezing_Fingerprint := 1;
      Row.Expected_Freezing_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 27,
         Audit.Status_Freezing_Fingerprint_Mismatch,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 28,
   ...
   begin
      Register_Routine
        (T, Test_Gap_Remediated'Access,
         "volatile atomic representation clause gap remediated");
      Register_Routine
        (T, Test_Rejections'Access,
         "volatile atomic representation clause rejection cases");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remediation.Representation.Volatile_Atomic_Representation;
