...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1403;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1403 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1403;
   ...
   use type Audit.Formal_Derived_Interface_Closure;
   use type Audit.Formal_Derived_Interface_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1403");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Formal_Derived_Interface_Form := Audit.Form_Formal_Derived_Interface_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Formal_Derived_Interface_Private_View_Edge;
      Row.Family := Matrix.Family_Generics_Contracts_Substitution_Replay;
      Row.Owner := Matrix.Slice_Generic_Formal_Type_Family;
      ...
      Row.Closure := Audit.Closure_Formal_Derived_Interface_Private_View;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/formal-derived-interface-private-view.ads");
      ...
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1403");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1403");
      Row.Blocker_Family :=
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1403 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1403 precision classification");
   end Expect_Status;

   procedure Test_Formal_Derived_Interface_Private_View_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Formal_Derived_Interface_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Formal_Derived_Parent_Mismatch);
      Row.Formal_Derived_Parent_Mismatch := True;
      ...
      Row.Complete_Interface_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1403 should close the formal-derived/interface/private-view gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Formal_Derived_Interface_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Formal_Derived_Parent_Mismatch,
   ...
   end Test_Formal_Derived_Interface_Private_View_Gap_Remediated;

   procedure Test_Formal_Derived_Interface_Private_View_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Interface_Primitive_Missing := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (12);
      Row.Private_View_Leak := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Dispatching_Profile_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Missing_Interface_Evidence := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (15);
      Row.Stale_Interface_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Interface_Primitive_Missing,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Abstract_Primitive_Unimplemented,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Private_View_Leak,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Illegal_Dispatching_Profile_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 15, Audit.Status_Indeterminate_Stale_Interface_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Formal_Derived_Interface_Private_View_Rejections;
   ...
   begin
      ...
      Row := Base_Row (26);
      Row.Interface_Fingerprint := 1;
      Row.Expected_Interface_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 25, Audit.Status_Semantic_Result_Unconsumed,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Interface_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Effect_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Formal_Derived_Interface_Private_View_Gap_Remediated'Access,
         "formal derived interface private-view gap remediated");
      Register_Routine
        (T, Test_Formal_Derived_Interface_Private_View_Rejections'Access,
         "missing interface primitive, abstract primitive, private-view leak, dispatching profile mismatch, missing evidence, and stale evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1403;
