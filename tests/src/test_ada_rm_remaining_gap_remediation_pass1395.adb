...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1395;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1395 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1395;
   ...
   use type Audit.Declare_Closure;
   use type Audit.Declare_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1395");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Declare_Form := Audit.Form_Declare_Lifetime_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Declare_Expression_Object_Lifetime_Edge;
      ...
      Row.Closure := Audit.Closure_Declare_Expression;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/declare-expression-object-lifetime.ads");
      ...
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1395");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1395");
      Row.Blocker_Family :=
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1395 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1395 precision classification");
   end Expect_Status;

   procedure Test_Declare_Object_Lifetime_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Declare_Lifetime_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Result_Type_Mismatch);
      Row.Result_Type_Mismatch := True;
      ...
      Row.Complete_Declare_Evidence := False;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1395 should close the declare expression object lifetime gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Declare_Lifetime_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Result_Type_Mismatch,
   ...
   end Test_Declare_Object_Lifetime_Gap_Remediated;

   procedure Test_Declare_Object_Lifetime_Rejections
   ...
   begin
      ...
      Row := Base_Row (12);
      Row.Accessibility_Escape := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Missing_Declare_Evidence := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Stale_Declare_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 10, Audit.Status_Illegal_Missing_Initializer,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Result_Type_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Accessibility_Escape,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Stale_Declare_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Declare_Object_Lifetime_Rejections;
   ...
   begin
      ...
      Row := Base_Row (26);
      Row.Declare_Fingerprint := 1;
      Row.Expected_Declare_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 25, Audit.Status_Semantic_Result_Unconsumed,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 26, Audit.Status_Declare_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Lifetime_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Declare_Object_Lifetime_Gap_Remediated'Access,
         "declare expression object lifetime gap remediated");
      Register_Routine
        (T, Test_Declare_Object_Lifetime_Rejections'Access,
         "missing-initializer declare object, result type mismatch, lifetime profile mismatch, missing declare, and stale declare evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1395;
