...
package body Test_Ada_Legality.Expressions.RM_Coverage_Review_Application_Legality is
   ...
   package Prov renames Editor.Ada_Legality.Expressions.RM_Closure_Closure_Stable_Diagnostic_Provenance;
   ...
   begin
      return AUnit.Format ("Ada remaining RM edge recheck application legality");
   end Name;
   ...
   procedure Eligibility_Is_Applied_To_Remaining_Edge_Boundary
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Rechecks : constant EL.Remaining_RM_Edge_Recheck_Model := Build_Rechecks;
      Apps : constant AP.Remaining_RM_Edge_Application_Model := AP.Build (Rechecks);
   begin
      Assert (AP.Row_Count (Apps) = EL.Row_Count (Rechecks),
              "application should preserve remaining-edge recheck row count");
      Assert (AP.Current_Count (Apps) = EL.Current_Evidence_Count (Rechecks),
              "current non-diagnostic remaining-edge evidence should stay current");
      Assert (AP.Withheld_Count (Apps) = EL.Blocked_Count (Rechecks),
              "blocked recheck rows should remain withheld at application boundary");
      Assert (AP.Stable_Fingerprint (Apps) /= 0,
              "application fingerprint should be deterministic");
   end Eligibility_Is_Applied_To_Remaining_Edge_Boundary;

   procedure Preserves_Remaining_Edge_And_Closure_Withholding
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Apps : constant AP.Remaining_RM_Edge_Application_Model := Build_Applications;
   begin
      Assert (AP.Count_Status (Apps, AP.Remaining_RM_Edge_Application_Current_Non_Diagnostic_Evidence) = 1,
              "accepted remaining-edge evidence should remain current and non-diagnostic");
      Assert (AP.Remaining_Edge_Withheld_Count (Apps) = 1,
              "remaining-edge blockers should be withheld distinctly");
      Assert (AP.Stabilized_Closure_Withheld_Count (Apps) = 2,
              "stabilized closure blockers should be withheld distinctly");
      Assert (AP.Count_Action (Apps, AP.Remaining_RM_Edge_Application_Action_Withhold_For_Remaining_Edge) = 1,
              "remaining-edge blocker action should remain explicit");
      Assert (AP.Count_Action (Apps, AP.Remaining_RM_Edge_Application_Action_Withhold_For_Stabilized_Closure) = 2,
              "stabilized closure blocker action should remain explicit");
   end Preserves_Remaining_Edge_And_Closure_Withholding;

   procedure Preserves_Fingerprint_And_Family_Blockers
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Apps : constant AP.Remaining_RM_Edge_Application_Model := Build_Applications;
   begin
      Assert (AP.Count_Family (Apps, D.Remaining_RM_Edge_Stabilized_Diagnostic_Remaining_Edge) = 1,
              "remaining-edge diagnostic family should survive application");
      Assert (AP.Count_Family (Apps, D.Remaining_RM_Edge_Stabilized_Diagnostic_Stabilized_Closure) = 2,
              "stabilized-closure diagnostic family should survive application");
      Assert (AP.Fingerprint_Withheld_Count (Apps) = 1,
              "source or substitution fingerprint blockers should remain separate");
      Assert (AP.Count_Action (Apps, AP.Remaining_RM_Edge_Application_Action_Withhold_For_Source_Fingerprint) = 1,
              "source fingerprint mismatch should stay withheld");
   end Preserves_Fingerprint_And_Family_Blockers;

   procedure Query_Surface_Finds_Applied_Rows
     (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      Apps : constant AP.Remaining_RM_Edge_Application_Model := Build_Applications;
      Row : constant AP.Remaining_RM_Edge_Application_Row := AP.Row_At (Apps, 3);
   begin
      Assert (AP.Query_Count (AP.Query_Node (Apps, Row.Node)) >= 1,
              "node query should find remaining-edge application rows");
      Assert (AP.Query_Count (AP.Query_Source_Fingerprint (Apps, Row.Source_Fingerprint)) >= 1,
              "source fingerprint query should find remaining-edge application rows");
      Assert (AP.Blocks_Downstream (Row),
              "withheld remaining-edge application row should block downstream trust");
   end Query_Surface_Finds_Applied_Rows;

   overriding procedure Register_Tests (T : in out Test_Case) is
      use AUnit.Test_Cases.Registration;
   begin
      Register_Routine
        (T, Eligibility_Is_Applied_To_Remaining_Edge_Boundary'Access,
         "eligibility is applied to remaining-edge boundary");
      Register_Routine
        (T, Preserves_Remaining_Edge_And_Closure_Withholding'Access,
         "preserves remaining-edge and closure withholding");
      Register_Routine
        (T, Preserves_Fingerprint_And_Family_Blockers'Access,
         "preserves fingerprint and family blockers");
      Register_Routine
        (T, Query_Surface_Finds_Applied_Rows'Access,
         "query surface finds applied rows");
   end Register_Tests;

end Test_Ada_Legality.Expressions.RM_Coverage_Review_Application_Legality;
