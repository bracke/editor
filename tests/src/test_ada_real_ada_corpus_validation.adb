...
with Editor.Ada_Real_Ada_Corpus_Validation;

package body Test_Ada_Real_Ada_Corpus_Validation is
   package Corpus renames Editor.Ada_Real_Ada_Corpus_Validation;
   ...
   begin
      return AUnit.Format
        ("Test_Ada_Real_Ada_Corpus_Validation");
   end Name;
   ...
   begin
      Assert (Feed_Item.Status = Status, "unexpected corpus status");
      Assert (Feed_Item.Result_Class = Result_Class,
              "unexpected corpus class");
   ...
   begin
      ...
      Row := Base_Row
        (6, Corpus.Family_Generic_Instance, Corpus.Expect_Accepted,
         "generic instance body replay agrees with instantiated source",
         "package I is new G (T => Integer);");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (7, Corpus.Family_Tasking_Protected, Corpus.Expect_Rejected,
         "protected requeue loop is rejected with precise diagnostic",
         "protected body P is entry E when Flag is begin requeue E; end; end P;");
      Row.Actual_Rejected := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (8, Corpus.Family_Representation_Freezing, Corpus.Expect_Accepted,
         "representation clause freezing remains accepted",
         "for R use record X at 0 range 0 .. 7; end record;");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (9, Corpus.Family_Incremental_Edit, Corpus.Expect_Accepted,
         "incremental edit refresh owns the latest semantic snapshot",
         "package body P is X : Integer := 2; end P;");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);
      ...
      Assert (Corpus.Corpus_Validation_Achieved (Model),
              "real Ada corpus validation should be closed");
      Assert (Model.Legal_Accepted_Count = 4, "legal corpus acceptance count");
      Assert (Model.Illegal_Rejected_Count = 2, "illegal corpus rejection count");
      ...
      Assert (Model.Required_Family_Count = 9,
              "all required corpus families are known");
      Assert (Model.Missing_Family_Count = 0,
              "no corpus family is missing");
      Assert (Model.Duplicate_Family_Count = 0,
              "no corpus family is duplicated");
      ...
      Expect_Status
        (Model, 6, Corpus.Status_Accepted_Legal_Corpus,
         Corpus.Class_Validated);
      Expect_Status
        (Model, 7, Corpus.Status_Rejected_Illegal_Corpus,
         Corpus.Class_Validated);
      Expect_Status
        (Model, 8, Corpus.Status_Accepted_Legal_Corpus,
         Corpus.Class_Validated);
      Expect_Status
        (Model, 9, Corpus.Status_Accepted_Legal_Corpus,
         Corpus.Class_Validated);
   ...
   procedure Test_Duplicate_And_Missing_Corpus_Families_Block_Closure
   ...
   begin
      Row := Base_Row
        (30, Corpus.Family_Legal_Ada_2022_Unit, Corpus.Expect_Accepted,
         "legal unit", "package P is end P;");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (31, Corpus.Family_Legal_Ada_2022_Unit, Corpus.Expect_Accepted,
         "duplicate legal unit cannot substitute for incremental edit",
         "package Q is end Q;");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (32, Corpus.Family_Illegal_Ada_2022_Unit, Corpus.Expect_Rejected,
         "illegal unit", "X : Integer := ;");
      Row.Actual_Rejected := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (33, Corpus.Family_Runtime_Check_Preserved, Corpus.Expect_Runtime_Check,
         "runtime check", "A (I) := B;");
      Row.Actual_Accepted := True;
      Row.Runtime_Check_Preserved := True;
      Corpus.Add_Row (Input, Row);
      ...
      Row := Base_Row
        (34, Corpus.Family_Warning_Only_Preserved, Corpus.Expect_Warning_Only,
         "warning only", "pragma Warnings (Off);");
      Row.Actual_Accepted := True;
      Row.Warning_Only_Preserved := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (35, Corpus.Family_Cross_Unit_Project, Corpus.Expect_Accepted,
         "cross unit", "with P; package body C is end C;");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (36, Corpus.Family_Generic_Instance, Corpus.Expect_Accepted,
         "generic instance", "package I is new G (Integer);");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (37, Corpus.Family_Tasking_Protected, Corpus.Expect_Rejected,
         "tasking protected", "protected body P is end P;");
      Row.Actual_Rejected := True;
      Corpus.Add_Row (Input, Row);

      Row := Base_Row
        (38, Corpus.Family_Representation_Freezing, Corpus.Expect_Accepted,
         "representation freezing", "for R'Size use 32;");
      Row.Actual_Accepted := True;
      Corpus.Add_Row (Input, Row);

      Model := Corpus.Build (Input);

      Assert (not Corpus.Corpus_Validation_Achieved (Model),
              "corpus validation must require each family exactly once");
      Assert (Model.Total_Rows = 9,
              "regression keeps the accepted broad-verdict row-count shape");
      Assert (Model.Required_Family_Count = 9,
              "all required corpus families are known to the model");
      Assert (Model.Duplicate_Family_Count = 1,
              "duplicate corpus families are counted");
      Assert (Model.Missing_Family_Count = 1,
              "omitted corpus families are counted");
      Expect_Status (Model, 31, Corpus.Status_Rejected_Duplicate_Family,
                     Corpus.Class_Rejected);
   end Test_Duplicate_And_Missing_Corpus_Families_Block_Closure;
   ...
   begin
      ...
      Register_Routine
        (T, Test_Duplicate_And_Missing_Corpus_Families_Block_Closure'Access,
         "real Ada corpus validation rejects duplicate or missing families");
   end Register_Tests;
