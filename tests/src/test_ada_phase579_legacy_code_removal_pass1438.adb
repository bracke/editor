...
with Editor.Ada_Phase579_Legacy_Code_Removal_Pass1438;

package body Test_Ada_Phase579_Legacy_Code_Removal_Pass1438 is
   package Removal renames Editor.Ada_Phase579_Legacy_Code_Removal_Pass1438;
   ...
   begin
      return AUnit.Format ("Test_Ada_Phase579_Legacy_Code_Removal_Pass1438");
   end Name;
   ...
   begin
      ...
      Row.Removal_Reason :=
        To_Unbounded_String
          ("diagnostic recovery render final status scaffold was superseded by "
           & "pass1428 remaining-gap closure and pass1429 architecture cleanup");
      Row.Blocker_Family :=
        To_Unbounded_String ("Phase579.LegacyCodeRemoval.Pass1438");
   ...
   begin
      Assert (Feed_Item.Status = Status, "unexpected pass1438 removal status");
      Assert (Feed_Item.Result_Class = Result_Class,
              "unexpected pass1438 removal class");
      Assert (Removal.Class_For_Status (Status) = Result_Class,
              "removal status mapping drifted");
   end Expect_Status;

   procedure Test_Removed_Diagnostic_Recovery_Final_Status
   ...
   begin
        ...
        (Input,
         Base_Row
           (6,
            ...
            "docs.release.LEGACY_CODE_REMOVAL_PASS1438"));

      Model := Removal.Build (Input);

      Assert (Removal.Legacy_Code_Removal_Achieved (Model),
              "pass1438 legacy code removal should be closed");
      ...
      Expect_Status
        (Model, 6, Removal.Status_Documented_Removal,
         Removal.Class_Accepted);
   end Test_Removed_Diagnostic_Recovery_Final_Status;
   ...
   begin
      Register_Routine
        (T, Test_Removed_Diagnostic_Recovery_Final_Status'Access,
         "removed diagnostic recovery render final status scaffold");
      Register_Routine
...
end Test_Ada_Phase579_Legacy_Code_Removal_Pass1438;
