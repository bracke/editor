...
package body Test_Ada_RM_Remediation.Finalization.Exception_Finalization is
   ...
   begin
      Assert (Item.Status = Status, "unexpected case 1378 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected case 1378 precision classification");
   end Expect_Status;
   ...
   begin
      ...
      Assert (Audit.Gap_Remediated (Results),
              "case 1378 should close the selected exception finalization gap");
...
end Test_Ada_RM_Remediation.Finalization.Exception_Finalization;
