...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1401;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1401 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1401;
   ...
   use type Audit.Null_Record_Aggregate_Closure;
   use type Audit.Null_Record_Aggregate_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1401");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Null_Record_Aggregate_Form := Audit.Form_Null_Record_Aggregate_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Null_Record_Aggregate_Box_Default_Edge;
      ...
      Row.Closure := Audit.Closure_Null_Record_Aggregate;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/aggregate-box-default.ads");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("null record aggregates, box defaults, discriminants, component defaults, runtime default checks, and diagnostic consumers must share one canonical aggregate/box/default result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1401");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1401");
      Row.Blocker_Family :=
        To_Unbounded_String ("RM.Aggregates.Null_Record_Box_Default");
      return Row;
   end Base_Row;
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1401 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1401 precision classification");
   end Expect_Status;

   procedure Test_Null_Record_Aggregate_Box_Default_Gap_Remediated
   ...
   begin
      Audit.Add_Row (Input, Base_Row (1, Precision.Class_Legal,
                                      Audit.Form_Null_Record_Aggregate_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Null_Aggregate_Not_Allowed);
      Row.Null_Aggregate_Not_Allowed := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Default_Check_Preserved);
      Row.Runtime_Default_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Assert (Audit.Gap_Remediated (Results),
              "pass1401 should close the aggregate/box/default gap");
      ...
      Expect_Status (Results, 1, Audit.Status_Null_Record_Aggregate_Resolved,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Null_Aggregate_Not_Allowed,
                     Precision.Class_Illegal);
      Expect_Status (Results, 3, Audit.Status_Runtime_Default_Check_Preserved,
   ...
   end Test_Null_Record_Aggregate_Box_Default_Gap_Remediated;

   procedure Test_Null_Record_Aggregate_Box_Default_Rejections
   ...
   begin
      ...
      Row := Base_Row (11);
      Row.Box_Default_Missing := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Discriminant_Default_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Limited_Component_Default_Missing := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Missing_Component_Default_Evidence := True;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 10, Audit.Status_Illegal_Null_Aggregate_Not_Allowed,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Box_Default_Missing,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Discriminant_Default_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Illegal_Limited_Component_Default_Missing,
                     Precision.Class_Illegal);
      Expect_Status (Results, 14, Audit.Status_Indeterminate_Private_View,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 15, Audit.Status_Indeterminate_Stale_Aggregate_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Null_Record_Aggregate_Box_Default_Rejections;
   ...
   begin
      ...
      Row := Base_Row (27);
      Row.Default_Fingerprint := 1;
      Row.Expected_Default_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 26, Audit.Status_Aggregate_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 27, Audit.Status_Default_Fingerprint_Mismatch,
                     Precision.Class_Indeterminate);
   ...
   begin
      Register_Routine
        (T, Test_Null_Record_Aggregate_Box_Default_Gap_Remediated'Access,
         "aggregate box default gap remediated");
      Register_Routine
        (T, Test_Null_Record_Aggregate_Box_Default_Rejections'Access,
         "null aggregate disallowed, missing box default, discriminant default mismatch, limited component default missing, missing evidence, and stale evidence rejections");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1401;
