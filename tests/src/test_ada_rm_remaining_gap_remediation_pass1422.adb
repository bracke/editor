...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1422;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1422 is
   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1422;
   ...
   use type Audit.Generic_Formal_Subprogram_Call_Closure;
   use type Audit.Generic_Formal_Subprogram_Call_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1422");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Generic_Formal_Subprogram_Call_Form := Audit.Form_Generic_Formal_Subprogram_Call_Resolved)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Generic_Formal_Subprogram_Call_Edge;
      ...
      Row.Closure := Audit.Closure_Formal_Subprogram_Profile;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/generic-formal-subprogram-call.adb");
      Row.Concrete_Subrule :=
        To_Unbounded_String
          ("generic formal subprogram actual/default calls, profile and mode"
           & "conformance, null-exclusion contracts, substitution evidence,"
           & "runtime profile checks, warning diagnostics, stale generic-call"
           & "evidence, and semantic consumers must share one source-shaped"
           & "legality result");
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1422");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1422");
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1422 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1422 precision classification");
   end Expect_Status;
   ...
   begin
      Audit.Add_Row
        (Input,
         Base_Row (1, Precision.Class_Legal,
                   Audit.Form_Generic_Formal_Subprogram_Call_Resolved));

      Row := Base_Row (2, Precision.Class_Illegal,
                       Audit.Form_Illegal_Missing_Formal_Subprogram_Actual);
      Row.Missing_Formal_Subprogram_Actual := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (3, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Form_Runtime_Profile_Check_Preserved);
      Row.Runtime_Profile_Check_Preserved := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (6, Precision.Class_Legal,
                       Audit.Form_Default_Subprogram_Call_Resolved);
      Row.Closure := Audit.Closure_Generic_Instance_Substitution;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Assert (Audit.Gap_Remediated (Results),
              "pass1422 should close the generic formal subprogram call gap");
      ...
      Expect_Status
        (Results, 1,
         Audit.Status_Generic_Formal_Subprogram_Call_Resolved,
         Precision.Class_Legal);
      Expect_Status
        (Results, 2,
         Audit.Status_Illegal_Missing_Formal_Subprogram_Actual,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 3,
         Audit.Status_Runtime_Profile_Check_Preserved,
      ...
      Expect_Status
        (Results, 6,
         Audit.Status_Default_Subprogram_Call_Resolved,
         Precision.Class_Legal);
   ...
   begin
      ...
      Row := Base_Row (11);
      Row.Default_Subprogram_Profile_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Mode_Conformance_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Null_Exclusion_Actual_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Abstract_Subprogram_Actual := True;
      Audit.Add_Row (Input, Row);
      ...
      Row := Base_Row (16);
      Row.Stale_Generic_Call_Evidence := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (17);
      Row.Missing_Substitution_Evidence := True;
      Row.Complete_Substitution_Evidence := False;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 11,
         Audit.Status_Illegal_Default_Subprogram_Profile_Mismatch,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 12,
         Audit.Status_Illegal_Mode_Conformance_Mismatch,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 13,
         Audit.Status_Illegal_Null_Exclusion_Actual_Mismatch,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 14,
         Audit.Status_Illegal_Abstract_Subprogram_Actual,
         Precision.Class_Illegal);
      Expect_Status
        (Results, 15,
         Audit.Status_Indeterminate_Private_Formal_View,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 16,
         Audit.Status_Indeterminate_Stale_Generic_Call_Evidence,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 17,
   ...
   begin
      ...
      Row := Base_Row (26);
      Row.Formal_Subprogram_Fingerprint := 1;
      Row.Expected_Formal_Subprogram_Fingerprint := 2;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (27);
      Row.Substitution_Fingerprint := 1;
      Row.Expected_Substitution_Fingerprint := 2;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status
        (Results, 26,
         Audit.Status_Formal_Subprogram_Fingerprint_Mismatch,
         Precision.Class_Indeterminate);
      Expect_Status
        (Results, 27,
   ...
   begin
      Register_Routine
        (T, Test_Gap_Remediated'Access,
         "generic formal subprogram call gap remediated");
      Register_Routine
        (T, Test_Rejections'Access,
         "generic formal subprogram call rejection cases");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1422;
