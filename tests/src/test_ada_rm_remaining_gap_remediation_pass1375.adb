...
with Editor.Ada_RM_Remaining_Gap_Remediation_Pass1375;

package body Test_Ada_RM_Remaining_Gap_Remediation_Pass1375 is

   package Audit renames Editor.Ada_RM_Remaining_Gap_Remediation_Pass1375;
   ...
   use type Audit.Fixed_Point_Context;
   use type Audit.Fixed_Point_Form;
   ...
   begin
      return AUnit.Format ("Test_Ada_RM_Remaining_Gap_Remediation_Pass1375");
   end Name;

   function Base_Row
     (Id : Natural;
      Expected : Audit.Precision_Classification := Precision.Class_Legal;
      Form : Audit.Fixed_Point_Form := Audit.Fixed_Compatible)
      return Audit.Remediation_Row is
      Row : Audit.Remediation_Row;
   begin
      Row.Id := Id;
      Row.Gap := Audit.Remaining_Fixed_Point_Conversion_Rounding_Edge;
      ...
      Row.Context := Audit.Context_Fixed_Point_Conversion;
      Row.Form := Form;
      Row.Source_File := To_Unbounded_String ("src/fixed-point-conversion-rounding.adb");
      ...
      Row.Candidate_Implementing_Package :=
        To_Unbounded_String ("Editor.Ada_RM_Remaining_Gap_Remediation_Pass1375");
      Row.Candidate_Pass := To_Unbounded_String ("Pass1375");
      Row.Blocker_Family :=
   ...
   begin
      Assert (Item.Status = Status, "unexpected pass1375 status");
      Assert (Audit.Expected_For_Status (Item.Status) = Expected,
              "unexpected pass1375 precision classification");
   end Expect_Status;

   procedure Test_Fixed_Point_Gap_Remediated
   ...
   begin
      ...
      Expect_Status (Results, 1, Audit.Status_Legal_Fixed_Point_Conversion_Agreement,
                     Precision.Class_Legal);
      Expect_Status (Results, 2, Audit.Status_Illegal_Static_Range_Overflow,
                     Precision.Class_Illegal);
      Expect_Status (Results, 3, Audit.Status_Runtime_Range_Check_Preserved,
                     Precision.Class_Legal_With_Runtime_Check);
      Expect_Status (Results, 4, Audit.Status_Indeterminate_Missing_Delta_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Fixed_Point_Gap_Remediated;

   procedure Test_Fixed_Point_Static_Rejections
   ...
   begin
      Row := Base_Row (10);
      Row.Static_Delta_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (11);
      Row.Static_Digits_Mismatch := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (12);
      Row.Non_Static_Delta_Required := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (13);
      Row.Universal_Real_Resolution_Valid := False;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (14);
      Row.Rounding_Mode_Agrees := False;
      Audit.Add_Row (Input, Row);
      ...
      Expect_Status (Results, 10, Audit.Status_Illegal_Static_Delta_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 11, Audit.Status_Illegal_Static_Digits_Mismatch,
                     Precision.Class_Illegal);
      Expect_Status (Results, 12, Audit.Status_Illegal_Non_Static_Delta_Required,
                     Precision.Class_Illegal);
      Expect_Status (Results, 13, Audit.Status_Illegal_Universal_Real_Resolution,
                     Precision.Class_Illegal);
      Expect_Status (Results, 14, Audit.Status_Illegal_Rounding_Mode_Disagreement,
                     Precision.Class_Illegal);
      Expect_Status (Results, 15, Audit.Status_Illegal_Consumer_Surface_Disagreement,
                     Precision.Class_Illegal);
   end Test_Fixed_Point_Static_Rejections;

   procedure Test_Runtime_And_Indeterminate_Fixed_Point
   ...
   begin
      Row := Base_Row (20, Precision.Class_Legal_With_Runtime_Check,
                       Audit.Fixed_Runtime_Predicate_Check);
      Row.Runtime_Predicate_Check := True;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (21, Precision.Class_Indeterminate,
                       Audit.Fixed_Missing_Target_Fixed_Type);
      Row.Target_Fixed_Type_Present := False;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (22, Precision.Class_Indeterminate,
                       Audit.Fixed_Missing_Digits_Evidence);
      Row.Digits_Evidence_Present := False;
      Audit.Add_Row (Input, Row);

      Row := Base_Row (23, Precision.Class_Indeterminate,
                       Audit.Fixed_Stale_Static_Evidence);
      Row.Stale_Static_Evidence := True;
      Audit.Add_Row (Input, Row);

      Results := Audit.Build (Input);

      Expect_Status (Results, 20, Audit.Status_Runtime_Predicate_Check_Preserved,
                     Precision.Class_Legal_With_Runtime_Check);
      Expect_Status (Results, 21, Audit.Status_Indeterminate_Missing_Target_Fixed_Type,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 22, Audit.Status_Indeterminate_Missing_Digits_Evidence,
                     Precision.Class_Indeterminate);
      Expect_Status (Results, 23, Audit.Status_Indeterminate_Stale_Static_Evidence,
                     Precision.Class_Indeterminate);
   end Test_Runtime_And_Indeterminate_Fixed_Point;
   ...
   begin
      Register_Routine
        (T, Test_Fixed_Point_Gap_Remediated'Access,
         "fixed-point conversion gap remediated");
      Register_Routine
        (T, Test_Fixed_Point_Static_Rejections'Access,
         "fixed-point static rejections");
      Register_Routine
        (T, Test_Runtime_And_Indeterminate_Fixed_Point'Access,
         "runtime and indeterminate fixed-point cases");
      Register_Routine
        (T, Test_Inventory_And_Fingerprint_Gates'Access,
         "inventory and fingerprint gates");
   end Register_Tests;

end Test_Ada_RM_Remaining_Gap_Remediation_Pass1375;
