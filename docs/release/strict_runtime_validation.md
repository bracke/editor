...
### Pass968 - generic formal/actual kind conformance validation
...
- Phase 579 pass971: generic body contract visibility must remain parser-owned and bounded; body-region formal lookup must not read files, reload buffers, or mutate dirty state.

- Phase 579 pass972: generic subprogram actual overload selection must enumerate only parser-owned direct-visibility declarations and must not perform file IO, reload buffers, or mutate dirty state.
...
Pass976 strict validation note: generic formal subprogram null-exclusion/access-profile conformance remains deterministic, bounded, and snapshot-owned.


Pass977 strict validation note: include generic instantiation cases where a formal subprogram expects Convention => C and the actual is default Ada, and the reverse case where a default-Ada formal receives a C-convention actual.

Pass978 strict validation note: include generic instantiation cases where a formal subprogram exposes a defaulted parameter and the actual subprogram parameter is required, plus the inverse case where a required formal parameter receives a defaulted actual parameter.

Pass983 extends subtype compatibility with private-view-aware effective type views. The stricter path must remain deterministic, snapshot-owned, and free of rendering-side parsing or dirty-state mutation.
...
Pass1036 records generic renaming and nested generic instantiation visibility as immutable semantic projection metadata only.
Pass1037 records generic object default-expression type conformance as immutable semantic projection metadata only.
...
Pass1105: Validate generic instance/freezing/representation closure as deterministic semantic metadata only, with no mutable editor-surface effects.

Pass1106: Validate cross-unit semantic closure as deterministic semantic metadata only, with no mutable editor-surface effects.
...
Pass1112 strict validation: contract/aspect legality remains projection-free and deterministic.
...
Pass1123 validation note: Global/Depends dataflow legality remains snapshot-owned and routes blockers through integrated semantic closure without render/workspace/keybinding mutation.
...
Pass1191 validation: final representation/freezing hard-case facts must remain snapshot-owned and must not mutate files, buffers, dirty state, rendering, command routing, or workspace state.
