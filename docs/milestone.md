# Milestone Startup and Dogfood Readiness

packages the current dogfood editor workflow as a repeatable local milestone. It keeps startup and reload behavior understandable while preserving the existing transient-state boundaries.
...
Startup does not create project data, sample rows, reserved content, build candidates, search results, quick-open matches, outline rows, diagnostics rows, or persisted command-palette state. Startup also does not run build/search/outline refresh and does not write settings, keybindings, workspace, or recent-project files unless an explicit save/reset command is invoked.
...
After a project is open, show or refresh the File Tree to browse real project files. Activating a file-tree node opens the file through the canonical file-open path. No synthetic file-tree rows are shown in product startup or normal project flows.
