...
The Messages feature is a session-local feature-panel surface for editor messages.
It is not a compiler diagnostics engine, LSP diagnostics store, project-wide analyzer,
search-results model, persistent history, or background event queue.

Messages are owned by `Editor.Feature_Messages`, not by the generic feature panel.
The feature panel stores only the current projection rows, selection, reveal state, and
projection generation. Messages may optionally carry buffer targets; activation is
allowed only after the current projection generation, row mapping, and target metadata
all validate.

Messages are cleared on workspace reset and project reset. Buffer-close cleanup removes
messages that target the closed buffer so no activatable row can point at a closed buffer.
