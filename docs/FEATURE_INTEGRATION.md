...
The editor now treats the Feature Panel as a real product surface rather than a
placeholder container. Current feature-panel-backed workflows include Outline,
Messages, Search Results, Diagnostics, and Build Output / Build Result details.

Feature integration rules:

- feature rows must be produced from the current editor state or from an explicit
  test fixture seam;
- product commands must route through the Executor instead of mutating render
  rows directly;
- hidden/internal fixtures must not leak into command palette labels,
  keybindings, workspace persistence, or release documentation;
- feature rows must have stable labels, selection behavior, and empty-state
  behavior;
- diagnostics/build rows remain review surfaces and do not own compiler/LSP
  analysis state.

The release checks guard against reintroducing removed placeholder feature-panel
claims. Tests may still use deterministic fixture data, but fixture data must be
scoped to tests and must not be documented as the product behavior.
