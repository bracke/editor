...
freezes the feature-panel architecture after four real feature-panel-backed features: Outline, Messages, Search Results, and Diagnostics. The feature panel remains an explicit built-in subsystem; it is not a dynamic plugin host and it does not load feature descriptors dynamically.
